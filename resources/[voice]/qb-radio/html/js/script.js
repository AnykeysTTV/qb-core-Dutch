$(function() {
    ...
    document.onkeyup = function (data) {
        if (data.which == 27) { // Escape key
            $.post('https://poly-radio/escape', JSON.stringify({}));
            QBRadio.SlideDown()
        } else if (data.which == 13) { // Enter key
            $.post('https://poly-radio/joinRadio', JSON.stringify({
                channel: $("#channel").val()
            }));
        }
    };
...
$(document).on('click', '#disconnect', function(e){
    ...
    $.post('https://poly-radio/leaveRadio');
});

$(document).on('click', '#volumeUp', function(e){
    e.preventDefault();

    $.post('https://poly-radio/volumeUp', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#volumeDown', function(e){
    ...
    $.post('https://poly-radio/volumeDown', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#decreaseradiochannel', function(e){
    ...
    $.post('https://poly-radio/decreaseradiochannel', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#increaseradiochannel', function(e){
    ...
    $.post('https://poly-radio/increaseradiochannel', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#poweredOff', function(e){
    ...
    $.post('https://poly-radio/poweredOff', JSON.stringify({
        channel: $("#channel").val()
    }));
});

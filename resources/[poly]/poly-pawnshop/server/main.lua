...
local function exploitBan(id, reason)
    ...
    TriggerEvent('poly-log:server:CreateLog', 'pawnshop', 'Player Banned', 'red',
        string.format('%s was banned by %s for %s', GetPlayerName(id), 'poly-pawnshop', reason), true)
    DropPlayer(id, 'You were permanently banned by the server for: Exploiting')
end

RegisterNetEvent('poly-pawnshop:server:sellPawnItems', function(itemName, itemAmount, itemPrice)
    local src = source
    ...
    TriggerClientEvent('poly-pawnshop:client:openMenu', src)
end)

RegisterNetEvent('poly-pawnshop:server:meltItemRemove', function(itemName, itemAmount, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.Functions.RemoveItem(itemName, itemAmount) then
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[itemName], 'remove')
        local meltTime = (tonumber(itemAmount) * item.time)
        TriggerClientEvent('poly-pawnshop:client:startMelting', src, item, tonumber(itemAmount), (meltTime * 60000 / 1000))
...
RegisterNetEvent('poly-pawnshop:server:pickupMelted', function(item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local playerCoords = GetEntityCoords(GetPlayerPed(src))
    local dist
    ...
    TriggerClientEvent('poly-pawnshop:client:resetPickup', src)
    TriggerClientEvent('poly-pawnshop:client:openMenu', src)
end)

QBCore.Functions.CreateCallback('poly-pawnshop:server:getInv', function(source, cb)

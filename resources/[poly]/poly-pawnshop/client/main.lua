...
CreateThread(function()
    if Config.UseTarget then
        for key, value in pairs(Config.PawnLocation) do
            exports['poly-target']:AddBoxZone('PawnShop'..key, value.coords, value.length, value.width, {
                name = 'PawnShop'..key,
                ...
                options = {
                    {
                        type = 'client',
                        event = 'poly-pawnshop:client:openMenu',
                        icon = 'fas fa-ring',
                        label = 'Pawn Shop',
                    },
                },
...
RegisterNetEvent('poly-pawnshop:client:openMenu', function()
        ...
        if GetClockHours() >= Config.TimeOpen and GetClockHours() <= Config.TimeClosed then
            local pawnShop = {
                ...
                },
                {
                    ...
                    params = {
                        event = 'poly-pawnshop:client:openPawn',
                        args = {
            ...
            if not isMelting then
                pawnShop[#pawnShop + 1] = {
                    ...
                    params = {
                        event = 'poly-pawnshop:client:openMelt',
                        args = {
            ...
            if canTake then
                pawnShop[#pawnShop + 1] = {
                    ...
                    params = {
                        isServer = true,
                        event = 'poly-pawnshop:server:pickupMelted',
                        args = {
                            items = meltedItem
                        }
                    }
            ...
            exports['poly-menu']:openMenu(pawnShop)
        else
...
RegisterNetEvent('poly-pawnshop:client:openPawn', function(data)
    QBCore.Functions.TriggerCallback('poly-pawnshop:server:getInv', function(inventory)
            ...
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    pawnMenu[#pawnMenu + 1] = {
                        ...
                        params = {
                            event = 'poly-pawnshop:client:pawnitems',
                            args = {
        ...
        exports['poly-menu']:openMenu(pawnMenu)
    end)
end)

RegisterNetEvent('poly-pawnshop:client:openMelt', function(data)
    QBCore.Functions.TriggerCallback('poly-pawnshop:server:getInv', function(inventory)
        local PlyInv = inventory
        local meltMenu = {
            {
            ...
            for i = 1, #data.items do
                if v.name == data.items[i].item then
                    meltMenu[#meltMenu + 1] = {
                        ...
                        params = {
                            event = 'poly-pawnshop:client:meltItems',
                            args = {
                                label = QBCore.Shared.Items[v.name].label,
        ...
        exports['poly-menu']:openMenu(meltMenu)
    end)
end)

RegisterNetEvent('poly-pawnshop:client:pawnitems', function(item)
    local sellingItem = exports['poly-input']:ShowInput({
        header = Lang:t('info.title'),
    ...
    if sellingItem then
        ...
        if tonumber(sellingItem.amount) > 0 then
            TriggerServerEvent('poly-pawnshop:server:sellPawnItems', item.name, sellingItem.amount, item.price)
        else
...
RegisterNetEvent('poly-pawnshop:client:meltItems', function(item)
    local meltingItem = exports['poly-input']:ShowInput({
        header = Lang:t('info.melt'),
    ...
    if meltingItem then
        ...
        if meltingItem.amount ~= nil then
            if tonumber(meltingItem.amount) > 0 then
                TriggerServerEvent('poly-pawnshop:server:meltItemRemove', item.name, meltingItem.amount, item)
...
RegisterNetEvent('poly-pawnshop:client:startMelting', function(item, meltingAmount, meltTimees)
    if not isMelting then
        isMelting = true
        meltTime = meltTimees
        meltedItem = {}
...
RegisterNetEvent('poly-pawnshop:client:resetPickup', function()
    canTake = false
end)

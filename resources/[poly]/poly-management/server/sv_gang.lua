...
RegisterNetEvent("poly-gangmenu:server:withdrawMoney", function(amount)
	local src = source
	...
	if RemoveGangMoney(gang, amount) then
		...
		TriggerEvent('poly-log:server:CreateLog', 'gangmenu', 'Withdraw Money', 'yellow', Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname .. ' successfully withdrew $' .. amount .. ' (' .. gang .. ')', false)
	...
	TriggerClientEvent('poly-gangmenu:client:OpenMenu', src)
end)

RegisterNetEvent("poly-gangmenu:server:depositMoney", function(amount)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	...
	if Player.Functions.RemoveMoney("cash", amount) then
		...
		TriggerEvent('poly-log:server:CreateLog', 'gangmenu', 'Deposit Money', 'yellow', Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname .. ' successfully deposited $' .. amount .. ' (' .. gang .. ')', false)
...
QBCore.Functions.CreateCallback('poly-gangmenu:server:GetAccount', function(_, cb, GangName)
	local gangmoney = GetGangAccount(GangName)
	cb(gangmoney)
end)

-- Get Employees
QBCore.Functions.CreateCallback('poly-gangmenu:server:GetEmployees', function(source, cb, gangname)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
-- Grade Change
RegisterNetEvent('poly-gangmenu:server:GradeUpdate', function(data)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
-- Fire Member
RegisterNetEvent('poly-gangmenu:server:FireMember', function(target)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			...
			if Employee.Functions.SetGang("none", '0') then
				TriggerEvent("poly-log:server:CreateLog", "gangmenu", "Gang Fire", "orange", Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname .. ' successfully fired ' .. Employee.PlayerData.charinfo.firstname .. " " .. Employee.PlayerData.charinfo.lastname .. " (" .. Player.PlayerData.gang.name .. ")", false)
...
-- Recruit Player
RegisterNetEvent('poly-gangmenu:server:HireMember', function(recruit)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	...
	if Target and Target.Functions.SetGang(Player.PlayerData.gang.name, 0) then
		...
		TriggerEvent('poly-log:server:CreateLog', 'gangmenu', 'Recruit', 'yellow', (Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname).. ' successfully recruited ' .. Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname .. ' (' .. Player.PlayerData.gang.name .. ')', false)
	end
	TriggerClientEvent('poly-gangmenu:client:OpenMenu', src)
end)

-- Get closest player sv
QBCore.Functions.CreateCallback('poly-gangmenu:getplayers', function(source, cb)
	local src = source
	local players = {}

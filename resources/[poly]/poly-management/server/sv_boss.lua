...
function ExploitBan(id, reason)
	...
	TriggerEvent('poly-log:server:CreateLog', 'bans', 'Player Banned', 'red', string.format('%s was banned by %s for %s', GetPlayerName(id), 'poly-management', reason), true)
...
RegisterNetEvent("poly-bossmenu:server:withdrawMoney", function(amount)
	local src = source
	...
	if RemoveMoney(job, amount) then
		...
		TriggerEvent('poly-log:server:CreateLog', 'bossmenu', 'Withdraw Money', "blue", Player.PlayerData.name.. "Withdrawal $" .. amount .. ' (' .. job .. ')', false)
	...
	TriggerClientEvent('poly-bossmenu:client:OpenMenu', src)
end)

RegisterNetEvent("poly-bossmenu:server:depositMoney", function(amount)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	...
	if Player.Functions.RemoveMoney("cash", amount) then
		local job = Player.PlayerData.job.name
		AddMoney(job, amount)
		TriggerEvent('poly-log:server:CreateLog', 'bossmenu', 'Deposit Money', "blue", Player.PlayerData.name.. "Deposit $" .. amount .. ' (' .. job .. ')', false)
...
QBCore.Functions.CreateCallback('poly-bossmenu:server:GetAccount', function(_, cb, jobname)
	local result = GetAccount(jobname)
	cb(result)
end)

-- Get Employees
QBCore.Functions.CreateCallback('poly-bossmenu:server:GetEmployees', function(source, cb, jobname)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
-- Grade Change
RegisterNetEvent('poly-bossmenu:server:GradeUpdate', function(data)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
-- Fire Employee
RegisterNetEvent('poly-bossmenu:server:FireEmployee', function(target)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	...
	if Employee then
		if target ~= Player.PlayerData.citizenid then
			...
			if Employee.Functions.SetJob("unemployed", '0') then
				TriggerEvent("poly-log:server:CreateLog", "bossmenu", "Job Fire", "red", Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname .. ' successfully fired ' .. Employee.PlayerData.charinfo.firstname .. " " .. Employee.PlayerData.charinfo.lastname .. " (" .. Player.PlayerData.job.name .. ")", false)
...
-- Recruit Player
RegisterNetEvent('poly-bossmenu:server:HireEmployee', function(recruit)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	...
	if Target and Target.Functions.SetJob(Player.PlayerData.job.name, 0) then
		...
		TriggerEvent('poly-log:server:CreateLog', 'bossmenu', 'Recruit', "lightgreen", (Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname).. " successfully recruited " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. ' (' .. Player.PlayerData.job.name .. ')', false)
	end
	TriggerClientEvent('poly-bossmenu:client:OpenMenu', src)
end)

-- Get closest player sv
QBCore.Functions.CreateCallback('poly-bossmenu:getplayers', function(source, cb)
	local src = source
	local players = {}

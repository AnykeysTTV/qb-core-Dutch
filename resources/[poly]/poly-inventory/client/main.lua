...
local function FormatWeaponAttachments(itemdata)
    ...
    if itemdata.info.attachments ~= nil and next(itemdata.info.attachments) ~= nil then
        for _, v in pairs(itemdata.info.attachments) do
            if exports['poly-weapons']:getConfigWeaponAttachments(itemdata.name) then
                for key, value in pairs(exports['poly-weapons']:getConfigWeaponAttachments(itemdata.name)) do
...
local function CreateItemDrop(index)
	...
	if Config.UseTarget then
		exports['poly-target']:AddTargetEntity(dropItem, {
			options = {
				{
...
RegisterNetEvent("poly-inventory:client:closeinv", function()
    closeInventory()
end)
...
RegisterNetEvent('poly-inventory:client:giveAnim', function()
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    ...
    local Attachment = exports['poly-weapons']:getConfigWeaponAttachments(WeaponData.name:upper())[data.AttachmentData.attachment]
    QBCore.Functions.TriggerCallback('weapons:server:RemoveAttachment', function(NewAttachments)
        if NewAttachments ~= false then
            ...
            for _, v in pairs(NewAttachments) do
                for _, pew in pairs(exports['poly-weapons']:getConfigWeaponAttachments(WeaponData.name:upper())) do
...
CreateThread(function()
    if Config.UseTarget then
        exports['poly-target']:AddTargetModel(Config.VendingObjects, {
            options = {
                {
...
CreateThread(function()
    if Config.UseTarget then
        exports['poly-target']:AddTargetModel(Config.CraftingObject, {
            options = {
                {

...
local function ShowEntranceHeaderMenu()
    ...
    exports['poly-menu']:showHeader(headerMenu)
end
    ...
    QBCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            QBCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['poly-interior']:CreateApartmentFurnished(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                Wait(100)
                TriggerServerEvent('poly-apartments:server:SetInsideMeta', house, apartmentId, true, false)
            ...
            TriggerEvent('poly-weathersync:client:DisableSync')
            Wait(100)
            TriggerServerEvent('poly-apartments:server:SetInsideMeta', house, apartmentId, true, true)
            TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_close", 0.1)
            TriggerServerEvent("apartments:server:setCurrentApartment", CurrentApartment)
        end
        ...
        if new ~= nil then
            if new then
                TriggerEvent('poly-interior:client:SetNewState', true)
            else
                TriggerEvent('poly-interior:client:SetNewState', false)
            end
        else
            TriggerEvent('poly-interior:client:SetNewState', false)
        end
...
local function LeaveApartment(house)
    TriggerServerEvent("InteractSound_SV:PlayOnSource", "houses_door_open", 0.1)
    openHouseAnim()
    TriggerServerEvent("poly-apartments:returnBucket")
    DoScreenFadeOut(500)
    ...
    exports['poly-interior']:DespawnInterior(HouseObj, function()
        ...
        TriggerServerEvent('poly-apartments:server:SetInsideMeta', CurrentApartment, false)
...
function MenuOwners()
    QBCore.Functions.TriggerCallback('apartments:GetAvailableApartments', function(apartments)
        if next(apartments) == nil then
            ...
            exports['poly-menu']:openMenu(apartmentMenu)
        end
    end, ClosestHouse)
end
...
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    QBCore.Functions.TriggerCallback('apartments:GetOwnedApartment', function(result)
        if result then
            TriggerEvent('poly-spawn:client:setupSpawns', cData, false, nil)
            TriggerEvent('poly-spawn:client:openUI', true)
            TriggerEvent("apartments:client:SetHomeBlip", result.type)
        else
            if Apartments.Starting then
                TriggerEvent('poly-spawn:client:setupSpawns', cData, true, Apartments.Locations)
                TriggerEvent('poly-spawn:client:openUI', true)
            else
                TriggerEvent('poly-spawn:client:setupSpawns', cData, false, nil)
                TriggerEvent('poly-spawn:client:openUI', true)
...
RegisterNetEvent('poly-apartments:client:LastLocationHouse', function(apartmentType, apartmentId)

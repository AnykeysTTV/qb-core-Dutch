...
local function hintToDisplay(text)
	exports['poly-core']:DrawText(text)
end
...
RegisterNetEvent('poly-armoredtruckheist:client:911alert', function()
	if PoliceAlert == 0 then
		...
		TriggerServerEvent("poly-armoredtruckheist:server:callCops", streetLabel, transCoords)
		PlaySoundFrontend(-1, "Mission_Pass_Notify", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
		PoliceAlert = 1
	end
end)

RegisterNetEvent('poly-armoredtruckheist:client:robberyCall', function(streetLabel, coords)
	if PlayerJob.name == "police" then
		local store = "Armored Truck"
		PlaySound(-1, "Lose_1st", "GTAO_FM_Events_Soundset", 0, 0, 1)
		TriggerEvent('poly-policealerts:client:AddPoliceAlert', {
			timeOut = 10000,
				...
				if dist <= 7 and BlownUp == 0 and PlayerJob.name ~= 'police' then
					if BlowBackdoor == 0 then
						...
						if IsControlJustPressed(0, 47) then
							...
							TriggerEvent("poly-armoredtruckheist:client:911alert")
							hideLastHint()
							sleep = 500
						end
					end

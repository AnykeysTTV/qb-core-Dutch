...
QBCore.Functions.CreateCallback('poly-doorlock:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

QBCore.Functions.CreateCallback('poly-doorlock:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('poly-doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	TriggerClientEvent('poly-doorlock:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		if Config.DoorStates[doorID] == nil then Config.DoorStates[doorID] = locked elseif Config.DoorStates[doorID] ~= locked then Config.DoorStates[doorID] = nil end
		TriggerClientEvent('poly-doorlock:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)

RegisterNetEvent('poly-doorlock:server:saveNewDoor', function(data, doubleDoor)
	local src = source
	...
	TriggerClientEvent('poly-doorlock:client:newDoorAdded', -1, configData, identifier, src)
end)
...
RegisterNetEvent('poly-doorlock:server:removeLockpick', function(type)
	local Player = QBCore.Functions.GetPlayer(source)
...
QBCore.Commands.Add('newdoor', Lang:t("general.newdoor_command_description"), {}, false, function(source)
	TriggerClientEvent('poly-doorlock:client:addNewDoor', source)
...
QBCore.Commands.Add('doordebug', Lang:t("general.doordebug_command_description"), {}, false, function(source)
	TriggerClientEvent('poly-doorlock:client:ToggleDoorDebug', source)
end, Config.CommandPermission)

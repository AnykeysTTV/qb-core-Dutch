...
local function lockpickFinish(success)
	if success then
		...
		Wait(1800)
		TriggerServerEvent('poly-doorlock:server:updateState', closestDoor.id, false, false, true, false) -- Broadcast new state of the door to everyone
	else
		QBCore.Functions.Notify(Lang:t("error.lockpick_fail"), 'error', 2500)
		if math.random(1,100) <= 17 then
			if usingAdvanced then
				TriggerServerEvent("poly-doorlock:server:removeLockpick", "advancedlockpick")
				TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["advancedlockpick"], "remove")
			else
				TriggerServerEvent("poly-doorlock:server:removeLockpick", "lockpick")
...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		QBCore.Functions.TriggerCallback('poly-doorlock:server:checkItems', function(result)
			p:resolve(result)
...
function SetupDoors()
	local p = promise.new()
	QBCore.Functions.TriggerCallback('poly-doorlock:server:setupDoors', function(result)
		p:resolve(result)
	end)
...
RegisterNetEvent('poly-doorlock:client:setState', function(serverId, doorID, state, src, enableAnimation, enableSounds)
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
	...
	TriggerEvent('poly-lockpick:client:openLockpick', lockpickFinish)
end)

RegisterNetEvent('poly-doorlock:client:addNewDoor', function()
	canContinue = false
	hideNUI()
	...
	if doorData.doortype == 'door' or doorData.doortype == 'sliding' or doorData.doortype == 'garage' then
		...
		TriggerServerEvent('poly-doorlock:server:saveNewDoor', doorData, false)
		canContinue = true
	else
		...
		doorData.model = model
		doorData.heading = heading
		TriggerServerEvent('poly-doorlock:server:saveNewDoor', doorData, true)
		canContinue = true
	end
end)

RegisterNetEvent('poly-doorlock:client:newDoorAdded', function(data, id, creatorSrc)
	Config.DoorList[id] = data
	TriggerEvent('poly-doorlock:client:setState', creatorSrc, id, data.locked, false, true, true)
end)

RegisterNetEvent('poly-doorlock:client:ToggleDoorDebug', function()
...
RegisterCommand('toggledoorlock', function()
	...
	TriggerServerEvent('poly-doorlock:server:updateState', closestDoor.id, locked, src, false, false, true, true) -- Broadcast new state of the door to everyone
end, false)
...
RegisterCommand('remotetriggerdoor', function()
	...
	TriggerServerEvent('poly-doorlock:server:updateState', nearestDoor.id, not nearestDoor.data.locked, NetworkGetNetworkIdFromEntity(playerPed), false, false, true, true) -- Broadcast new state of the door to everyone
end, false)

...
local function RobberyPed()
    if Config.UseTarget then
        exports['poly-target']:AddEntityZone('stealingPed', stealingPed, {
            name = 'stealingPed',
            debugPoly = false,
        }, {
            options = {
                {
                    ...
                    action = function()
                        ...
                        TriggerServerEvent('poly-drugs:server:giveStealItems', stealData.drugType, stealData.amount)
                        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[stealData.item], "add")
                        stealingPed = nil
                        stealData = {}
                        exports['poly-target']:RemoveZone('stealingPed')
                    end,
                ...
                if IsEntityDead(stealingPed) then
                    ...
                    if not Config.UseTarget and #(pos - pedpos) < 1.5 then
                        if not textDrawn then
                            textDrawn = true
                            exports['poly-core']:DrawText(Lang:t("info.pick_up_button"))
                        end
...
local function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 and not IsPedDeadOrDying(ped) do
            ...
            if getRobbed <= Config.RobberyChance then
                TriggerServerEvent('poly-drugs:server:robCornerDrugs', drugType, bagAmount)
                ...
                if pedDist2 < 1.5 and cornerselling then
                    if Config.UseTarget and not zoneMade then
                        zoneMade = true
                        exports['poly-target']:AddEntityZone('sellingPed', ped, {
                            name = 'sellingPed',
                            debugPoly = false,
                        }, {
                            options = {
                                {
                                    ...
                                    action = function(entity)
                                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                                            ...
                                            exports['poly-target']:RemoveZone('sellingPed')
                                            return
                                        else
                                            QBCore.Functions.Progressbar("cornerSelling", Lang:t("info.selling_to_ped"), '5000', false, false, {
                                                ...
                                                TriggerServerEvent('poly-drugs:server:sellCornerDrugs', drugType, bagAmount, randomPrice)
                                                hasTarget = false
                    ...
                    elseif not Config.UseTarget then
                        if not textDrawn then
                            textDrawn = true
                            exports['poly-core']:DrawText(Lang:t("info.drug_offer", {bags = bagAmount, drugLabel = currentOfferDrug.label, randomPrice = randomPrice}))
                        end
...
-- Events
RegisterNetEvent('poly-drugs:client:cornerselling', function()
    QBCore.Functions.TriggerCallback('poly-drugs:server:cornerselling:getAvailableDrugs', function(result)
...
RegisterNetEvent('poly-drugs:client:refreshAvailableDrugs', function(items)

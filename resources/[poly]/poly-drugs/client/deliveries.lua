...
AddStateBagChangeHandler('isLoggedIn', nil, function(_, _, value)
    if value then
        QBCore.Functions.TriggerCallback('poly-drugs:server:RequestConfig', function(DealerConfig)
...
local function KnockDoorAnim(home)
    ...
    if home then
        ...
        exports['poly-core']:DrawText(Lang:t("info.other_dealers_button"), 'left')
        AwaitingInput()
    else
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "knock_door", 0.2)
        Wait(100)
...
local function RequestDelivery()
    if not waitingDelivery then
        ...
        TriggerServerEvent('poly-drugs:server:giveDeliveryItems', waitingDelivery)
        SetTimeout(2000, function()
            TriggerServerEvent('poly-phone:server:sendNewMail', {
                ...
                button = {
                    enabled = true,
                    buttonEvent = "poly-drugs:client:setLocation",
...
local function DeliverStuff()
    if deliveryTimeout > 0 then
        ...
        QBCore.Functions.Progressbar("work_dropbox", Lang:t("info.delivering_products"), 3500, false, true, {
            ...
            TriggerServerEvent('poly-drugs:server:successDelivery', activeDelivery, true)
            activeDelivery = nil
            if Config.UseTarget then
                exports['poly-target']:RemoveZone('drugDeliveryZone')
            else
        ...
        TriggerServerEvent('poly-drugs:server:successDelivery', activeDelivery, false)
    end
    deliveryTimeout = 0
end
...
function InitZones()
    ...
    if Config.UseTarget then
        for k,v in pairs(Config.Dealers) do
            exports["poly-target"]:AddBoxZone("dealer_"..k, vector3(v.coords.x, v.coords.y, v.coords.z), 1.5, 1.5, {
                name = "dealer_"..k,
                heading = v.heading,
                minZ = v.coords.z - 1,
                maxZ = v.coords.z + 1,
        ...
        dealerCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if not dealerIsHome then
                    exports['poly-core']:DrawText(Lang:t("info.knock_button"),'left')
                    AwaitingInput()
                elseif dealerIsHome then
                    exports['poly-core']:DrawText(Lang:t("info.other_dealers_button"), 'left')
                    AwaitingInput()
...
RegisterNetEvent('poly-drugs:client:RefreshDealers', function(DealerData)
    if not Config.UseTarget and dealerCombo then dealerCombo:destroy() end
...
RegisterNetEvent('poly-drugs:client:updateDealerItems', function(itemData, amount)
    TriggerServerEvent('poly-drugs:server:updateDealerItems', itemData, amount, currentDealer)
end)

RegisterNetEvent('poly-drugs:client:setDealerItems', function(itemData, amount, dealer)
    Config.Dealers[dealer]["products"][itemData.slot].amount = Config.Dealers[dealer]["products"][itemData.slot].amount - amount
end)

RegisterNetEvent('poly-drugs:client:setLocation', function(locationData)
    ...
    SetMapBlip(activeDelivery["coords"]["x"], activeDelivery["coords"]["y"])
    if Config.UseTarget then
        exports["poly-target"]:AddBoxZone('drugDeliveryZone', vector3(activeDelivery["coords"].x, activeDelivery["coords"].y, activeDelivery["coords"].z), 1.5, 1.5, {
        ...
        drugDeliveryZone:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                exports['poly-core']:DrawText(Lang:t("info.deliver_items_button", {itemAmount = activeDelivery["amount"], itemLabel = QBCore.Shared.Items[activeDelivery["itemData"]["item"]]["label"]}),'left')
                CreateThread(function()
...
RegisterNetEvent('poly-drugs:client:sendDeliveryMail', function(type, deliveryData)

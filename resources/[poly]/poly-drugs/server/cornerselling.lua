...
QBCore.Functions.CreateCallback('poly-drugs:server:cornerselling:getAvailableDrugs', function(source, cb)
    cb(getAvailableDrugs(source))
end)

RegisterNetEvent('poly-drugs:server:giveStealItems', function(drugType, amount)
...
RegisterNetEvent('poly-drugs:server:sellCornerDrugs', function(drugType, amount, price)
    local src = source
    ...
    if hasItem.amount >= amount then
        ...
        TriggerClientEvent('poly-drugs:client:refreshAvailableDrugs', src, getAvailableDrugs(src))
    else
        TriggerClientEvent('poly-drugs:client:cornerselling', src)
    end
end)

RegisterNetEvent('poly-drugs:server:robCornerDrugs', function(drugType, amount)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local availableDrugs = getAvailableDrugs(src)

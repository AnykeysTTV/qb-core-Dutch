...
-- Callbacks
QBCore.Functions.CreateCallback('poly-drugs:server:RequestConfig', function(_, cb)
    cb(Config.Dealers)
end)

-- Events
RegisterNetEvent('poly-drugs:server:updateDealerItems', function(itemData, amount, dealer)
    local src = source
    ...
    if Config.Dealers[dealer]["products"][itemData.slot].amount - 1 >= 0 then
        ...
        TriggerClientEvent('poly-drugs:client:setDealerItems', -1, itemData, amount, dealer)
    else
...
RegisterNetEvent('poly-drugs:server:giveDeliveryItems', function(deliveryData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-drugs:server:successDelivery', function(deliveryData, inTime)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if inTime then
        if invItem and invItem.amount >= itemAmount then -- on time correct amount
            ...
            SetTimeout(math.random(5000, 10000), function()
                TriggerClientEvent('poly-drugs:client:sendDeliveryMail', src, 'perfect', deliveryData)
            ...
            SetTimeout(math.random(5000, 10000), function()
                TriggerClientEvent('poly-drugs:client:sendDeliveryMail', src, 'bad', deliveryData)
                if curRep - 1 > 0 then
        ...
        if invItem and invItem.amount >= itemAmount then -- late correct amount
            ...
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[item], "remove")
            SetTimeout(math.random(5000, 10000), function()
                TriggerClientEvent('poly-drugs:client:sendDeliveryMail', src, 'late', deliveryData)
                if curRep - 1 > 0 then
                    Player.Functions.SetMetaData('dealerrep', (curRep - Config.DeliveryRepLoss))
                else
                    Player.Functions.SetMetaData('dealerrep', 0)
...
QBCore.Commands.Add("newdealer", Lang:t("info.newdealer_command_desc"), {{
    ...
    MySQL.insert('INSERT INTO dealers (name, coords, time, createdby) VALUES (?, ?, ?, ?)', {dealerName, pos, time, Player.PlayerData.citizenid}, function()
        ...
        TriggerClientEvent('poly-drugs:client:RefreshDealers', -1, Config.Dealers)
    end)
end, "admin")

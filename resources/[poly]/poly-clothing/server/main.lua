...
RegisterServerEvent("poly-clothing:saveSkin", function(model, skin)
    local src = source
...
RegisterServerEvent("poly-clothes:loadPlayerSkin", function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local result = MySQL.query.await('SELECT * FROM playerskins WHERE citizenid = ? AND active = ?', { Player.PlayerData.citizenid, 1 })
    if result[1] ~= nil then
        TriggerClientEvent("poly-clothes:loadSkin", src, false, result[1].model, result[1].skin)
    else
        TriggerClientEvent("poly-clothes:loadSkin", src, true)
    end
end)

RegisterServerEvent("poly-clothes:saveOutfit", function(outfitName, model, skinData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if model ~= nil and skinData ~= nil then
        ...
        MySQL.insert('INSERT INTO player_outfits (citizenid, outfitname, model, skin, outfitId) VALUES (?, ?, ?, ?, ?)', {
            ...
            if result[1] ~= nil then
                TriggerClientEvent('poly-clothing:client:reloadOutfits', src, result)
            else
                TriggerClientEvent('poly-clothing:client:reloadOutfits', src, nil)
            end
        end)
    end
end)

RegisterServerEvent("poly-clothing:server:removeOutfit", function(outfitName, outfitId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('poly-clothing:server:getOutfits', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local anusVal = {}

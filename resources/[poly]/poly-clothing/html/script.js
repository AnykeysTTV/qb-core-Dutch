...
QBClothing.ResetItemTexture = function(obj, category) {
    ...
    $.post('https://poly-clothing/updateSkin', JSON.stringify({
...
$(document).on('click', '.clothing-menu-option-item-right', function(e) {
    ...
    if (canChange) {
        if (hasTracker && clothingCategory == "accessory") {
            $.post('https://poly-clothing/TrackerError');
            return
        } else {
            if (clothingCategory == "model") {
                $(inputElem).val(newValue);
                $.post('https://poly-clothing/setCurrentPed', JSON.stringify({ ped: newValue }), function(model) {
...
$(document).on('change', '.item-number', function() {
    ...
    if (hasTracker && clothingCategory == "accessory") {
        ...
        if (clothingCategory == "accessory" && inputVal == 13) {
            ...
            $.post('https://poly-clothing/updateSkinOnInput', JSON.stringify({
                clothingType: clothingCategory,
...
$(document).on('click', '.clothing-menu-header-camera-btn', function(e) {
    ...
    if (selectedCam == null) {
        ...
        $.post('https://poly-clothing/setupCam', JSON.stringify({
            value: camValue
        }));
        selectedCam = this;
    } else {
...
$(document).on('keydown', function() {
    switch (event.keyCode) {
        case 68: // D
            $.post('https://poly-clothing/rotateRight');
            break;
        case 65: // A
            $.post('https://poly-clothing/rotateLeft');
            break;
        case 38: // UP
            ChangeUp();
            break;
...
$(document).on('click', "#save-menu", function(e) {
    e.preventDefault();
    QBClothing.Close();
    $.post('https://poly-clothing/saveClothing');
});

$(document).on('click', "#cancel-menu", function(e) {
    e.preventDefault();
    QBClothing.Close();
    $.post('https://poly-clothing/resetOutfit');
});
...
$(document).on('click', '.clothing-menu-outfit-option-button', function(e) {
    ...
    $.post('https://poly-clothing/selectOutfit', JSON.stringify({
...
$(document).on('click', '.clothing-menu-myOutfit-option-button-remove', function(e) {
    ...
    $.post('https://poly-clothing/removeOutfit', JSON.stringify({
        outfitData: outfitData.skin,
        outfitName: outfitData.outfitname,
        outfitId: outfitData.outfitId,
    }));
...
QBClothing.Close = function() {
    $.post('https://poly-clothing/close');
...
$(document).on('click', '#save-outfit-save', function(e) {
    ...
    $.post('https://poly-clothing/saveOutfit', JSON.stringify({
...
$(document).on('click', '.change-camera-button', function(e) {
    ...
    $.post('https://poly-clothing/rotateCam', JSON.stringify({
        type: rotationType
    }))
});

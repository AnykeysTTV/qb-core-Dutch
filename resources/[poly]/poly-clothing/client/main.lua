...
local function SaveSkin()
    ...
    TriggerServerEvent("poly-clothing:saveSkin", model, clothing)
end
...
local function reloadSkin(health)
    ...
    TriggerServerEvent("poly-clothes:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES
    TriggerServerEvent("poly-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
...
local function getOutfits(gradeLevel, data)
    ...
    QBCore.Functions.TriggerCallback('poly-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('poly-clothing:client:openMenu')
AddEventHandler('poly-clothing:client:openMenu', function()
...
RegisterNetEvent('poly-clothing:client:reloadOutfits')
AddEventHandler('poly-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
        action = "reloadMyOutfits",
        outfits = myOutfits
    })
end)
RegisterNetEvent('poly-clothes:client:CreateFirstCharacter')
AddEventHandler('poly-clothes:client:CreateFirstCharacter', function()
...
RegisterNetEvent("poly-clothes:loadSkin")
AddEventHandler("poly-clothes:loadSkin", function(_, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('poly-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)
RegisterNetEvent('poly-clothing:client:loadPlayerClothing')
AddEventHandler('poly-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('poly-clothing:client:loadOutfit')
AddEventHandler('poly-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
...
RegisterNetEvent("poly-clothing:client:adjustfacewear")
AddEventHandler("poly-clothing:client:adjustfacewear",function(type)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("poly-clothes:loadPlayerSkin")
    PlayerData = QBCore.Functions.GetPlayerData()
--    QBCore.Shared.Jobs = exports['poly-jobs']:AddJobs()
end)
...
RegisterNetEvent('poly-clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('poly-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('poly-clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('poly-clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('poly-clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('poly-clothing:server:removeOutfit', data.outfitName, data.outfitId)
...
if Config.UseTarget then
    CreateThread(function()
        for k, v in pairs(Config.Stores) do
            ...
            exports['poly-target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                name = v.shopType .. k,
                debugPoly = false,
                minZ = v.coords.z-1,
                maxZ = v.coords.z+1,
        ...
        for k, v in pairs(Config.ClothingRooms) do
            ...
            exports['poly-target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
        ...
        clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                if zoneName == 'surgeon' then
                    exports['poly-core']:DrawText('[E] - Plastic Surgery', 'left')
                elseif zoneName == 'clothing' then
                    exports['poly-core']:DrawText('[E] - Clothing Shop', 'left')
                elseif zoneName == 'barber' then
                    exports['poly-core']:DrawText('[E] - Barber', 'left')
                end
            else
                inZone = false

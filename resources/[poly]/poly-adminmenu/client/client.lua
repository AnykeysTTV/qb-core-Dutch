...
RegisterNetEvent('poly-admin:client:openMenu', function()
...
menu2_admin_display_names:On('change', function()
    TriggerEvent('poly-admin:client:toggleNames')
end)

-- Toggle player blip display
menu2_admin_display_blips:On('change', function()
    TriggerEvent('poly-admin:client:toggleBlips')
end)
...
for _,v in pairs(QBCore.Shared.Weapons) do
    menu15:AddButton({icon = '🎁',
        ...
        description = Lang:t("desc.spawn_weapons_desc"),
        select = function(_)
            TriggerServerEvent('poly-admin:giveWeapon', v.name)
...
local function OpenPermsMenu(permsply)
    QBCore.Functions.TriggerCallback('poly-admin:server:getrank', function(rank)
        if rank then
            ...
            menu11:AddButton({
                ...
                select = function(_)
                    if selectedgroup ~= 'Unknown' then
                        TriggerServerEvent('poly-admin:server:setPermissions', permsply.id, selectedgroup)
    ...
    menu10:AddButton({
        ...
        select = function(_)
            if kickreason ~= 'Unknown' then
                TriggerServerEvent('poly-admin:server:kick', kickplayer, kickreason)
                kickreason = 'Unknown'
            else
    ...
    menu9:AddButton({
        ...
        select = function(_)
            if banreason ~= 'Unknown' and banlength ~= nil then
                TriggerServerEvent('poly-admin:server:ban', banplayer, banlength, banreason)
                banreason = 'Unknown'
                banlength = nil
            else
        ...
        Players:AddButton({
            ...
            select = function(btn)
                ...
                if values ~= "ban" and values ~= "kick" and values ~= "perms" then
                    TriggerServerEvent('poly-admin:server:'..values, player)
...
menu3_server_weather:On("select",function()
    ...
    for _, v in ipairs(elements) do
        menu8:AddButton({icon = v.icon,label = v.label,value = v,description = v.description,select = function(btn)
            ...
            TriggerServerEvent('poly-weathersync:server:setWeather', selection.value)
...
menu3_server_time:On("select", function(_, value)
    TriggerServerEvent("poly-weathersync:server:setTime", value, value)
...
menu7_dev_info_mode:On('change', function(_, _, _)
    ...
    TriggerEvent('poly-admin:client:ToggleDevmode')
...
RegisterNetEvent('poly-admin:client:copyToClipboard', function(dataType)
...
RegisterNetEvent('poly-admin:client:ToggleCoords', function()

...
RegisterNetEvent('poly-admin:client:inventory', function(targetPed)
    TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", targetPed)
end)

RegisterNetEvent('poly-admin:client:spectate', function(targetPed)
...
RegisterNetEvent('poly-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('poly-admin:server:SendReport', name, src, msg)
end)
...
RegisterNetEvent('poly-admin:client:SaveCar', function()
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if QBCore.Shared.Vehicles[vehname] ~= nil and next(QBCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('poly-admin:server:SaveCar', props, QBCore.Shared.Vehicles[vehname], GetHashKey(veh), plate)
        else
...
RegisterNetEvent('poly-admin:client:SetModel', function(skin)
    local ped = PlayerPedId()
...
RegisterNetEvent('poly-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == "fast" then
...
RegisterNetEvent('poly-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    local ped = PlayerPedId()
...
RegisterNetEvent('poly-admin:client:GiveNuiFocus', function(focus, mouse)
...
RegisterNetEvent('poly-admin:client:maxmodVehicle', function()

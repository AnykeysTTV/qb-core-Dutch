...
local function CheckStationHits()
    ...
    if table.type(bankHits) == 'array' then TriggerClientEvent("poly-bankrobbery:client:BankSecurity", -1, bankHits, false) end
end
...
RegisterNetEvent('poly-bankrobbery:server:setBankState', function(bankId)
    ...
    if bankId == "paleto" then
        if Config.BigBanks["paleto"]["isOpened"] or #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks["paleto"]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "poly-bankrobbery:server:setBankState", extraInfo = " (paleto) ", source = source}))
        end
        Config.BigBanks["paleto"]["isOpened"] = true
        TriggerEvent('poly-bankrobbery:server:setTimeout')
    elseif bankId == "pacific" then
        if Config.BigBanks["pacific"]["isOpened"] or #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks["pacific"]["coords"][2]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "poly-bankrobbery:server:setBankState", extraInfo = " (pacific) ", source = source}))
        end
        ...
        if Config.SmallBanks[bankId]["isOpened"] or #(GetEntityCoords(GetPlayerPed(source)) - Config.SmallBanks[bankId]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "poly-bankrobbery:server:setBankState", extraInfo = " (smallbank "..bankId..") ", source = source}))
        end
        Config.SmallBanks[bankId]["isOpened"] = true
        TriggerEvent('poly-bankrobbery:server:SetSmallBankTimeout', bankId)
    end
    TriggerClientEvent('poly-bankrobbery:client:setBankState', -1, bankId)
    robberyBusy = true
    Config.OnRobberyStart(bankId)
end)

RegisterNetEvent('poly-bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" or bankId == "pacific" then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks[bankId]["lockers"][lockerId]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "poly-bankrobbery:server:setLockerState", extraInfo = " ("..bankId..") ", source = source}))
        end
        ...
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.SmallBanks[bankId]["lockers"][lockerId]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "poly-bankrobbery:server:setLockerState", extraInfo = " (smallbank "..bankId..") ", source = source}))
        end
        Config.SmallBanks[bankId]["lockers"][lockerId][state] = bool
    end
    TriggerClientEvent('poly-bankrobbery:client:setLockerState', -1, bankId, lockerId, state, bool)
end)

RegisterNetEvent('poly-bankrobbery:server:recieveItem', function(type, bankId, lockerId)
    local src = source
    local ply = QBCore.Functions.GetPlayer(src)
    if not ply then return end
    if type == "small" then
        if #(GetEntityCoords(GetPlayerPed(src)) - Config.SmallBanks[bankId]["lockers"][lockerId]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "poly-bankrobbery:server:receiveItem", extraInfo = " (smallbank "..bankId..") ", source = source}))
        end
    ...
    elseif type == "paleto" then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks["paleto"]["lockers"][lockerId]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "poly-bankrobbery:server:receiveItem", extraInfo = " (paleto) ", source = source}))
        end
        local itemType = math.random(#Config.RewardTypes)
        local tierChance = math.random(1, 100)
    ...
    elseif type == "pacific" then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks["pacific"]["lockers"][lockerId]["coords"]) > 2.5 then
            return error(Lang:t("error.event_trigger_wrong", {event = "poly-bankrobbery:server:receiveItem", extraInfo = " (pacific) ", source = source}))
        end
        local itemType = math.random(#Config.RewardTypes)
...
AddEventHandler('poly-bankrobbery:server:setTimeout', function()
    ...
    CreateThread(function()
        SetTimeout(60000 * 90, function()
            ...
            TriggerClientEvent('poly-bankrobbery:client:ClearTimeoutDoors', -1)
...
AddEventHandler('poly-bankrobbery:server:SetSmallBankTimeout', function(bankId)
    if robberyBusy or timeOut then return end
    timeOut = true
    CreateThread(function()
        SetTimeout(60000 * 30, function()
            ...
            TriggerClientEvent('poly-bankrobbery:client:ResetFleecaLockers', -1, bankId)
            timeOut = false
            robberyBusy = false
...
RegisterNetEvent('poly-bankrobbery:server:callCops', function(type, bank, coords)
    if type == "small" then
        if not Config.SmallBanks[bank]["alarm"] then
            return error(Lang:t("error.event_trigger_wrong", {event = "poly-bankrobbery:server:callCops", extraInfo = " (smallbank "..bank..") ", source = source}))
        end
    elseif type == "paleto" then
        if not Config.BigBanks["paleto"]["alarm"] then
            return error(Lang:t("error.event_trigger_wrong", {event = "poly-bankrobbery:server:callCops", extraInfo = " (paleto) ", source = source}))
        end
    elseif type == "pacific" then
        if not Config.BigBanks["pacific"]["alarm"] then
            return error(Lang:t("error.event_trigger_wrong", {event = "poly-bankrobbery:server:callCops", extraInfo = " (pacific) ", source = source}))
        end
    end
    TriggerClientEvent("poly-bankrobbery:client:robberyCall", -1, type, coords)
end)

RegisterNetEvent('poly-bankrobbery:server:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
    TriggerClientEvent("poly-bankrobbery:client:SetStationStatus", -1, key, isHit)
    if AllStationsHit() then
        exports["poly-weathersync"]:setBlackout(true)
        TriggerClientEvent("poly-bankrobbery:client:disableAllBankSecurity", -1)
        Config.OnBlackout(true)
        CreateThread(function()
            SetTimeout(60000 * Config.BlackoutTimer, function()
                exports["poly-weathersync"]:setBlackout(false)
                TriggerClientEvent("poly-bankrobbery:client:enableAllBankSecurity", -1)
...
RegisterNetEvent('poly-bankrobbery:server:removeElectronicKit', function()
    local src = source
...
RegisterNetEvent('poly-bankrobbery:server:removeBankCard', function(number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
QBCore.Functions.CreateCallback('poly-bankrobbery:server:isRobberyActive', function(_, cb)
    cb(robberyBusy)
end)

QBCore.Functions.CreateCallback('poly-bankrobbery:server:GetConfig', function(_, cb)
...
QBCore.Functions.CreateUseableItem("security_card_01", function(source)
    ...
    TriggerClientEvent("poly-bankrobbery:UseBankcardA", source)
end)
...
QBCore.Functions.CreateUseableItem("security_card_02", function(source)
    ...
    TriggerClientEvent("poly-bankrobbery:UseBankcardB", source)
end)

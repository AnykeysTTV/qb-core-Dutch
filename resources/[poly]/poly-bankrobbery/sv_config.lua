...
function Config.OnRobberyStart(bankId)
    ...
    TriggerEvent('poly-scoreboard:server:SetActivityBusy', bankName, true)
    if bankName ~= "bankrobbery" then return end
    TriggerEvent('poly-banking:server:SetBankClosed', bankId, true)
end
...
function Config.OnRobberyTimeoutEnd(bankId)
    local bankName = type(bankId) == "number" and "bankrobbery" or bankId
    TriggerEvent('poly-scoreboard:server:SetActivityBusy', bankName, false)
    if bankName ~= "bankrobbery" then return end
    TriggerEvent('poly-banking:server:SetBankClosed', bankId, false)
end

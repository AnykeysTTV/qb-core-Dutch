...
RegisterNetEvent('poly-bankrobbery:UseBankcardA', function()
            ...
            if CurrentCops >= Config.MinimumPaletoPolice then
                if not Config.BigBanks["paleto"]["isOpened"] then
                    ...
                    QBCore.Functions.Progressbar("security_pass", Lang:t("general.validating_bankcard"), math.random(5000, 10000), false, true, {
                        ...
                        TriggerServerEvent('poly-bankrobbery:server:setBankState', 'paleto')
                        TriggerServerEvent('poly-bankrobbery:server:removeBankCard', '01')
                        Config.DoorlockAction(4, false)
                        if copsCalled or not Config.BigBanks["paleto"]["alarm"] then return end
                        TriggerServerEvent("poly-bankrobbery:server:callCops", "paleto", 0, pos)
                        copsCalled = true
                    end, function() -- Cancel
                        StopAnimTask(ped, "anim@gangops@facility@servers@", "hotwire", 1.0)
...
CreateThread(function()
    ...
    for k in pairs(Config.BigBanks["paleto"]["lockers"]) do
        if Config.UseTarget then
            exports['poly-target']:AddBoxZone('paleto_coords_locker_'..k, Config.BigBanks["paleto"]["lockers"][k]["coords"], 1.0, 1.0, {

...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('poly-bankrobbery:server:GetConfig', function(config1, config2, config3)
...
function openLocker(bankId, lockerId) -- Globally Used
    ...
    TriggerServerEvent('poly-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', true)
    if bankId == "paleto" then
        ...
        if hasItem then
            ...
            QBCore.Functions.Progressbar("open_locker_drill", Lang:t("general.breaking_open_safe"), math.random(18000, 30000), false, true, {
                ...
                TriggerServerEvent('poly-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                TriggerServerEvent('poly-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                TriggerServerEvent('poly-bankrobbery:server:recieveItem', 'paleto', bankId, lockerId)
    ...
    elseif bankId == "pacific" then
        ...
        if hasItem then
            ...
            QBCore.Functions.Progressbar("open_locker_drill", Lang:t("general.breaking_open_safe"), math.random(18000, 30000), false, true, {
                ...
                TriggerServerEvent('poly-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                TriggerServerEvent('poly-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                TriggerServerEvent('poly-bankrobbery:server:recieveItem', 'pacific', bankId, lockerId)
                QBCore.Functions.Notify(Lang:t("success.success_message"), "success")
                SetTimeout(500, function()
                    IsDrilling = false
                end)
        ...
        QBCore.Functions.Progressbar("open_locker", Lang:t("general.breaking_open_safe"), math.random(27000, 37000), false, true, {
            ...
            TriggerServerEvent('poly-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
            TriggerServerEvent('poly-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            TriggerServerEvent('poly-bankrobbery:server:recieveItem', 'small', bankId, lockerId)
            QBCore.Functions.Notify(Lang:t("success.success_message"), "success")
            SetTimeout(500, function()
                IsDrilling = false
            end)
...
RegisterNetEvent('electronickit:UseElectronickit', function()
    ...
    QBCore.Functions.TriggerCallback('poly-bankrobbery:server:isRobberyActive', function(isBusy)
        if not isBusy then
                ...
                if not Config.SmallBanks[closestBank]["isOpened"] then
                    ...
                    if hasItem then
                        ...
                        QBCore.Functions.Progressbar("hack_gate", Lang:t("general.connecting_hacking_device"), math.random(5000, 10000), false, true, {
                            ...
                            StopAnimTask(ped, "anim@gangops@facility@servers@", "hotwire", 1.0)
                            TriggerServerEvent('poly-bankrobbery:server:removeElectronicKit')
                            ...
                            TriggerServerEvent("poly-bankrobbery:server:callCops", "small", closestBank, pos)
                            copsCalled = true
...
RegisterNetEvent('poly-bankrobbery:client:setBankState', function(bankId)
    if bankId == "paleto" then
...
RegisterNetEvent('poly-bankrobbery:client:enableAllBankSecurity', function()
    for k in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('poly-bankrobbery:client:disableAllBankSecurity', function()
    for k in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('poly-bankrobbery:client:BankSecurity', function(key, status)
    ...
    elseif type(key) == 'number' then
        ...
        error(Lang:t("error.wrong_type", {receiver = 'poly-bankrobbery:client:BankSecurity', argument = "key", receivedType = type(key), receivedValue = key, expected = "table/array"}))
    end
end)

RegisterNetEvent('poly-bankrobbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
...
RegisterNetEvent('poly-bankrobbery:client:ResetFleecaLockers', function(BankId)
...
RegisterNetEvent('poly-bankrobbery:client:robberyCall', function(type, coords)
    ...
    for i = 1, #Config.SmallBanks do
        ...
        for k in pairs(Config.SmallBanks[i]["lockers"]) do
            if Config.UseTarget then
                exports['poly-target']:AddBoxZone('fleeca_'..i..'_coords_locker_'..k, Config.SmallBanks[i]["lockers"][k]["coords"], 1.0, 1.0, {
                ...
                lockerZone:onPlayerInOut(function(inside)
                    if inside and closestBank ~= 0 and not IsDrilling and Config.SmallBanks[i]["isOpened"] and not Config.SmallBanks[i]["lockers"][k]["isOpened"] and not Config.SmallBanks[i]["lockers"][k]["isBusy"] then
                        exports['poly-core']:DrawText(Lang:t("general.break_safe_open_option_drawtext"), 'right')
                        currentLocker = k
                    else

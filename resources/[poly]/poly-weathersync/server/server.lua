...
local function nextWeatherStage()
    ...
    TriggerEvent("poly-weathersync:server:RequestStateSync")
end
...
-- EVENTS
RegisterNetEvent('poly-weathersync:server:RequestStateSync', function()
    TriggerClientEvent('poly-weathersync:client:SyncWeather', -1, CurrentWeather, blackout)
    TriggerClientEvent('poly-weathersync:client:SyncTime', -1, baseTime, timeOffset, freezeTime)
end)

RegisterNetEvent('poly-weathersync:server:setWeather', function(weather)
...
RegisterNetEvent('poly-weathersync:server:setTime', function(hour, minute)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('poly-weathersync:server:toggleBlackout', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('poly-weathersync:server:toggleFreezeTime', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then
...
RegisterNetEvent('poly-weathersync:server:toggleDynamicWeather', function(state)
    local src = getSource(source)
    if isAllowedToChange(src) then

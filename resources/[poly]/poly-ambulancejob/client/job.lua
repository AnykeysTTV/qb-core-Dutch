...
function Status()
    if isStatusChecking then
        ...
        exports['poly-menu']:openMenu(statusMenu)
    end
end
    ...
    CreateThread(function()
        ...
        while check do
            if IsControlJustPressed(0, 38) then
                exports['poly-core']:KeyPressed(38)
                if variable == "sign" then
                    TriggerEvent('EMSToggle:Duty')
                elseif variable == "stash" then
                    TriggerEvent('poly-ambulancejob:stash')
                elseif variable == "armory" then
                    TriggerEvent('poly-ambulancejob:armory')
                elseif variable == "storeheli" then
                    TriggerEvent('poly-ambulancejob:storeheli')
                elseif variable == "takeheli" then
                    TriggerEvent('poly-ambulancejob:pullheli')
                elseif variable == "roof" then
                    TriggerEvent('poly-ambulancejob:elevator_main')
                elseif variable == "main" then
                    TriggerEvent('poly-ambulancejob:elevator_roof')
                end
            end
            Wait(1)
        end
...
RegisterNetEvent('poly-ambulancejob:stash', function()
    if onDuty then
...
RegisterNetEvent('poly-ambulancejob:armory', function()
    if onDuty then
...
RegisterNetEvent('poly-ambulancejob:elevator_roof', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('poly-ambulancejob:elevator_main', function()
    local ped = PlayerPedId()
...
CreateThread(function()
    for k, v in pairs(Config.Locations["vehicle"]) do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" and onDuty then
                exports['poly-core']:DrawText(Lang:t('text.veh_button'), 'left')
                EMSVehicle(k)
            else
                CheckVehicle = false
    ...
    for k, v in pairs(Config.Locations["helicopter"]) do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" and onDuty then
                exports['poly-core']:DrawText(Lang:t('text.heli_button'), 'left')
                EMSHelicopter(k)
            else
                CheckHeli = false
                exports['poly-core']:HideText()
...
if Config.UseTarget then
    CreateThread(function()
        for k, v in pairs(Config.Locations["duty"]) do
            exports['poly-target']:AddBoxZone("duty" .. k, vector3(v.x, v.y, v.z), 1.5, 1, {
                name = "duty" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['poly-target']:AddBoxZone("stash" .. k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "stash" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
        ...
        for k, v in pairs(Config.Locations["armory"]) do
            exports['poly-target']:AddBoxZone("armory" .. k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "armory" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        type = "client",
                        event = "poly-ambulancejob:armory",
                        icon = "fa fa-hand",
                        label = "Open Armory",
                        job = "ambulance"
                    }
        ...
        for k, v in pairs(Config.Locations["roof"]) do
            exports['poly-target']:AddBoxZone("roof" .. k, vector3(v.x, v.y, v.z), 2, 2, {
                name = "roof" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        type = "client",
                        event = "poly-ambulancejob:elevator_roof",
        ...
        for k, v in pairs(Config.Locations["main"]) do
            exports['poly-target']:AddBoxZone("main" .. k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = "main" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        type = "client",
                        event = "poly-ambulancejob:elevator_main",
                        icon = "fas fa-hand-point-up",
                        label = "Take Elevator",
                        job = "ambulance"
                    },
    ...
    CreateThread(function()
        ...
        signCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if not onDuty then
                    exports['poly-core']:DrawText(Lang:t('text.onduty_button'), 'left')
                    EMSControls("sign")
                else
                    exports['poly-core']:DrawText(Lang:t('text.offduty_button'), 'left')
                    EMSControls("sign")
                end
            else
                check = false
        ...
        stashCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['poly-core']:DrawText(Lang:t('text.pstash_button'), 'left')
                    EMSControls("stash")
                end
            else
                check = false
        ...
        armoryCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['poly-core']:DrawText(Lang:t('text.armory_button'), 'left')
                    EMSControls("armory")
                end
            else
                check = false
        ...
        roofCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['poly-core']:DrawText(Lang:t('text.elevator_main'), 'left')
                    EMSControls("main")
                else
                    exports['poly-core']:DrawText(Lang:t('error.not_ems'), 'left')
                end
            else
                check = false
                exports['poly-core']:HideText()
        ...
        mainCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    exports['poly-core']:DrawText(Lang:t('text.elevator_roof'), 'left')
                    EMSControls("roof")
                else
                    exports['poly-core']:DrawText(Lang:t('error.not_ems'), 'left')
                end

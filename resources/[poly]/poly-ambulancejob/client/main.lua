...
RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    ...
    if exports["poly-policejob"]:IsHandcuffed() then
...
CreateThread(function()
    while true do
        ...
        if isInHospitalBed and canLeaveBed then
            sleep = 0
            exports['poly-core']:DrawText(Lang:t('text.bed_out'))
        ...
        while listen do
            if IsControlJustPressed(0, 38) then
                ...
                if variable == "checkin" then
                   TriggerEvent('poly-ambulancejob:checkin')
                    listen = false
                elseif variable == "beds" then
                    TriggerEvent('poly-ambulancejob:beds')
                    listen = false
                end
            end
            Wait(1)
...
RegisterNetEvent('poly-ambulancejob:checkin', function()
...
RegisterNetEvent('poly-ambulancejob:beds', function()
...
if Config.UseTarget then
    CreateThread(function()
        for k, v in pairs(Config.Locations["checking"]) do
            exports['poly-target']:AddBoxZone("checking"..k, vector3(v.x, v.y, v.z), 3.5, 2, {
                name = "checking"..k,
                heading = -72,
                debugPoly = false,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        ...
                        event = "poly-ambulancejob:checkin",
                        label = "Check In",
                    }
                },
                distance = 1.5
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            exports['poly-target']:AddBoxZone("beds"..k,  v.coords, 2.5, 2.3, {
                name = "beds"..k,
                heading = -20,
                debugPoly = false,
                minZ = v.coords.z - 1,
    ...
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["checking"]) do
            ...
            checkingCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    if doctorCount >= Config.MinimalDoctors then
                        exports['poly-core']:DrawText(Lang:t('text.call_doc'),'left')
                        CheckInControls("checkin")
                    else
                        exports['poly-core']:DrawText(Lang:t('text.check_in'), 'left')
                        CheckInControls("checkin")
                    end
                else
                    listen = false
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            ...
            bedCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    exports['poly-core']:DrawText(Lang:t('text.lie_bed'), 'left')

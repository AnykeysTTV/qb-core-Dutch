...
local function DeleteTarget(id)
    if Config.UseTarget then
        exports['poly-target']:RemoveZone(id)
    else
...
local function RegisterDutyTarget()
    ...
    if Config.UseTarget then
        exports['poly-target']:AddBoxZone(dutyTargetBoxID, coords, 1.5, 1.5, {
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['poly-core']:DrawText("[E] " .. label, 'left')
            else
...
local function RegisterStashTarget()
    ...
    if Config.UseTarget then
        exports['poly-target']:AddBoxZone(stashTargetBoxID, coords, 1.5, 1.5, {
            ...
            options = {{
                type = "client",
                event = "poly-mechanicjob:client:target:OpenStash",
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['poly-core']:DrawText(Lang:t('labels.o_stash'), 'left')
            else
                exports['poly-core']:HideText()
            end
...
local function RegisterGarageZone()
    ...
    vehicleZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            ...
            if inVehicle then
                exports['poly-core']:DrawText(Lang:t('labels.h_vehicle'), 'left')
            else
                exports['poly-core']:DrawText(Lang:t('labels.g_vehicle'), 'left')
            end
        else
            exports['poly-core']:HideText()
        end
    ...
    plateZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            if plate.AttachedVehicle then
                exports['poly-core']:DrawText(Lang:t('labels.o_menu'), 'left')
            else
                if IsPedInAnyVehicle(PlayerPedId()) then
                    exports['poly-core']:DrawText(Lang:t('labels.work_v'), 'left')
                end
            end
        else
            exports['poly-core']:HideText()
    ...
    local openMenu = {
        {
            ...
            params = {
                event = "poly-mechanicjob:client:UnattachVehicle",
            }
        }, {
            header = Lang:t('lift_menu.header_stats'),
            txt = Lang:t('lift_menu.desc_stats'),
            params = {
                event = "poly-mechanicjob:client:CheckStatus",
                args = {
                    number = 1,
                }
            }
            ...
            params = {
                event = "poly-mechanicjob:client:PartsMenu",
                args = {
                    number = 1,
                }
            }
            ...
            txt = "",
            params = {
                event = "poly-mechanicjob:client:target:CloseMenu",
            }
        },
    }
    exports['poly-menu']:openMenu(openMenu)
end
        ...
        for k,v in pairs(Config.ValuesLabels) do
            if math.ceil(VehicleStatus[plate][k]) ~= Config.MaxStatusValues[k] then
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    params = {
                        event = "poly-mechanicjob:client:PartMenu",
                        args = {
                            name = v,
                            parts = k
                        }
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    txt = Lang:t('parts_menu.status') .. percentage .. ".0% / 100.0%",
                    params = {
                        event = "poly-mechanicjob:client:NoDamage",
                    }
                }
            end
        end
...
local function PartMenu(data)
    ...
    local TestMenu1 = {
        ...
        },
        {
            ...
            params = {
                event = "poly-mechanicjob:client:RepairPart",
                args = {
                    part = part,
                }
            }
    ...
    exports['poly-menu']:openMenu(TestMenu1)
end
...
local function NoDamage()
    ...
    exports['poly-menu']:openMenu(noDamage)
end
...
local function UnattachVehicle()
    ...
    TriggerServerEvent('poly-vehicletuning:server:SetAttachedVehicle', false, ClosestPlate)
...
local function VehicleList()
    ...
    for k,v in pairs(Config.Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "Vehicle: "..v.."",
            params = {
                event = "poly-mechanicjob:client:SpawnListVehicle",
                args = {
                    headername = v,
                    spawnName = k
                }
...
local function RepairPart(part)
    ...
    local indx = 0
    local countitem = 0
    QBCore.Functions.TriggerCallback('poly-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            QBCore.Functions.Progressbar("repair_part", Lang:t('labels.progress_bar') ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                TriggerEvent('poly-vehicletuning:client:RepaireeePart', part)
                TriggerServerEvent('poly-inventory:server:SaveStashItems', "mechanicstash", StashItems)
...
RegisterNetEvent("poly-mechanicjob:client:UnattachVehicle",function()
    UnattachVehicle()
end)

RegisterNetEvent("poly-mechanicjob:client:PartsMenu",function()
    PartsMenu()
end)

RegisterNetEvent("poly-mechanicjob:client:PartMenu",function(data)
    PartMenu(data)
end)

RegisterNetEvent("poly-mechanicjob:client:NoDamage",function()
    NoDamage()
end)

RegisterNetEvent("poly-mechanicjob:client:CheckStatus",function()
    CheckStatus()
end)

RegisterNetEvent("poly-mechanicjob:client:SpawnListVehicle",function(data)
    local vehicleSpawnName = data.spawnName
    SpawnListVehicle(vehicleSpawnName)
end)

RegisterNetEvent("poly-mechanicjob:client:RepairPart",function(data)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    ...
    QBCore.Functions.TriggerCallback('poly-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    QBCore.Functions.TriggerCallback('poly-vehicletuning:server:GetDrivingDistances', function(retval)
...
RegisterNetEvent('poly-vehicletuning:client:SetAttachedVehicle', function(veh, key)
    if veh ~= false then
...
RegisterNetEvent('poly-vehicletuning:client:RepaireeePart', function(part)
                    ...
                    if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                        ...
                        QBCore.Functions.Progressbar("repair_advanced", Lang:t('notifications.progress_bar'), lockpickTime, false, true, {
                            ...
                            if part == "body" then
                                ...
                                TriggerServerEvent("poly-mechanicjob:server:removePart", part, needAmount)
                            ...
                            elseif part ~= "engine" then
                                ...
                                TriggerServerEvent("poly-mechanicjob:server:removePart", part, level)
                                TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items[Config.RepairCost[part]], "remove")
                            end
                        end, function() -- Cancel
                            openingDoor = false
...
RegisterNetEvent('poly-mechanicjob:client:target:OpenStash', function ()
...
RegisterNetEvent('poly-mechanicjob:client:target:CloseMenu', function()
    DestroyVehiclePlateZone(ClosestPlate)
    RegisterVehiclePlateZone(ClosestPlate, Config.Plates[ClosestPlate])

    TriggerEvent('poly-menu:client:closeMenu')
end)
        ...
        if PlayerJob.type == 'mechanic' then
            ...
            if onDuty then
                if isInsideStashZone then
                    wait = 0
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent("poly-mechanicjob:client:target:OpenStash")
                    end
                end
                ...
                if isInsideVehiclePlateZone then
                    ...
                    if attachedVehicle then
                        ...
                        if IsControlJustPressed(0, 38) and IsPedInAnyVehicle(PlayerPedId()) then
                            ...
                            Wait(500)
                            DoScreenFadeIn(150)
                            TriggerServerEvent('poly-vehicletuning:server:SetAttachedVehicle', veh, ClosestPlate)

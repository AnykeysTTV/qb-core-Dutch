...
QBCore.Functions.CreateCallback('poly-vehicletuning:server:GetDrivingDistances', function(_, cb)
    cb(VehicleDrivingDistance)
end)

QBCore.Functions.CreateCallback('poly-vehicletuning:server:IsVehicleOwned', function(_, cb, plate)
    local retval = false
    local result = MySQL.scalar.await('SELECT 1 from player_vehicles WHERE plate = ?', {plate})
    if result then
        retval = true
...
QBCore.Functions.CreateCallback('poly-vehicletuning:server:GetAttachedVehicle', function(_, cb)
    cb(Config.Plates)
end)

QBCore.Functions.CreateCallback('poly-vehicletuning:server:IsMechanicAvailable', function(_, cb)
    local amount = 0
...
QBCore.Functions.CreateCallback('poly-vehicletuning:server:GetStatus', function(_, cb, plate)
...
RegisterNetEvent('poly-vehicletuning:server:SaveVehicleProps', function(vehicleProps)
...
RegisterNetEvent('poly-vehicletuning:server:UpdateDrivingDistance', function(amount, plate)
    VehicleDrivingDistance[plate] = amount
    TriggerClientEvent('poly-vehicletuning:client:UpdateDrivingDistance', -1, VehicleDrivingDistance[plate], plate)
...
RegisterNetEvent('poly-vehicletuning:server:LoadStatus', function(veh, plate)
...
RegisterNetEvent('poly-vehicletuning:server:SetPartLevel', function(plate, part, level)
    if VehicleStatus[plate] ~= nil then
        VehicleStatus[plate][part] = level
        TriggerClientEvent("vehiclemod:client:setVehicleStatus", -1, plate, VehicleStatus[plate])
    end
...
RegisterNetEvent('poly-vehicletuning:server:SetAttachedVehicle', function(veh, k)
    if veh ~= false then
        Config.Plates[k].AttachedVehicle = veh
        TriggerClientEvent('poly-vehicletuning:client:SetAttachedVehicle', -1, veh, k)
    else
        Config.Plates[k].AttachedVehicle = nil
        TriggerClientEvent('poly-vehicletuning:client:SetAttachedVehicle', -1, false, k)
    end
end)

RegisterNetEvent('poly-vehicletuning:server:CheckForItems', function(part)
    local src = source
    ...
    if RepairPart ~= nil then
        if RepairPart.amount >= Config.RepairCostAmount[part].costs then
            TriggerClientEvent('poly-vehicletuning:client:RepaireeePart', src, part)
...
RegisterNetEvent('poly-mechanicjob:server:removePart', function(part, amount)

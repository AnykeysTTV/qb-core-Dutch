...
RegisterNetEvent('poly-streetraces:NewRace', function(RaceTable)
    local src = source
    ...
    if xPlayer.Functions.RemoveMoney('cash', RaceTable.amount, "streetrace-created") then
        ...
        TriggerClientEvent('poly-streetraces:SetRace', -1, Races)
        TriggerClientEvent('poly-streetraces:SetRaceId', src, RaceId)
        TriggerClientEvent('QBCore:Notify', src, "You joind the race for €"..Races[RaceId].amount..",-", 'success')
    end
end)

RegisterNetEvent('poly-streetraces:RaceWon', function(RaceId)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('poly-streetraces:SetRace', -1, Races)
    TriggerClientEvent('poly-streetraces:RaceDone', -1, RaceId, GetPlayerName(src))
end)

RegisterNetEvent('poly-streetraces:JoinRace', function(RaceId)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
...
QBCore.Commands.Add("createrace", "Start A Street Race", {{name="amount", help="The Stake Amount For The Race."}}, false, function(source, args)
    ...
    if GetJoinedRace(QBCore.Functions.GetIdentifier(src, 'license')) == 0 then
        TriggerClientEvent('poly-streetraces:CreateRace', src, amount)
    else
...
QBCore.Commands.Add("startrace", "Start The Race", {}, false, function(source)
    ...
    if RaceId ~= 0 then
        ...
        TriggerClientEvent('poly-streetraces:SetRace', -1, Races)
        TriggerClientEvent("poly-streetraces:StartRace", -1, RaceId)
    else
                ...
                if not Races[key].started then
                    for _, iden in pairs(Races[key].joined) do
                        local xdPlayer = QBCore.Functions.GetPlayer(iden)
                            ...
                            TriggerClientEvent('poly-streetraces:StopRace', xdPlayer.PlayerData.source)
                            RemoveFromRace(iden)
                    end
                else

                    ...
                    if Races[k] ~= nil then
                        if #(pos - vector3(Races[k].startx, Races[k].starty, Races[k].startz)) < 15.0 and not Races[k].started then
                            ...
                            if IsControlJustReleased(0, 74) then
                                TriggerServerEvent("poly-streetraces:JoinRace", k)
                            end
                        end
                    end
            ...
            if RaceId ~= 0 and InRace then
                if #(pos - vector3(Races[RaceId].endx, Races[RaceId].endy, pos.z)) < 250.0 and Races[RaceId].started then
                    ...
                    if #(pos - vector3(Races[RaceId].endx, Races[RaceId].endy, pos.z)) < 15.0 then
                        TriggerServerEvent("poly-streetraces:RaceWon", RaceId)
                        InRace = false
                    end
                end
            end
...
RegisterNetEvent('poly-streetraces:StartRace', function(race)
...
RegisterNetEvent('poly-streetraces:RaceDone', function(race, winner)
    if RaceId ~= 0 and RaceId == race then
        RaceId = 0
        InRace = false
...
RegisterNetEvent('poly-streetraces:StopRace', function()
    RaceId = 0
    InRace = false
end)

RegisterNetEvent('poly-streetraces:CreateRace', function(amount)
    local pos = GetEntityCoords(PlayerPedId(), true)
    ...
    if DoesBlipExist(WaypointHandle) then
        ...
        if #(pos - c) > 500.0 then
            ...
            TriggerServerEvent("poly-streetraces:NewRace", race)
...
RegisterNetEvent('poly-streetraces:SetRace', function(RaceTable)
    Races = RaceTable
end)

RegisterNetEvent('poly-streetraces:SetRaceId', function(race)
    RaceId = race
    SetNewWaypoint(Races[RaceId].endx, Races[RaceId].endy)
end)

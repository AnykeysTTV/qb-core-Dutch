...
RegisterNUICallback('save', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            ...
            TriggerServerEvent('poly-tunerchip:server:TuneStatus', QBCore.Functions.GetPlate(veh), true)
        end
        cb('ok')
    end, data)
end)

RegisterNetEvent('poly-tunerchip:client:TuneStatus', function()
    local ped = PlayerPedId()
    ...
    if vehModel ~= 0 then
        QBCore.Functions.TriggerCallback('poly-tunerchip:server:GetStatus', function(status)
            if status then
...
RegisterNetEvent('poly-tunerchip:client:openChip', function()
    local ped = PlayerPedId()

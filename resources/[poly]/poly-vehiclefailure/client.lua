...
local function DamageRandomComponent()
    ...
    exports['poly-mechanicjob']:SetVehicleStatus(QBCore.Functions.GetPlate(vehicle), randomComponent, exports['poly-mechanicjob']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), randomComponent) - randomDamage)
end
...
local function CleanVehicle(veh)
    ...
    QBCore.Functions.Progressbar("cleaning_vehicle", Lang:t("progress.clean_veh"), math.random(10000, 20000), false, true, {
        ...
        TriggerServerEvent('poly-vehiclefailure:server:removewashingkit', veh)
...
local function RepairVehicleFull(veh)
    ...
    QBCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.repair_veh"), math.random(20000, 30000), false, true, {
        ...
        TriggerServerEvent('poly-vehiclefailure:removeItem', "advancedrepairkit")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
        QBCore.Functions.Notify(Lang:t("error.failed_notification"), "error")
        if (IsBackEngine(GetEntityModel(veh))) then
...
local function RepairVehicle(veh)
    ...
    QBCore.Functions.Progressbar("repair_vehicle", Lang:t("progress.repair_veh"), math.random(10000, 20000), false, true, {
        ...
        TriggerServerEvent('poly-vehiclefailure:removeItem', "repairkit")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
        QBCore.Functions.Notify(Lang:t("error.failed_notification"), "error")
        if (IsBackEngine(GetEntityModel(veh))) then
...
RegisterNetEvent('poly-vehiclefailure:client:RepairVehicle', function()
...
RegisterNetEvent('poly-vehiclefailure:client:SyncWash', function(veh)
    SetVehicleDirtLevel(veh, 0.1)
    SetVehicleUndriveable(veh, false)
    WashDecalsFromVehicle(veh, 1.0)
end)

RegisterNetEvent('poly-vehiclefailure:client:CleanVehicle', function()
    local veh = QBCore.Functions.GetClosestVehicle()
...
RegisterNetEvent('poly-vehiclefailure:client:RepairVehicleFull', function()
    local veh = QBCore.Functions.GetClosestVehicle()
    if veh ~= nil and veh ~= 0 then
        local ped = PlayerPedId()
        local pos = GetEntityCoords(ped)
                    ...
                    if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
                        ...
                        if speed >= 1.0 then
                            ...
                            if brake > 127 then
                                ...
                                --exports['poly-vehicletuning']:SetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "brakes", exports['poly-vehicletuning']:GetVehicleStatus(QBCore.Functions.GetPlate(vehicle), "brakes") - 0.01)
                            end

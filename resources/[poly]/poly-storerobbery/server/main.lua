...
RegisterNetEvent('poly-storerobbery:server:takeMoney', function(register, isDone)
    local src = source
...
RegisterNetEvent('poly-storerobbery:server:setRegisterStatus', function(register)
    Config.Registers[register].robbed = true
    Config.Registers[register].time = Config.resetTime
    TriggerClientEvent('poly-storerobbery:client:setRegisterStatus', -1, register, Config.Registers[register])
end)

RegisterNetEvent('poly-storerobbery:server:setSafeStatus', function(safe)
    Config.Safes[safe].robbed = true
    TriggerClientEvent('poly-storerobbery:client:setSafeStatus', -1, safe, true)

    SetTimeout(math.random(40, 80) * (60 * 1000), function()
        Config.Safes[safe].robbed = false
        TriggerClientEvent('poly-storerobbery:client:setSafeStatus', -1, safe, false)
    end)
end)

RegisterNetEvent('poly-storerobbery:server:SafeReward', function(safe)
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('poly-storerobbery:server:callCops', function(type, safe, streetLabel, coords)
    local cameraId
    if type == "safe" then
    ...
    TriggerClientEvent("poly-storerobbery:client:robberyCall", -1, type, safe, streetLabel, coords)
    TriggerClientEvent("poly-phone:client:addPoliceAlert", -1, alertData)
end)

RegisterNetEvent('poly-storerobbery:server:removeAdvancedLockpick', function()
...
RegisterNetEvent('poly-storerobbery:server:removeLockpick', function()
    local Player = QBCore.Functions.GetPlayer(source)
...
CreateThread(function()
    while true do
        ...
        if #toSend > 0 then
            ...
            TriggerClientEvent('poly-storerobbery:client:setRegisterStatus', -1, toSend, false)
        end
...
QBCore.Functions.CreateCallback('poly-storerobbery:server:isCombinationRight', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

QBCore.Functions.CreateCallback('poly-storerobbery:server:getPadlockCombination', function(_, cb, safe)
    cb(SafeCodes[safe])
end)

QBCore.Functions.CreateCallback('poly-storerobbery:server:getRegisterStatus', function(_, cb)
    cb(Config.Registers)
end)

QBCore.Functions.CreateCallback('poly-storerobbery:server:getSafeStatus', function(_, cb)
    cb(Config.Safes)
end)

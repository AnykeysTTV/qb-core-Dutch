                            ...
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if Config.Safes[safe].type == "keypad" then
                                        ...
                                        QBCore.Functions.TriggerCallback('poly-storerobbery:server:getPadlockCombination', function(combination)
                                    ...
                                    if not copsCalled then
                                        ...
                                        TriggerServerEvent("poly-storerobbery:server:callCops", "safe", currentSafe, streetLabel, pos)
                                        copsCalled = true
                                    end
                                else
            ...
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                if usingAdvanced then
                    ...
                    if not copsCalled then
                        ...
                        TriggerServerEvent("poly-storerobbery:server:callCops", "cashier", currentRegister, streetLabel, pos)
                        copsCalled = true
                    end
                else
                    lockpick(true)
...
function setupRegister()
    QBCore.Functions.TriggerCallback('poly-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    QBCore.Functions.TriggerCallback('poly-storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('success', function(_, cb)
    if currentRegister ~= 0 then
        lockpick(false)
        TriggerServerEvent('poly-storerobbery:server:setRegisterStatus', currentRegister)
        ...
        QBCore.Functions.Progressbar("search_register", Lang:t("text.emptying_the_register"), lockpickTime, false, true, {
            ...
            TriggerServerEvent('poly-storerobbery:server:takeMoney', currentRegister, true)
        end, function() -- Cancel
            openingDoor = false
            ClearPedTasks(PlayerPedId())
...
function LockpickDoorAnim(time)
    ...
    CreateThread(function()
        while openingDoor do
            ...
            Wait(2000)
            time = time - 2
            TriggerServerEvent('poly-storerobbery:server:takeMoney', currentRegister, false)
            if time <= 0 then
                openingDoor = false
        ...
        if won then
            if currentSafe ~= 0 then
                if not Config.Safes[currentSafe].robbed then
                    ...
                    TriggerServerEvent("poly-storerobbery:server:SafeReward", currentSafe)
                    TriggerServerEvent("poly-storerobbery:server:setSafeStatus", currentSafe)
                    currentSafe = 0
                    takeAnim()
                end
            else
...
RegisterNUICallback('fail', function(_ ,cb)
    if usingAdvanced then
        if math.random(1, 100) < 20 then
            TriggerServerEvent("poly-storerobbery:server:removeAdvancedLockpick")
            TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["advancedlockpick"], "remove")
        end
    else
        if math.random(1, 100) < 40 then
            TriggerServerEvent("poly-storerobbery:server:removeLockpick")
...
RegisterNUICallback('TryCombination', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-storerobbery:server:isCombinationRight', function(combination)
...
RegisterNetEvent('poly-storerobbery:client:setRegisterStatus', function(batch, val)
...
RegisterNetEvent('poly-storerobbery:client:setSafeStatus', function(safe, bool)
    Config.Safes[safe].robbed = bool
end)

RegisterNetEvent('poly-storerobbery:client:robberyCall', function(_, _, _, coords)

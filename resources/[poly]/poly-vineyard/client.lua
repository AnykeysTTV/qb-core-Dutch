...
local function log(debugMessage)
	print(('^6[^3poly-vineyard^6]^0 %s'):format(debugMessage))
end
...
local function pickProcess()
    QBCore.Functions.Progressbar("pick_grape", Lang:t("progress.pick_grapes"), math.random(6000,8000), false, true, {
		...
		tasking = false
        TriggerServerEvent("poly-vineyard:server:getGrapes")
		DeleteBlip()
			...
			if k==random then
				CreateThread(function()
					while grapeZones[k].isInside and k==random do
						exports['poly-core']:DrawText(Lang:t("task.start_task"),'right')
...
local function grapeJuiceProcess()
    QBCore.Functions.Progressbar("grape_juice", Lang:t("progress.process_grapes"), math.random(15000,20000), false, true, {
        ...
        TriggerServerEvent("poly-vineyard:server:receiveGrapeJuice")
        ClearPedTasks(PlayerPedId())
    end, function() -- Cancel
        ClearPedTasks(PlayerPedId())
        QBCore.Functions.Notify(Lang:t("task.cancel_task"), "error")
				...
				while Zones[2].isInside do
					if not wineStarted then
						if not loadIngredients then
							exports['poly-core']:DrawText(Lang:t("task.load_ingrediants"),'right')
							if IsControlJustPressed(0, 38) and not LocalPlayer.state.inv_busy then
								QBCore.Functions.TriggerCallback('poly-vineyard:server:loadIngredients', function(result)
							...
							if not finishedWine then
								exports['poly-core']:DrawText(Lang:t("task.wine_process"),'right')
								if IsControlJustPressed(0, 38) and not LocalPlayer.state.inv_busy then
									StartWineProcess()
								end
							else
								exports['poly-core']:DrawText(Lang:t("task.get_wine"),'right')
								if IsControlJustPressed(0, 38) and not LocalPlayer.state.inv_busy then
									TriggerServerEvent("poly-vineyard:server:receiveWine")
									finishedWine = false
						...
						exports['poly-core']:DrawText(Lang:t("task.countdown",{time=winetimer}),'right')
						Wait(999)
					end
					Wait(1)
				end
		...
		if not startVineyard and PlayerJob.name == "vineyard" then
			CreateThread(function()
				while Zones[3].isInside do
					exports['poly-core']:DrawText(Lang:t("task.make_grape_juice"),'right')
					if IsControlJustPressed(0, 38) and not LocalPlayer.state.inv_busy then
						QBCore.Functions.TriggerCallback('poly-vineyard:server:grapeJuice', function(result)

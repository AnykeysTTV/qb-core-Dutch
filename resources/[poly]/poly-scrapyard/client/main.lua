...
RegisterNetEvent("QBCore:Client:OnPlayerLoaded", function()
    TriggerServerEvent("poly-scrapyard:server:LoadVehicleList")
end)
        ...
        for k,v in pairs(Config.Locations[i]) do
            if k ~= 'main' then
                if Config.UseTarget then
                    if k == 'deliver' then
                        exports["poly-target"]:AddBoxZone("yard"..i, v.coords, v.length, v.width, {
                            name = "yard"..i,
                            heading = v.heading,
                            minZ = v.coords.z - 1,
                            maxZ = v.coords.z + 1,
                        ...
                        exports["poly-target"]:AddBoxZone("list"..i, v.coords, v.length, v.width, {
                            name = "list"..i,
                            heading = v.heading,
                            minZ = v.coords.z - 1,
                            maxZ = v.coords.z + 1,
                    ...
                    scrapCombo:onPlayerInOut(function(isPointInside)
                        if isPointInside then
                            if not isBusy then
                                if k == 'deliver' then
                                    exports['poly-core']:DrawText(Lang:t('text.disassemble_vehicle'),'left')
                                else
                                    exports['poly-core']:DrawText(Lang:t('text.email_list'),'left')
                                end
                                KeyListener(k)
                            end
                        else
...
RegisterNetEvent('poly-scapyard:client:setNewVehicles', function(vehicleList)
        ...
        if not isBusy then
            if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
                if IsVehicleValid(GetEntityModel(vehicle)) then
                    ...
                    QBCore.Functions.TriggerCallback('poly-scrapyard:checkOwnerVehicle',function(retval)
                        if retval then
                            isBusy = true
                            ...
                            QBCore.Functions.Progressbar("scrap_vehicle", Lang:t('text.demolish_vehicle'), scrapTime, false, true, {
                                ...
                                TriggerServerEvent("poly-scrapyard:server:ScrapVehicle", GetVehicleKey(GetEntityModel(vehicle)))

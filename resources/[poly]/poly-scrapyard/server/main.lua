...
RegisterNetEvent('poly-scrapyard:server:LoadVehicleList', function()
    local src = source
    TriggerClientEvent("poly-scapyard:client:setNewVehicles", src, Config.CurrentVehicles)
end)


QBCore.Functions.CreateCallback('poly-scrapyard:checkOwnerVehicle', function(_, cb, plate)
...
RegisterNetEvent('poly-scrapyard:server:ScrapVehicle', function(listKey)
    local src = source
    ...
    if Config.CurrentVehicles[listKey] ~= nil then
        ...
        TriggerClientEvent("poly-scapyard:client:setNewVehicles", -1, Config.CurrentVehicles)
    end
end)

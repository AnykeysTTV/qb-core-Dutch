...
local function MenuGarage()
    ...
    for k in pairs(Config.Vehicles) do
        towMenu[#towMenu+1] = {
            ...
            params = {
                event = "poly-tow:client:TakeOutVehicle",
                args = {
                    vehicle = k
                }
            }
    ...
    exports['poly-menu']:openMenu(towMenu)
end
...
local function CreateZone(type, number)
    ...
    elseif type == "towspots" then
        event = "poly-tow:client:SpawnNPCVehicle"
    ...
    if Config.UseTarget and type == "main" then
        ...
        zoneCombo:onPlayerInOut(function(isPointInside)
            if isPointInside then
                if type == "main" then
                    TriggerEvent('poly-tow:client:PaySlip')
                elseif type == "vehicle" then
                    TriggerEvent('poly-tow:client:Vehicle')
                elseif type == "towspots" then
                    TriggerEvent('poly-tow:client:SpawnNPCVehicle')
                end
            end
        end)
        if type == "vehicle" then
            ...
            zoneComboV:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    TriggerEvent('poly-tow:client:ShowMarker', true)
                else
                    TriggerEvent('poly-tow:client:ShowMarker', false)
                end
            end)
        elseif type == "towspots" then
...
RegisterNetEvent('poly-tow:client:SpawnVehicle', function()
...
RegisterNetEvent('poly-tow:client:TowVehicle', function()
                    ...
                    if #(towPos - targetPos) < 11.0 then
                        QBCore.Functions.Progressbar("towing_vehicle", Lang:t("mission.towing_vehicle"), 5000, false, true, {
                            ...
                            if NpcOn then
                                ...
                                TriggerServerEvent('poly-tow:server:nano', vehNetID)
                                --remove zone
...
RegisterNetEvent('poly-tow:client:TakeOutVehicle', function(data)
    local coords = Config.Locations["vehicle"].coords
    ...
    if #(pos - coords) <= 5 then
        ...
        TriggerServerEvent('poly-tow:server:DoBail', true, vehicleInfo)
...
RegisterNetEvent('poly-tow:client:Vehicle', function()
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
    if not CurrentTow then
        if vehicle and isTowVehicle(vehicle) then
            DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
            TriggerServerEvent('poly-tow:server:DoBail', false)
        else
            MenuGarage()
        end
    else
...
RegisterNetEvent('poly-tow:client:PaySlip', function()
    if JobsDone > 0 then
        RemoveBlip(CurrentBlip)
        TriggerServerEvent("poly-tow:server:11101110", JobsDone)
        JobsDone = 0
        NpcOn = false
    else
...
RegisterNetEvent('poly-tow:client:SpawnNPCVehicle', function()
...
RegisterNetEvent('poly-tow:client:ShowMarker', function(active)
    if PlayerJob.name == "tow" then
        showMarker = active
    end
end)

...
RegisterNetEvent('poly-tow:server:DoBail', function(bool, vehInfo)
    local src = source
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.BailPrice then
            ...
            TriggerClientEvent('poly-tow:client:SpawnVehicle', src, vehInfo)
...
RegisterNetEvent('poly-tow:server:nano', function(vehNetID)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-tow:server:11101110', function(drops)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
QBCore.Commands.Add("tow", Lang:t("info.tow"), {}, false, function(source)
    ...
    if Player.PlayerData.job.name == "tow"  or Player.PlayerData.job.name == "mechanic" then
        TriggerClientEvent("poly-tow:client:TowVehicle", source)
    end
end)

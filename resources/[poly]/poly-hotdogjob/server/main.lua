...
QBCore.Functions.CreateCallback('poly-hotdogjob:server:HasMoney', function(source, cb)
...
QBCore.Functions.CreateCallback('poly-hotdogjob:server:BringBack', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
...
RegisterNetEvent('poly-hotdogjob:server:Sell', function(coords, amount, price)
    local src = source
    local pCoords = GetEntityCoords(GetPlayerPed(src))
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if #(pCoords - coords) > 4 then exports['poly-core']:ExploitBan(src, 'hotdog job') end
    Player.Functions.AddMoney('cash', tonumber(amount * price), 'hotdog')
end)

RegisterNetEvent('poly-hotdogjob:server:UpdateReputation', function(quality)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if quality == "exotic" then
        if JobReputation["hotdog"] ~= nil and JobReputation["hotdog"] + 3 > Config.MaxReputation then
            ...
            TriggerClientEvent('poly-hotdogjob:client:UpdateReputation', src, JobReputation)
            return
        end
...
QBCore.Commands.Add("removestand", Lang:t("info.command"), {}, false, function(source, _)
    TriggerClientEvent('poly-hotdogjob:staff:DeletStand', source)
end, 'admin')

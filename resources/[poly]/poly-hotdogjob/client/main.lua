...
local function FinishMinigame(faults)
    ...
    if Config.Stock[Quality].Current + 1 <= Config.Stock[Quality].Max[Config.MyLevel] then
        TriggerServerEvent('poly-hotdogjob:server:UpdateReputation', Quality)
...
local function StartHotdogMinigame()
    PrepareAnim()
    TriggerEvent('poly-keyminigame:show')
    TriggerEvent('poly-keyminigame:start', FinishMinigame)
end
...
local function StartWorking()
    QBCore.Functions.TriggerCallback('poly-hotdogjob:server:HasMoney', function(HasMoney)
        if HasMoney then
            if Config.UseTarget then
                ...
                exports['poly-target']:AddTargetEntity(StandObject, {
                    options = {
                        {
                            icon = "fas fa-hand",
                            ...
                            type = 'client',
                            event = 'poly-hotdogjob:client:ToggleSell',
                            canInteract = function()
                                return IsWorking
                            end
                        }
...
local function StopWorking()
    if DoesEntityExist(StandObject) then
        QBCore.Functions.TriggerCallback('poly-hotdogjob:server:BringBack', function(DidBail)
            if DidBail then
                DeleteObject(StandObject)
                ClearPedTasksImmediately(PlayerPedId())
                IsWorking = false
            ...
            if SellingData.Hotdog ~= nil then
                ...
                if Config.UseTarget then
                    if not zoneMade then
                        zoneMade = true
                        exports['poly-target']:AddEntityZone('sellingDogPed', ped, {
                            name = 'sellingDogPed',
                            debugPoly = false,
                        }, {
                            options = {
                                {
                                    ...
                                    action = function(entity)
                                        ...
                                        TriggerServerEvent('poly-hotdogjob:server:Sell', pedCoords, HotdogsForSale, SellingPrice)
                                        SellingData.HasTarget = false
                                        LoadAnim('mp_common')
                                        ...
                                        SellingData.Hotdog = nil
                                        exports['poly-target']:RemoveZone('sellingDogPed')
                                        zoneMade = false
                                    end,
                                },
                                {
...
RegisterNetEvent('poly-hotdogjob:client:UpdateReputation', function(JobRep)
    PlayerData.metadata["jobrep"] = JobRep
    UpdateLevel()
end)

RegisterNetEvent('poly-hotdogjob:client:ToggleSell', function()
...
RegisterNetEvent('poly-hotdogjob:staff:DeletStand', function()
    local ped = PlayerPedId()
...
CreateThread(function()
    if Config.UseTarget then
        exports['poly-target']:AddBoxZone('hotdog_start', vector3(Config.Locations["take"].coords.x, Config.Locations["take"].coords.y, Config.Locations["take"].coords.z), 1, 1, {
            name = 'hotdog_start',
            debugPoly = false,
        ...
        hotdogStart:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                if PlayerData.job.name == 'hotdog' then
                    if not IsWorking then
                        exports['poly-core']:DrawText(Lang:t("info.start_working"), 'left')
                        CreateThread(function()
                            while inZone do
                                Wait(0)
                                if IsControlJustPressed(0, 38) then
                        ...
                        exports['poly-core']:DrawText(Lang:t("info.stop_working"), 'left')
                        CreateThread(function()
                            while inZone do
                                Wait(0)
                                if IsControlJustPressed(0, 38) then

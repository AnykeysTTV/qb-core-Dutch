...
function closeBanking() {
    $.post("https://poly-atms/NUIFocusOff", JSON.stringify({}));
    currentLimit = null;
    clientPin = null;
    cardSelected = null;
    clientCid = null;
...
$( function() {
        ...
        if ($('#pinContainer').is(':visible')) {
            if (key.which >= 48 && key.which <= 57) { // 48 - 57 (0-9 keys)
                ...
                if (currentVal == clientPin) {
                    ...
                    $('#errorMsg').html('Voer uw pincode van uw betaalpas in om toegang te krijgen tot de pinautomaat.');
                    $('#pinCode').val('');
                    $.post("https://poly-atms/loadBankingAccount", JSON.stringify({
                        cid: clientCid,
                        cardnumber: cardNumb
                    }));
                } else {
                    ...
                    setTimeout(function () {
                        $('#errorMsg').addClass('alert-info').removeClass('alert-danger');
                        $('#errorMsg').html('Voer uw pincode van uw betaalkaart in om toegang te krijgen tot de pinautomaat.');
                    }, 5000);
                }
            }
        }
    ...
    $(document).on('click','[data-action=useCard]',function(){
        $.post("https://poly-atms/playATMAnim", JSON.stringify({}));
    ...
    $(document).on('click','[data-action=removeCard]',function(){
        var selectedCard = $(this).data('card')
        $.post("https://poly-atms/removeCard", JSON.stringify({
    ...
    document.querySelectorAll('[data-withdrawal]').forEach(function (element) {
        element.addEventListener('click', (evt) => {
            ...
            if (amount == undefined || amount == null || amount <= 0) {
                ...
                return errorMessage.innerHTML = 'Er is een fout opgetreden bij uw opname. Probeer het alstublieft opnieuw.';
            }

            $.post("https://poly-atms/doATMWithdraw", JSON.stringify({
    ...
    $(document).on('click','[data-act=enterNumber]',function(){
            ...
            if (number == "ACCEPT") {
                ...
                if (currentVal == clientPin) {
                    $('#errorMsg').addClass('alert-info').removeClass('alert-danger');
                    $('#errorMsg').html('Voer uw pincode van uw betaalpas in om toegang te krijgen tot de geldautomaat.');
                    $('#pinCode').val('');
                    $.post("https://poly-atms/loadBankingAccount", JSON.stringify({
                        cid: clientCid,
                        cardnumber: cardNumb
                    ...
                    $('#pinCode').val('');
                    $('#errorMsg').removeClass('alert-info').addClass('alert-danger');
                    $('#errorMsg').html('U heeft een onjuiste pincode ingevoerd, probeer het alstublieft opnieuw.')
                    setTimeout(function(){ 
                        $('#errorMsg').addClass('alert-info').removeClass('alert-danger');
                        $('#errorMsg').html('Voer uw pincode van uw betaalpas in om toegang te krijgen tot de geldautomaat.');
                    }, 5000);

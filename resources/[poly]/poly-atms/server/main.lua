...
CreateThread(function()
    while true do
        Wait(3600000)
        dailyWithdraws = {}
        TriggerClientEvent('QBCore:Notify', -1, "Dagelijkse opnamelimiet opnieuw ingesteld.", "success")
    end
end)
...
RegisterCommand('atm', function(source)
    ...
    TriggerClientEvent('poly-atms:client:loadATM', src, cards)
end)
end
-- Event

RegisterNetEvent('poly-atms:server:enteratm',function ()
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local visas = xPlayer.Functions.GetItemsByName('visa')
    local masters = xPlayer.Functions.GetItemsByName('mastercard')
...
RegisterNetEvent('poly-atms:server:doAccountWithdraw', function(data)
    if data ~= nil then
        local src = source
        local xPlayer = QBCore.Functions.GetPlayer(src)
        ...
        if tonumber(data.amount) <= 0 then 
            return TriggerClientEvent('QBCore:Notify', src, "Het bedrag moet groter zijn dan 0.", "error")
        end   
        ...
        if dailyWith < Config.DailyLimit then
            ...
            if xCH ~= nil then
                ...
                if bankCount > 0 then
                    ...
                    TriggerClientEvent('QBCore:Notify', src, "Haal $" .. data.amount .. " op van de creditcard. Dagelijkse opnames:" .. dailyWithdraws[cardHolder], "success")
                else
                    TriggerClientEvent('QBCore:Notify', src, "Niet genoeg geld", "error")
                end
                ...
                if bankCount > 0  then
                    ...
                    dailyWithdraws[cardHolder] = dailyWithdraws[cardHolder] + tonumber(data.amount)
                    TriggerClientEvent('QBCore:Notify', src, "Opnamen $" .. data.amount .. ' van de creditcard. Dagelijkse opnames: ' .. dailyWithdraws[cardHolder], "success")
                else
                    TriggerClientEvent('QBCore:Notify', src, "Niet genoeg geld", "error")
                end
            ...
            TriggerClientEvent('poly-atms:client:updateBankInformation', src, banking)
        else
            TriggerClientEvent('QBCore:Notify', src, "U heeft het dagelijkse limiet bereikt.", "error")
        end
    end
end)
...
QBCore.Functions.CreateCallback('poly-debitcard:server:requestCards', function(source, cb)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local visas = xPlayer.Functions.GetItemsByName('visa')
    local masters = xPlayer.Functions.GetItemsByName('mastercard')
...
QBCore.Functions.CreateCallback('poly-debitcard:server:deleteCard', function(source, cb, data)
...
QBCore.Functions.CreateCallback('poly-atms:server:loadBankAccount', function(source, cb, cid, cardnumber)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)

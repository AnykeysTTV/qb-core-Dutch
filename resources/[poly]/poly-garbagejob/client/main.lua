...
local function DeliverAnim()
    ...
    if Config.UseTarget and hasBag then
        ...
        exports['poly-target']:RemoveTargetEntity(garbageVehicle)
        if (amountOfBags - 1) <= 0 then
            ...
            exports['poly-target']:AddCircleZone('garbagebin', vector3(CL.coords.x, CL.coords.y, CL.coords.z), 2.0,{
...
function TakeAnim()
    ...
    QBCore.Functions.Progressbar("bag_pickup", Lang:t("info.picking_bag"), math.random(3000, 5000), false, true, {
        ...
        if Config.UseTarget and not hasBag then
            hasBag = true
            exports['poly-target']:RemoveZone("garbagebin")
            exports['poly-target']:AddTargetEntity(garbageVehicle, {
            options = {
                ...
                if not hasBag and canTakeBag then
                    if Distance < 1.5 then
                        if not GarbText then
                            GarbText = true
                            exports['poly-core']:DrawText(Lang:t("info.grab_garbage"), 'left')
                        end
                    ...
                    if DoesEntityExist(garbageVehicle) then
                        ...
                        if TruckDist < 2 then
                            if not TrucText then
                                TrucText = true
                                exports['poly-core']:DrawText(Lang:t("info.dispose_garbage"), 'left')
                            end
    ...
    CreateThread(function()
        while ControlListen do
            if IsControlJustReleased(0, 38) then
                TriggerEvent("poly-garbagejob:client:MainMenu")
            end
            Wait(1)
        end
    end)
...
local function spawnPeds()
    ...
    for i = 1, #Config.Peds do
        ...
        if Config.UseTarget then
            exports['poly-target']:AddTargetEntity(ped, {
                options = {{type = "client", event = "poly-garbagejob:client:MainMenu", label = Lang:t("target.talk"), icon = 'fa-solid fa-recycle', job = "garbage",}},
                distance = 2.0
            })
        else
            ...
            if options then
                ...
                zone:onPlayerInOut(function(inside)
                    if LocalPlayer.state.isLoggedIn then
                        if inside then
                            exports['poly-core']:DrawText(Lang:t("info.talk"), 'left')
                            Listen4Control()
                        else
                            ControlListen = false
                            exports['poly-core']:HideText()
...
RegisterNetEvent('poly-garbagejob:client:RequestRoute', function()
                ...
                for _,v in pairs(Config.Locations["vehicle"].coords) do
                    if not IsAnyVehicleNearPoint(vector3(v.x,v.y,v.z), 2.5) then
                        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                            ...
                            garbageVehicle = veh
                            SetVehicleNumberPlateText(veh, "poly-" .. tostring(math.random(1000, 9999)))
                            ...
                            TriggerServerEvent("poly-garbagejob:server:payDeposit")
...
RegisterNetEvent('poly-garbagejob:client:RequestPaycheck', function()
...
RegisterNetEvent('poly-garbagejob:client:MainMenu', function()
    if playerJob.name == Config.Jobname then
        local MainMenu = {}
        MainMenu[#MainMenu+1] = {isMenuHeader = true,header = Lang:t("menu.header")}
        MainMenu[#MainMenu+1] = { header = Lang:t("menu.collect"),txt = Lang:t("menu.return_collect"),params = { event = 'poly-garbagejob:client:RequestPaycheck',}}
        if not garbageVehicle or finished then
            MainMenu[#MainMenu+1] = { header = Lang:t("menu.route"), txt = Lang:t("menu.request_route"), params = { event = 'poly-garbagejob:client:RequestRoute',}}
        end

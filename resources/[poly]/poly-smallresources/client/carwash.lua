            ...
            if Driver == PlayerPed and not washingVehicle then
                if IsControlPressed(0, 38) then
                    if dirtLevel > Config.DirtLevel then
                        TriggerServerEvent('poly-carwash:server:washCar')
                    else
...
RegisterNetEvent('poly-carwash:client:washCar', function()
    local PlayerPed = PlayerPedId()
    local PedVehicle = GetVehiclePedIsIn(PlayerPed, false)
    washingVehicle = true
...
CreateThread(function()
    for k,v in pairs(Config.CarWash) do
        if Config.UseTarget then
            exports["poly-target"]:AddBoxZone('carwash_'..k, v['poly'].coords, v['poly'].length, v['poly'].width, {
                name = 'carwash_'..k,
                debugPoly = false,
            ...
            washCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    exports['poly-core']:DrawText(Lang:t('wash.wash_vehicle'),'left')
                    if not listen then
                        listen = true
                        WashLoop()
                    end

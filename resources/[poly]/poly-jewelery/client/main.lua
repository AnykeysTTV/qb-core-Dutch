...
local function smashVitrine(k)
    ...
    QBCore.Functions.TriggerCallback('poly-jewellery:server:getCops', function(cops)
        if cops >= Config.RequiredCops then
            ...
            QBCore.Functions.Progressbar("smash_vitrine", Lang:t('info.progressbar'), Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
                ...
                TriggerServerEvent('poly-jewellery:server:vitrineReward', k)
                TriggerServerEvent('poly-jewellery:server:setTimeout')
                TriggerServerEvent('police:server:policeAlert', 'Robbery in progress')
                smashing = false
                TaskPlayAnim(ped, animDict, "exit", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            end, function() -- Cancel
                TriggerServerEvent('poly-jewellery:server:setVitrineState', "isBusy", false, k)
                smashing = false
                TaskPlayAnim(ped, animDict, "exit", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
            end)
            TriggerServerEvent('poly-jewellery:server:setVitrineState', "isBusy", true, k)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	QBCore.Functions.TriggerCallback('poly-jewellery:server:getVitrineState', function(result)
		Config.Locations = result
	end)
end)

RegisterNetEvent('poly-jewellery:client:setVitrineState', function(stateType, state, k)
            ...
            if IsControlJustPressed(0, 38) then
                ...
                if not Config.Locations[case]["isBusy"] and not Config.Locations[case]["isOpened"] then
                    exports['poly-core']:KeyPressed()
                        ...
                        exports['poly-core']:DrawText(Lang:t('general.drawtextui_broken'), 'left')
                    end
                end
            Wait(1)
        end
...
CreateThread(function()
    if Config.UseTarget then
        for k, v in pairs(Config.Locations) do
            exports["poly-target"]:AddBoxZone("jewelstore" .. k, v.coords, 1, 1, {
                name = "jewelstore" .. k,
                heading = 40,
                minZ = v.coords.z - 1,
                maxZ = v.coords.z + 1,
        ...
        for k, v in pairs(Config.Locations) do
            ...
            boxZone:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    Listen4Control(k)
                    exports['poly-core']:DrawText(Lang:t('general.drawtextui_grab'), 'left')
                else
                    listen = false

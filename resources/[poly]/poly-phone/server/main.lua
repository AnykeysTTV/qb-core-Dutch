...
local function sendNewMailToOffline(citizenid, mailData)
    ...
    if Player then
        ...
        if mailData.button == nil then
            ...
            TriggerClientEvent('poly-phone:client:NewMailNotify', src, mailData)
        else
        ...
        SetTimeout(200, function()
            ...
            TriggerClientEvent('poly-phone:client:UpdateMails', src, mails)
        end)
    else
        if mailData.button == nil then
...
QBCore.Functions.CreateCallback('poly-phone:server:GetCallState', function(_, cb, ContactData)
...
QBCore.Functions.CreateCallback('poly-phone:server:GetPhoneData', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('poly-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Invoices = {}
    ...
    exports['poly-phone']:sendNewMailToOffline(sendercitizenid, invoiceMailData)
    exports['poly-management']:AddMoney(society, amount)
...
QBCore.Functions.CreateCallback('poly-phone:server:DeclineInvoice', function(source, cb, _, _, invoiceId)
    local Invoices = {}
    local Ply = QBCore.Functions.GetPlayer(source)
    MySQL.query('DELETE FROM phone_invoices WHERE id = ?', {invoiceId})
    local invoices = MySQL.query.await('SELECT * FROM phone_invoices WHERE citizenid = ?', {Ply.PlayerData.citizenid})
...
QBCore.Functions.CreateCallback('poly-phone:server:GetContactPictures', function(_, cb, Chats)
...
QBCore.Functions.CreateCallback('poly-phone:server:GetContactPicture', function(_, cb, Chat)
...
QBCore.Functions.CreateCallback('poly-phone:server:GetPicture', function(_, cb, number)
    local query = '%' .. number .. '%'
    local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', {query})
    if result[1] ~= nil then
...
QBCore.Functions.CreateCallback('poly-phone:server:FetchResult', function(_, cb, search)
...
QBCore.Functions.CreateCallback('poly-phone:server:GetVehicleSearchResults', function(_, cb, search)
    search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
...
QBCore.Functions.CreateCallback('poly-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData
    if plate ~= nil then
...
QBCore.Functions.CreateCallback('poly-phone:server:HasPhone', function(source, cb)
...
QBCore.Functions.CreateCallback('poly-phone:server:CanTransferMoney', function(source, cb, amount, iban)
...
QBCore.Functions.CreateCallback('poly-phone:server:GetCurrentLawyers', function(_, cb)
    local Lawyers = {}
...
QBCore.Functions.CreateCallback("poly-phone:server:GetWebhook",function(_,cb)
	if WebHook ~= "" then
		cb(WebHook)
	else
...
RegisterNetEvent('poly-phone:server:AddAdvert', function(msg, url)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('poly-phone:client:UpdateAdverts', -1, Adverts, "@" .. Player.PlayerData.charinfo.firstname .. "" .. Player.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('poly-phone:server:DeleteAdvert', function()
    local Player = QBCore.Functions.GetPlayer(source)
    local citizenid = Player.PlayerData.citizenid
    Adverts[citizenid] = nil
    TriggerClientEvent('poly-phone:client:UpdateAdvertsDel', -1, Adverts)
end)

RegisterNetEvent('poly-phone:server:SetCallState', function(bool)
    local src = source
...
RegisterNetEvent('poly-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', {Player.PlayerData.citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0})
...
RegisterNetEvent('poly-phone:server:sendNewEventMail', function(citizenid, mailData)
    local Player = QBCore.Functions.GetPlayerByCitizenId(citizenid)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', {citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0})
    else
    ...
    SetTimeout(200, function()
        ...
        TriggerClientEvent('poly-phone:client:UpdateMails', Player.PlayerData.source, mails)
    end)
end)

RegisterNetEvent('poly-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for _, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                ...
                TriggerClientEvent('poly-phone:client:GetMentioned', Player.PlayerData.source, TweetMessage, AppAlerts[Player.PlayerData.citizenid]["twitter"])
            else
...
RegisterNetEvent('poly-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Target = QBCore.Functions.GetPlayerByPhone(TargetData.number)
    if Target ~= nil then
        TriggerClientEvent('poly-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

RegisterNetEvent('poly-phone:server:BillingEmail', function(data, paid)
    for _, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if target.PlayerData.job.name == data.society then
            if paid then
                ...
                TriggerClientEvent('poly-phone:client:BillingEmail', target.PlayerData.source, data, true, name)
            else
                local name = '' .. QBCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. QBCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('poly-phone:client:BillingEmail', target.PlayerData.source, data, false, name)
            end
        end
    end
end)

RegisterNetEvent('poly-phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('poly-phone:client:UpdateHashtags', -1, Handle, messageData)
end)

RegisterNetEvent('poly-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = QBCore.Functions.GetPlayer(src).citizenid
    QBPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)

RegisterNetEvent('poly-phone:server:DeleteTweet', function(tweetId)
    local Player = QBCore.Functions.GetPlayer(source)
    local delete = false
    local TID = tweetId
    ...
    if delete then
        ...
        TriggerClientEvent('poly-phone:client:UpdateTweets', -1, TWData, nil, true)
    end
end)

RegisterNetEvent('poly-phone:server:UpdateTweets', function(NewTweets, TweetData)
    local src = source
    if Config.Linux then
        ...
        TriggerClientEvent('poly-phone:client:UpdateTweets', -1, src, NewTweets, TweetData, false)
    else
        MySQL.insert('INSERT INTO phone_tweets (citizenid, firstName, lastName, message, date, url, picture, tweetid) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
            TweetData.citizenid,
            TweetData.firstName,
...
RegisterNetEvent('poly-phone:server:TransferMoney', function(iban, amount)
    local src = source
    ...
    if result[1] ~= nil then
        ...
        if reciever ~= nil then
            ...
            if PhoneItem ~= nil then
                TriggerClientEvent('poly-phone:client:TransferMoney', reciever.PlayerData.source, amount,
...
RegisterNetEvent('poly-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, _)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.update(
...
RegisterNetEvent('poly-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, _)
    local src = source
    ...
    if Player[1] ~= nil then
        ...
        if TargetData ~= nil then
            ...
            if Chat[1] ~= nil then
                ...
                TriggerClientEvent('poly-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, false)
            else
                -- Insert for target
                ...
                -- Send notification & Update messages for target
                TriggerClientEvent('poly-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, true)
            end
        else
            local Chat = MySQL.query.await('SELECT * FROM phone_messages WHERE citizenid = ? AND number = ?', {SenderData.PlayerData.citizenid, ChatNumber})
            if Chat[1] ~= nil then
...
RegisterNetEvent('poly-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
    local Hour = os.date("%H")
    local Minute = os.date("%M")
    local label = Hour .. ":" .. Minute
    TriggerClientEvent('poly-phone:client:AddRecentCall', src, data, label, type)
    local Trgt = QBCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('poly-phone:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterNetEvent('poly-phone:server:CancelCall', function(ContactData)
    local Ply = QBCore.Functions.GetPlayerByPhone(ContactData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('poly-phone:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('poly-phone:server:AnswerCall', function(CallData)
    local Ply = QBCore.Functions.GetPlayerByPhone(CallData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('poly-phone:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('poly-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('poly-phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterNetEvent('poly-phone:server:AddTransaction', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-phone:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    -- TriggerClientEvent('poly-phone:RefreshPhone', src)
end)

RegisterNetEvent('poly-phone:server:RemoveInstallation', function(App)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-phone:server:addImageToGallery', function(image)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    MySQL.insert('INSERT INTO phone_gallery (`citizenid`, `image`) VALUES (?, ?)',{Player.PlayerData.citizenid,image})
end)

RegisterNetEvent('poly-phone:server:getImageFromGallery', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local images = MySQL.query.await('SELECT * FROM phone_gallery WHERE citizenid = ? ORDER BY `date` DESC',{Player.PlayerData.citizenid})
    TriggerClientEvent('poly-phone:refreshImages', src, images)
end)

RegisterNetEvent('poly-phone:server:RemoveImageFromGallery', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-phone:server:sendPing', function(data)
    local src = source
    if src == data then
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('poly-phone:RefreshPhone', billed.PlayerData.source)

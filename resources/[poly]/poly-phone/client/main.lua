...
local function LoadPhone()
    Wait(100)
    QBCore.Functions.TriggerCallback('poly-phone:server:GetPhoneData', function(pData)
...
local function OpenPhone()
    QBCore.Functions.TriggerCallback('poly-phone:server:HasPhone', function(HasPhone)
        if HasPhone then
            PhoneData.PlayerData = QBCore.Functions.GetPlayerData()
            ...
            QBCore.Functions.TriggerCallback('poly-garage:server:GetPlayerVehicles', function(vehicles)
...
local function CancelCall()
    TriggerServerEvent('poly-phone:server:CancelCall', PhoneData.CallData)
    ...
    TriggerServerEvent('poly-phone:server:SetCallState', false)
...
local function CallContact(CallData, AnonymousCall)
    ...
    TriggerServerEvent('poly-phone:server:CallContact', PhoneData.CallData.TargetData, PhoneData.CallData.CallId, AnonymousCall)
    TriggerServerEvent('poly-phone:server:SetCallState', true)
...
local function AnswerCall()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        ...
        TriggerServerEvent('poly-phone:server:AnswerCall', PhoneData.CallData)
...
RegisterNUICallback('ClearRecentAlerts', function(_, cb)
    TriggerServerEvent('poly-phone:server:SetPhoneAlerts', "phone", 0)
...
RegisterNUICallback('SetBackground', function(data, cb)
    ...
    TriggerServerEvent('poly-phone:server:SaveMetaData', PhoneData.MetaData)
    cb('ok')
end)
...
RegisterNUICallback('RemoveMail', function(data, cb)
    ...
    TriggerServerEvent('poly-phone:server:RemoveMail', MailId)
    cb('ok')
end)
...
RegisterNUICallback('AcceptMailButton', function(data, cb)
    ...
    TriggerServerEvent('poly-phone:server:ClearButtonData', data.mailId)
    cb('ok')
end)
...
RegisterNUICallback('AddNewContact', function(data, cb)
    ...
    TriggerServerEvent('poly-phone:server:AddNewContact', data.ContactName, data.ContactNumber, data.ContactIban)
end)
...
RegisterNUICallback('GetProfilePicture', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('poly-phone:server:GetPicture', function(picture)
        cb(picture)
    end, number)
end)
...
RegisterNUICallback('PostAdvert', function(data, cb)
    TriggerServerEvent('poly-phone:server:AddAdvert', data.message, data.url)
    cb('ok')
end)

RegisterNUICallback("DeleteAdvert", function(_, cb)
    TriggerServerEvent("poly-phone:server:DeleteAdvert")
    cb('ok')
end)
...
RegisterNUICallback('ClearAlerts', function(data, cb)
    ...
    if PhoneData.Chats[ChatKey].Unread ~= nil then
        ...
        TriggerServerEvent('poly-phone:server:SetPhoneAlerts', "whatsapp", newAlerts)
...
RegisterNUICallback('PayInvoice', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('poly-phone:server:PayInvoice', function(CanPay, Invoices)
        if CanPay then PhoneData.Invoices = Invoices end
        cb(CanPay)
    end, society, amount, invoiceId, senderCitizenId)
    TriggerServerEvent('poly-phone:server:BillingEmail', data, true)
end)
...
RegisterNUICallback('DeclineInvoice', function(data, cb)
    ...
    local amount = data.amount
    local invoiceId = data.invoiceId
    QBCore.Functions.TriggerCallback('poly-phone:server:DeclineInvoice', function(_, Invoices)
        PhoneData.Invoices = Invoices
        cb('ok')
    end, society, amount, invoiceId)
    TriggerServerEvent('poly-phone:server:BillingEmail', data, false)
end)
...
RegisterNUICallback('EditContact', function(data, cb)
    ...
    Wait(100)
    cb(PhoneData.Contacts)
    TriggerServerEvent('poly-phone:server:EditContact', NewName, NewNumber, NewIban, OldName, OldNumber, OldIban)
end)
    ...
    if #Hashtag <= 3 then
        for i = 2, #Hashtag, 1 do
            ...
            if Handle ~= nil or Handle ~= "" then
                ...
                TriggerServerEvent('poly-phone:server:UpdateHashtags', Handle, TweetMessage)
            end
        end
            ...
            if Handle ~= nil or Handle ~= "" then
                ...
                if (Firstname ~= nil and Firstname ~= "") and (Lastname ~= nil and Lastname ~= "") then
                    if Firstname ~= PhoneData.PlayerData.charinfo.firstname and Lastname ~= PhoneData.PlayerData.charinfo.lastname then
                        TriggerServerEvent('poly-phone:server:MentionedPlayer', Firstname, Lastname, TweetMessage)
                    end
                end
            end
        end
        ...
        TriggerServerEvent('poly-phone:server:UpdateTweets', PhoneData.Tweets, TweetMessage)
    else
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNUICallback('DeleteTweet',function(data, cb)
    TriggerServerEvent('poly-phone:server:DeleteTweet', data.id)
    cb('ok')
end)
...
RegisterNUICallback('FetchSearchResults', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-phone:server:FetchResult', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('InstallApplication', function(data, cb)
    ...
    if NewSlot <= Config.MaxSlots then
        TriggerServerEvent('poly-phone:server:InstallApplication', {
            app = data.app,
        })
        cb({
            app = data.app,
...
RegisterNUICallback('DeleteImage', function(image, cb)
    TriggerServerEvent('poly-phone:server:RemoveImageFromGallery',image)
    Wait(400)
    TriggerServerEvent('poly-phone:server:getImageFromGallery')
    cb(true)
end)
...
RegisterNUICallback('DeleteContact', function(data, cb)
    ...
    TriggerServerEvent('poly-phone:server:RemoveContact', Name, Number)
end)

RegisterNUICallback('GetCryptoData', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-crypto:server:GetCryptoData', function(CryptoData)
        cb(CryptoData)
    end, data.crypto)
end)

RegisterNUICallback('BuyCrypto', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-crypto:server:BuyCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('SellCrypto', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-crypto:server:SellCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)

RegisterNUICallback('TransferCrypto', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-crypto:server:TransferCrypto', function(CryptoData)
        cb(CryptoData)
    end, data)
end)
...
RegisterNUICallback('GetAvailableRaces', function(_, cb)
    QBCore.Functions.TriggerCallback('poly-lapraces:server:GetRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('JoinRace', function(data, cb)
    TriggerServerEvent('poly-lapraces:server:JoinRace', data.RaceData)
    cb('ok')
end)

RegisterNUICallback('LeaveRace', function(data, cb)
    TriggerServerEvent('poly-lapraces:server:LeaveRace', data.RaceData)
    cb('ok')
end)

RegisterNUICallback('StartRace', function(data, cb)
    TriggerServerEvent('poly-lapraces:server:StartRace', data.RaceData.RaceId)
    cb('ok')
end)
...
RegisterNUICallback('FetchVehicleResults', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-phone:server:GetVehicleSearchResults', function(result)
        if result ~= nil then
...
RegisterNUICallback('FetchVehicleScan', function(_, cb)
    ...
    QBCore.Functions.TriggerCallback('poly-phone:server:ScanPlate', function(result)
        QBCore.Functions.TriggerCallback('police:IsPlateFlagged', function(flagged)
...
RegisterNUICallback('GetRaces', function(_, cb)
    QBCore.Functions.TriggerCallback('poly-lapraces:server:GetListedRaces', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('GetTrackData', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-lapraces:server:GetTrackData', function(TrackData, CreatorData)
...
RegisterNUICallback('SetupRace', function(data, cb)
    TriggerServerEvent('poly-lapraces:server:SetupRace', data.RaceId, tonumber(data.AmountOfLaps))
    cb("ok")
end)

RegisterNUICallback('HasCreatedRace', function(_, cb)
    QBCore.Functions.TriggerCallback('poly-lapraces:server:HasCreatedRace', function(HasCreated)
        cb(HasCreated)
    end)
end)

RegisterNUICallback('IsInRace', function(_, cb)
    local InRace = exports['poly-lapraces']:IsInRace()
    cb(InRace)
end)

RegisterNUICallback('IsAuthorizedToCreateRaces', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-lapraces:server:IsAuthorizedToCreateRaces', function(IsAuthorized, NameAvailable)
        data = {
            IsAuthorized = IsAuthorized,
            IsBusy = exports['poly-lapraces']:IsInEditor(),
...
RegisterNUICallback('StartTrackEditor', function(data, cb)
    TriggerServerEvent('poly-lapraces:server:CreateLapRace', data.TrackName)
    cb("ok")
end)

RegisterNUICallback('GetRacingLeaderboards', function(_, cb)
    QBCore.Functions.TriggerCallback('poly-lapraces:server:GetRacingLeaderboards', function(Races)
        cb(Races)
    end)
end)

RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            if data.Joined then
                TriggerEvent('poly-lapraces:client:WaitingDistanceCheck')
            end
            cb(true)
        else
...
RegisterNUICallback('IsBusyCheck', function(data, cb)
    if data.check == "editor" then
        cb(exports['poly-lapraces']:IsInEditor())
    else
        cb(exports['poly-lapraces']:IsInRace())
    end
end)

RegisterNUICallback('CanRaceSetup', function(_, cb)
    QBCore.Functions.TriggerCallback('poly-lapraces:server:CanRaceSetup', function(CanSetup)
        cb(CanSetup)
    end)
end)

RegisterNUICallback('GetPlayerHouses', function(_, cb)
    QBCore.Functions.TriggerCallback('poly-phone:server:GetPlayerHouses', function(Houses)
        cb(Houses)
    end)
end)

RegisterNUICallback('GetPlayerKeys', function(_, cb)
    QBCore.Functions.TriggerCallback('poly-phone:server:GetHouseKeys', function(Keys)
        cb(Keys)
    end)
end)
...
RegisterNUICallback('RemoveKeyholder', function(data, cb)
    TriggerServerEvent('poly-houses:server:removeHouseKey', data.HouseData.name, {
...
RegisterNUICallback('TransferCid', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('poly-phone:server:TransferCid', function(CanTransfer)
        cb(CanTransfer)
    end, TransferedCid, data.HouseData)
end)

RegisterNUICallback('FetchPlayerHouses', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-phone:server:MeosGetPlayerHouses', function(result)
        cb(result)
    end, data.input)
end)
...
RegisterNUICallback('GetCurrentLawyers', function(_, cb)
    QBCore.Functions.TriggerCallback('poly-phone:server:GetCurrentLawyers', function(lawyers)
        cb(lawyers)
    end)
end)
...
RegisterNUICallback('ClearMentions', function(_ ,cb)
    ...
    TriggerServerEvent('poly-phone:server:SetPhoneAlerts', "twitter", 0)
    SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
    cb('ok')
end)
...
RegisterNUICallback('ClearGeneralAlerts', function(data, cb)
    SetTimeout(400, function()
        ...
        TriggerServerEvent('poly-phone:server:SetPhoneAlerts', data.app, 0)
        SendNUIMessage({ action = "RefreshAppAlerts", AppData = Config.PhoneApplications })
        cb('ok')
    end)
end)
...
RegisterNUICallback('TransferMoney', function(data, cb)
    ...
    if tonumber(PhoneData.PlayerData.money.bank) >= data.amount then
        ...
        TriggerServerEvent('poly-phone:server:TransferMoney', data.iban, data.amount)
        local cbdata = {
            CanTransfer = true,
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
        QBCore.Functions.TriggerCallback('poly-phone:server:CanTransferMoney', function(Transferd)
            if Transferd then
...
RegisterNUICallback('GetWhatsappChats', function(_, cb)
    QBCore.Functions.TriggerCallback('poly-phone:server:GetContactPictures', function(Chats)
        cb(Chats)
    end, PhoneData.Chats)
end)

RegisterNUICallback('CallContact', function(data, cb)
    QBCore.Functions.TriggerCallback('poly-phone:server:GetCallState', function(CanCall, IsOnline, _)
        local status = {
            CanCall = CanCall,
            IsOnline = IsOnline,
...
RegisterNUICallback('SendMessage', function(data, cb)
    ...
    if PhoneData.Chats[NumberKey] ~= nil then
        ...
        if PhoneData.Chats[NumberKey].messages[ChatKey] ~= nil then
            ...
            TriggerServerEvent('poly-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, false)
            ...
            TriggerServerEvent('poly-phone:server:UpdateMessages', PhoneData.Chats[NumberKey].messages, ChatNumber, true)
            NumberKey = GetKeyByNumber(ChatNumber)
            ReorganizeChats(NumberKey)
        end
    else
    ...
    QBCore.Functions.TriggerCallback('poly-phone:server:GetContactPicture', function(Chat)
        SendNUIMessage({
            action = "UpdateChat",
            chatData = Chat,
...
RegisterNUICallback("TakePhoto", function(_,cb)
    ...
    while takePhoto do
        ...
        elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
            QBCore.Functions.TriggerCallback("poly-phone:server:GetWebhook",function(hook)
                if hook then
                    exports['screenshot-basic']:requestScreenshotUpload(tostring(hook), "files[]", function(data)
                        local image = json.decode(data)
                        DestroyMobilePhone()
                        CellCamActivate(false, false)
                        TriggerServerEvent('poly-phone:server:addImageToGallery', image.attachments[1].proxy_url)
                        Wait(400)
                        TriggerServerEvent('poly-phone:server:getImageFromGallery')
...
RegisterCommand('ping', function(_, args)
    if not args[1] then
        ...
        TriggerServerEvent('poly-phone:server:sendPing', args[1])
    end
end)
...
RegisterNetEvent('poly-phone:client:TransferMoney', function(amount, newmoney)
...
-- RegisterNetEvent('poly-phone:client:UpdateTweetsDel', function(source, Tweets)
...
RegisterNetEvent('poly-phone:client:UpdateTweets', function(src, Tweets, NewTweetData, delete)
    PhoneData.Tweets = Tweets
    local MyPlayerId = PhoneData.PlayerData.source
...
RegisterNetEvent('poly-phone:client:RaceNotify', function(message)
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = "Racing",
...
RegisterNetEvent('poly-phone:client:AddRecentCall', function(data, time, type)
    ...
    TriggerServerEvent('poly-phone:server:SetPhoneAlerts', "phone")
...
RegisterNetEvent("poly-phone-new:client:BankNotify", function(text)
    SendNUIMessage({
        action = "PhoneNotification",
        NotifyData = {
            title = "Bank",
...
RegisterNetEvent('poly-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = "Mail",
    ...
    TriggerServerEvent('poly-phone:server:SetPhoneAlerts', "mail")
end)

RegisterNetEvent('poly-phone:client:UpdateMails', function(NewMails)
    SendNUIMessage({
        action = "UpdateMails",
        Mails = NewMails
    })
...
RegisterNetEvent('poly-phone:client:UpdateAdvertsDel', function(Adverts)
...
RegisterNetEvent('poly-phone:client:UpdateAdverts', function(Adverts, LastAd)
    PhoneData.Adverts = Adverts
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
...
RegisterNetEvent('poly-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('poly-phone:server:sendNewMail', {
...
RegisterNetEvent('poly-phone:client:CancelCall', function()
    if PhoneData.CallData.CallType == "ongoing" then
        SendNUIMessage({
...
RegisterNetEvent('poly-phone:client:GetCalled', function(CallerNumber, CallId, AnonymousCall)
    local RepeatCount = 0
    local CallData = {
        number = CallerNumber,
        ...
        if not PhoneData.CallData.AnsweredCall then
            if RepeatCount + 1 ~= Config.CallRepeats + 1 then
                if PhoneData.CallData.InCall then
                    ...
                    TriggerServerEvent('poly-phone:server:AddRecentCall', "missed", CallData)
                    break
                end
                Wait(Config.RepeatTimeout)
            else
...
RegisterNetEvent('poly-phone:client:UpdateMessages', function(ChatMessages, SenderNumber, New)
    ...
    if New then
	    ...
	    SendNUIMessage({
            ...
            TriggerServerEvent('poly-phone:server:SetPhoneAlerts', "whatsapp")
        end
    else
...
RegisterNetEvent('poly-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            action = "PhoneNotification",
            PhoneNotify = {
...
RegisterNetEvent('poly-phone:RefreshPhone', function()
    LoadPhone()
    SetTimeout(250, function()
        SendNUIMessage({
...
RegisterNetEvent('poly-phone:client:AddTransaction', function(_, _, Message, Title)
    local Data = {
    ...
    TriggerServerEvent('poly-phone:server:AddTransaction', Data)
end)

RegisterNetEvent('poly-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    Config.PhoneApplications["phone"].Alerts = Config.PhoneApplications["phone"].Alerts + 1
    TriggerServerEvent('poly-phone:server:SetPhoneAlerts', "phone", Config.PhoneApplications["phone"].Alerts)
end)

RegisterNetEvent('poly-phone:client:UpdateHashtags', function(Handle, msgData)
...
RegisterNetEvent('poly-phone:client:AnswerCall', function()
    if (PhoneData.CallData.CallType == "incoming" or PhoneData.CallData.CallType == "outgoing") and PhoneData.CallData.InCall and not PhoneData.CallData.AnsweredCall then
        PhoneData.CallData.CallType = "ongoing"
        PhoneData.CallData.AnsweredCall = true
        PhoneData.CallData.CallTime = 0
...
RegisterNetEvent('poly-phone:client:addPoliceAlert', function(alertData)
    PlayerJob = QBCore.Functions.GetPlayerData().job
...
RegisterNetEvent('poly-phone:client:GiveContactDetails', function()
    local player, distance = GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local PlayerId = GetPlayerServerId(player)
        TriggerServerEvent('poly-phone:server:GiveContactDetails', PlayerId)
    else
        QBCore.Functions.Notify("No one nearby!", "error")
    end
end)

RegisterNetEvent('poly-phone:client:UpdateLapraces', function()
    SendNUIMessage({
        action = "UpdateRacingApp",
    })
end)

RegisterNetEvent('poly-phone:client:GetMentioned', function(TweetMessage, AppAlerts)
...
RegisterNetEvent('poly-phone:refreshImages', function(images)
    PhoneData.Images = images
end)

RegisterNetEvent("poly-phone:client:CustomNotification", function(title, text, icon, color, timeout) -- Send a PhoneNotification to the phone from anywhere
    SendNUIMessage({
        action = "PhoneNotification",
        PhoneNotify = {
            title = title,

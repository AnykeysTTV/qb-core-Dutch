...
local function RegisterTraphouseEntranceTarget(traphouseID, traphouseData)
    ...
    exports['poly-target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        name = boxName,
        ...
        options = {
            {
                type = 'client',
                event = 'poly-traphouse:client:EnterTraphouse',
...
local function RegisterTraphouseEntranceZone(traphouseID, traphouseData)
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside then
            exports['poly-core']:DrawText('[E] ' .. Lang:t('targetInfo.enter'), 'left')
        else
...
local function RegisterTraphouseInteractionZone(traphouseID, traphouseData)
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside then
            exports['poly-core']:DrawText('[E] ' .. Lang:t('targetInfo.options'), 'left')
        else
            exports['poly-core']:HideText()
            TriggerEvent('poly-traphouse:client:target:CloseMenu')
        end
...
local function RegisterTraphouseInteractionTarget(traphouseID, traphouseData)
    ...
    local options = {
        {
            type = "client",
            event = "poly-traphouse:client:target:TakeOver",
    ...
    if IsKeyHolder then
        options = {
            {
                type = "client",
                event = "poly-traphouse:client:target:ViewInventory",
                label = Lang:t("targetInfo.inventory"),
                traphouseData = traphouseData
            },
            {
                type = "client",
                event = "poly-traphouse:client:target:TakeMoney",
        ...
        if IsHouseOwner then
            options[#options+1] = {
                type = "client",
                event = "poly-traphouse:client:target:SeePinCode",
...
local function RegisterTraphouseExitZone(coords, traphouseID, traphouseData)
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside then
            exports['poly-core']:DrawText('[E] ' .. Lang:t("targetInfo.leave"), 'left')
        else
            exports['poly-core']:HideText()
        end
...
local function RegisterTraphouseExitTarget(coords, traphouseID, traphouseData)
    ...
    exports['poly-target']:AddBoxZone(boxName, coords, boxData.length, boxData.width, {
        ...
        options = {
            {
                type = "client",
                event = "poly-traphouse:client:target:ExitTraphouse",
...
local function OpenHeaderMenu(data)
    ...
    headerMenu[#headerMenu+1] = {
        ...
        params = {
            event = "poly-traphouse:client:target:CloseMenu",
        }
    }
...
local function EnterTraphouse(data)
    ...
    data = exports['poly-interior']:CreateTrevorsShell(coords)
    TraphouseObj = data[1]
    POIOffsets = data[2]
...
local function LeaveTraphouse(k, data)
    ...
    DoScreenFadeOut(250)
    Wait(250)
    exports['poly-interior']:DespawnInterior(TraphouseObj, function()
    ...
    if Config.UseTarget then
        exports['poly-target']:RemoveZone('traphouseInteraction' .. k)
        data.polyzoneBoxData['interaction'].created = false

        exports['poly-target']:RemoveZone('traphouseExit' .. k)
...
RegisterNetEvent('poly-traphouse:client:EnterTraphouse', function()
    if ClosestTraphouse ~= nil then
...
RegisterNetEvent('poly-traphouse:client:TakeoverHouse', function(TraphouseId)
    QBCore.Functions.Progressbar("takeover_traphouse", Lang:t("info.taking_over"), math.random(1000, 3000), false, true, {
        ...
        TriggerServerEvent('poly-traphouse:server:AddHouseKeyHolder', PlayerData.citizenid, TraphouseId, true)
    end, function()
        QBCore.Functions.Notify(Lang:t("error.cancelled"), "error")
    end)
end)

RegisterNetEvent('poly-traphouse:client:target:ViewInventory', function (data)
...
RegisterNetEvent('poly-traphouse:client:target:TakeOver', function ()
    TriggerServerEvent('poly-traphouse:server:TakeoverHouse', CurrentTraphouse)
end)

RegisterNetEvent('poly-traphouse:client:target:TakeMoney', function ()
    TriggerServerEvent("poly-traphouse:server:TakeMoney", CurrentTraphouse)
end)

RegisterNetEvent('poly-traphouse:client:target:SeePinCode', function (data)
    QBCore.Functions.Notify(Lang:t('info.pin_code', { value = data.traphouseData.pincode }))
end)

RegisterNetEvent('poly-traphouse:client:target:ExitTraphouse', function (data)
    LeaveTraphouse(data.traphouseID, Config.TrapHouses[data.traphouseID])
end)

RegisterNetEvent('poly-traphouse:client:SyncData', function(k, data)
...
RegisterNetEvent('poly-traphouse:client:target:CloseMenu', function ()
                                    ...
                                    if not IsEntityDead(targetPed) then
                                        if CanRob then
                                            if InDistance then
                                                ...
                                                TriggerServerEvent('poly-traphouse:server:RobNpc', ClosestTraphouse)
                                                CanRob = false
                                            end
                                        end
                                    end
            ...
            if not InsideTraphouse then
                if isInsideEntranceTarget then
                    ...
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent("poly-traphouse:client:EnterTraphouse")
                        exports['poly-core']:HideText()
                    end
                end
            else

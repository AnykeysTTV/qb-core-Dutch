        ...
        if itemName == "markedbills" then
            SetTimeout(math.random(1000, 5000), function()
                if Config.TrapHouses[traphouseId].inventory[slot] ~= nil then
                    ...
                    TriggerClientEvent('poly-traphouse:client:SyncData', -1, traphouseId, Config.TrapHouses[traphouseId])
                end
            end)
        else
            for _ = 1, amount, 1 do
...
RegisterServerEvent('poly-traphouse:server:TakeoverHouse', function(Traphouse)
    local src = source
    ...
    if not HasCitizenIdHasKey(CitizenId, Traphouse) then
        if Player.Functions.RemoveMoney('cash', Config.TakeoverPrice) then
            TriggerClientEvent('poly-traphouse:client:TakeoverHouse', src, Traphouse)
        else
...
RegisterServerEvent('poly-traphouse:server:AddHouseKeyHolder', function(CitizenId, TraphouseId, IsOwner)
    local src = source
...
RegisterServerEvent('poly-traphouse:server:TakeMoney', function(TraphouseId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('poly-traphouse:server:RobNpc', function(Traphouse)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
            ...
            if not HasCitizenIdHasKey(TargetData.PlayerData.citizenid, Traphouse) then
                if Config.TrapHouses[Traphouse] ~= nil then
                    ...
                    if Config.TrapHouses[Traphouse].keyholders == nil then
                        ...
                        TriggerClientEvent('poly-traphouse:client:SyncData', -1, Traphouse, Config.TrapHouses[Traphouse])
                    else

...
local function GetDeliveryLocation()
    ...
    PolyZone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inRange = true
            exports["poly-core"]:DrawText(Lang:t('info.busstop_text'), 'rgb(220, 20, 60)')
            CreateThread(function()
                repeat
                    ...
                    if IsControlJustPressed(0, 38) then
                        ...
                        resetNpcTask()
                        nextStop()
                        TriggerEvent('poly-busjob:client:DoBusNpc')
...
local function busGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu + 1] = {
            header = v.label,
            params = {
                event = "poly-busjob:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent("poly-busjob:client:TakeVehicle", function(data)
...
RegisterNetEvent('poly-busjob:client:DoBusNpc', function()
                    ...
                    CreateThread(function()
                        repeat
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                TriggerServerEvent('poly-busjob:server:NpcPay')
                                exports["poly-core"]:HideText()
                                PolyZone:destroy()
                                break
                            end
            ...
            if isPointInside then
                ...
                CreateThread(function()
                    repeat
                        Wait(5)
                        if not inVeh then
                            exports["poly-core"]:DrawText(Lang:t('info.busstop_text'), 'left')
                            ...
                            exports["poly-core"]:DrawText(Lang:t('info.bus_stop_work'), 'left')
                            if IsControlJustReleased(0, 38) then

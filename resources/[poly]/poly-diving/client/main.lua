...
local function callCops()
    ...
    if call == chance then
        TriggerServerEvent('poly-diving:server:CallCops', coords)
    end
end
...
local function takeCoral(coral)
    ...
    QBCore.Functions.Progressbar("take_coral", Lang:t("info.collecting_coral"), times * 1000, false, true, {
        ...
        TriggerServerEvent('poly-diving:server:TakeCoral', currentDivingLocation.area, coral, true)
        ClearPedTasks(ped)
...
local function setDivingLocation(divingLocation)
    if currentDivingLocation.area ~= 0 then
        for k in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
            if Config.UseTarget then
                exports['poly-target']:RemoveZone(k)
            else
    ...
    for k, v in pairs(Config.CoralLocations[currentDivingLocation.area].coords.Coral) do
        if Config.UseTarget then
            exports['poly-target']:AddBoxZone('diving_coral_zone_'..k, v.coords, v.length, v.width, {
            ...
            zones[k]:onPlayerInOut(function(inside)
                if inside then
                    currentArea = k
                    exports['poly-core']:DrawText(Lang:t("info.collect_coral_dt"))
                else
                    currentArea = 0
...
local function sellCoral()
    ...
    QBCore.Functions.Progressbar("sell_coral_items", Lang:t("info.checking_pockets"), math.random(2000, 4000), false, true, {}, {}, {}, {}, function() -- Done
        ...
        TriggerServerEvent('poly-diving:server:SellCoral')
    ...
    for i = 1, #Config.SellLocations do
        ...
        if Config.UseTarget then
            ...
            zone:onPlayerInOut(function(inside)
                if inside then
                    inSellerZone = true
                    exports['poly-core']:DrawText(Lang:t("info.sell_coral_dt"))
                else
                    inSellerZone = false
                    exports['poly-core']:HideText()
                end
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('poly-diving:server:GetDivingConfig', function(config, area)
...
RegisterNetEvent('poly-diving:client:NewLocations', function()
    QBCore.Functions.TriggerCallback('poly-diving:server:GetDivingConfig', function(config, area)
        Config.CoralLocations = config
        setDivingLocation(area)
    end)
...
RegisterNetEvent('poly-diving:client:UpdateCoral', function(area, coral, bool)
    Config.CoralLocations[area].coords.Coral[coral].PickedUp = bool
end)

RegisterNetEvent('poly-diving:client:CallCops', function(coords, msg)
...
RegisterNetEvent("poly-diving:client:setoxygenlevel", function()
    if oxgenlevell == 0 then
       ...
       TriggerServerEvent('poly-diving:server:removeItemAfterFill')
    else
...
RegisterNetEvent('poly-diving:client:UseGear', function()
    local ped = PlayerPedId()

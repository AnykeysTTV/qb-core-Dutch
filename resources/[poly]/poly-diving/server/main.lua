...
RegisterNetEvent('poly-diving:server:CallCops', function(coords)
    for _, Player in pairs(QBCore.Functions.GetQBPlayers()) do
        if Player then
            if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
                local msg = Lang:t("info.cop_msg")
                TriggerClientEvent('poly-diving:client:CallCops', Player.PlayerData.source, coords, msg)
                local alertData = {
...
RegisterNetEvent('poly-diving:server:SellCoral', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-diving:server:TakeCoral', function(area, coral, bool)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if (Config.CoralLocations[area].TotalCoral - 1) == 0 then
        ...
        TriggerClientEvent('poly-diving:client:NewLocations', -1)
    else
    ...
    TriggerClientEvent('poly-diving:client:UpdateCoral', -1, area, coral, bool)
end)

RegisterNetEvent('poly-diving:server:removeItemAfterFill', function()
   local src = source
   local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('poly-diving:server:GetDivingConfig', function(_, cb)
...
QBCore.Functions.CreateUseableItem("diving_gear", function(source)
    TriggerClientEvent("poly-diving:client:UseGear", source)
end)

QBCore.Functions.CreateUseableItem("diving_fill", function(source)
    TriggerClientEvent("poly-diving:client:setoxygenlevel", source)
end)

                ...
                if dist < 20 then
                    ...
                    if dist < 5 then
                        ...
                        if IsControlJustPressed(0, 38) then
                            ...
                            TriggerServerEvent('poly-taxi:server:NpcPay', meterData.currentFare)
                            meterActive = false
                            SendNUIMessage({
                                action = "resetMeter"
                            })
...
function TaxiGarage()
    ...
    for _, v in pairs(Config.AllowedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            header = v.label,
            params = {
                event = "poly-taxi:client:TakeVehicle",
                args = {
                    model = v.model
                }
            }
...
RegisterNetEvent("poly-taxi:client:TakeVehicle", function(data)
...
-- Events
RegisterNetEvent('poly-taxi:client:DoTaxiNpc', function()
...
RegisterNetEvent('poly-taxi:client:toggleMeter', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('poly-taxi:client:enableMeter', function()
    if meterIsOpen then
        SendNUIMessage({
            action = "toggleMeter"
        })
...
RegisterNetEvent('poly-taxi:client:toggleMuis', function()
    Wait(400)
    if meterIsOpen then
...
RegisterNetEvent('poly-taxijob:client:requestcab', function()
    TaxiGarage()
end)
        ...
        exports['poly-target']:SpawnPed({
            ...
            target = {
                options = {
                    {
                        type = "client",
                        event = "poly-taxijob:client:requestcab",
    ...
    zone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsidePickupZone and not NpcData.NpcTaken then
                ...
                exports['poly-core']:DrawText(Lang:t("info.call_npc"), Config.DefaultTextLocation)
                callNpcPoly()
            end
        else
    ...
    delieveryZone:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside then
            if whitelistedVehicle() and not isInsideDropZone and NpcData.NpcTaken then
                ...
                exports['poly-core']:DrawText(Lang:t("info.drop_off_npc"), Config.DefaultTextLocation)
                dropNpcPoly()
            end
        else
...
function setupCabParkingLocation()
    ...
    taxiParking:onPlayerInOut(function(isPlayerInside)
        if isPlayerInside and not Notified and Config.UseTarget then
            if whitelistedVehicle() then
                exports['poly-core']:DrawText(Lang:t("info.vehicle_parking"), Config.DefaultTextLocation)
                Notified = true
        ...
        if isPlayerInsideZone then
            if IsControlJustReleased(0, 38) then
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    ...
                    if meterIsOpen then
                        TriggerEvent('poly-taxi:client:toggleMeter')
                        meterActive = false
                    end
...
-- switched boss menu from poly-bossmenu to taxijob
CreateThread(function()
    while true do
        local sleep = 1000

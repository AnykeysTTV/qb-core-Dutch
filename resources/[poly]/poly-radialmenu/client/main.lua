    ...
    if Vehicle ~= 0 then
        ...
        if not IsVehicleOnAllWheels(Vehicle) then
            VehicleMenu.items[#VehicleMenu.items+1] = {
                ...
                icon = 'car-burst',
                type = 'client',
                event = 'poly-radialmenu:flipVehicle',
                shouldClose = true
            }
        end
        ...
        if IsPedInAnyVehicle(ped) then
            ...
            for i = 1, AmountOfSeats do
                ...
                VehicleMenu.items[seatIndex].items[newIndex] = {
                    ...
                    icon = 'caret-up',
                    type = 'client',
                    event = 'poly-radialmenu:client:ChangeSeat',
                    shouldClose = false,
                }
            end
        end
...
local function controlToggle(bool)
    for i = 1, #controlsToToggle,1 do
        if bool then
            exports['poly-smallresources']:addDisableControls(controlsToToggle[i])
        else
            exports['poly-smallresources']:removeDisableControls(controlsToToggle[i])
        end
    end
end
...
local function setRadialState(bool, sendMessage, delay)
    ...
    if Config.UseWhilstWalking then
        ...
        if bool then
            TriggerEvent('poly-radialmenu:client:onRadialmenuOpen')
            SetupRadialMenu()
        else
            TriggerEvent('poly-radialmenu:client:onRadialmenuClose')
        end
        SetNuiFocus(bool, bool)
    end
...
RegisterNetEvent('poly-radialmenu:client:noPlayers', function()
    QBCore.Functions.Notify(Lang:t("error.no_people_nearby"), 'error', 2500)
end)

RegisterNetEvent('poly-radialmenu:client:openDoor', function(data)
        ...
        if closestVehicle ~= GetVehiclePedIsIn(ped) then
            ...
            if GetVehicleDoorAngleRatio(closestVehicle, door) > 0.0 then
                if not IsVehicleSeatFree(closestVehicle, -1) then
                    TriggerServerEvent('poly-radialmenu:trunk:server:Door', false, plate, door)
                else
                    SetVehicleDoorShut(closestVehicle, door, false)
                end
            else
                if not IsVehicleSeatFree(closestVehicle, -1) then
                    TriggerServerEvent('poly-radialmenu:trunk:server:Door', true, plate, door)
                else
...
RegisterNetEvent('poly-radialmenu:client:setExtra', function(data)
    local string = data.id
...
RegisterNetEvent('poly-radialmenu:trunk:client:Door', function(plate, door, open)
...
RegisterNetEvent('poly-radialmenu:client:ChangeSeat', function(data)
    local Veh = GetVehiclePedIsIn(PlayerPedId())
    local IsSeatFree = IsVehicleSeatFree(Veh, data.id)
    local speed = GetEntitySpeed(Veh)
    local HasHarnass = exports['poly-smallresources']:HasHarness()
...
RegisterNetEvent('poly-radialmenu:flipVehicle', function()

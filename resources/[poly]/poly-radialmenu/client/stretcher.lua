...
local function LayOnStretcher()
    ...
    if player == -1 then
        ...
        if distance < 2.0 then
            TriggerServerEvent('poly-radialmenu:Stretcher:BusyCheck', GetPlayerServerId(player), "lay")
        else
            loadAnim(inBedDicts)
            if stretcherObject and #(coords - GetEntityCoords(stretcherObject)) <= 3.0 then
                TaskPlayAnim(PlayerPedId(), inBedDicts, inBedAnims, 8.0, 8.0, -1, 69, 1, false, false, false)
...
local function attachToStretcher()
    ...
    if stretcherObject then
        if closestPlayer == -1 then
            ...
            if distance < 2.0 then
                TriggerServerEvent('poly-radialmenu:Stretcher:BusyCheck', GetPlayerServerId(closestPlayer), "attach")
            else
                NetworkRequestControlOfEntity(stretcherObject)
                loadAnim("anim@heists@box_carry@")
                TaskPlayAnim(ped, 'anim@heists@box_carry@', 'idle', 8.0, 8.0, -1, 50, 0, false, false, false)
...
RegisterNetEvent('poly-radialmenu:client:TakeStretcher', function()
...
RegisterNetEvent('poly-radialmenu:client:RemoveStretcher', function()
    local ped = PlayerPedId()
    ...
    if stretcherObject then
        ...
        if #(coords - bCoords) < 3.0 then
            if DoesEntityExist(stretcherObject) then
                ...
                ClearPedTasks(ped)
                DetachEntity(ped, false, true)
                TriggerServerEvent('poly-radialmenu:server:RemoveStretcher', coords, stretcherObject)
                isAttached = false
                stretcherObject = nil
                isLayingOnBed = false
            end
...
RegisterNetEvent('poly-radialmenu:client:RemoveStretcherFromArea', function(playerPos, bObject)
    local ped = PlayerPedId()
...
RegisterNetEvent('poly-radialmenu:Stretcher:client:BusyCheck', function(otherId, type)
    local ped = PlayerPedId()
    if type == "lay" then
        loadAnim("anim@gangops@morgue@table@")
        if IsEntityPlayingAnim(ped, "anim@gangops@morgue@table@", "ko_front", 3) then
            TriggerServerEvent('poly-radialmenu:server:BusyResult', true, otherId, type)
        else
            TriggerServerEvent('poly-radialmenu:server:BusyResult', false, otherId, type)
        end
    else
        loadAnim('anim@heists@box_carry@')
...
RegisterNetEvent('poly-radialmenu:client:Result', function(isBusy, type)
    local ped = PlayerPedId()
    local inBedDicts = "anim@gangops@morgue@table@"
    local inBedAnims = "ko_front"
    if type == "lay" then

...
RegisterNetEvent('poly-kidnapping:client:SetKidnapping', function(bool)
    isKidnapping = bool
end)

RegisterNetEvent('poly-trunk:client:KidnapTrunk', function()
    ...
    if distance ~= -1 and distance < 2 then
        if isKidnapping then
            ...
            if closestVehicle ~= 0 then
                ...
                Wait(50)
                TriggerServerEvent("poly-trunk:server:KidnapTrunk", GetPlayerServerId(closestPlayer), closestVehicle)
            end
        else
...
RegisterNetEvent('poly-trunk:client:KidnapGetIn', function(veh)
    ...
    if Config.TrunkClasses[vehClass].allowed then
        QBCore.Functions.TriggerCallback('poly-trunk:server:getTrunkBusy', function(isBusy)
                    ...
                    if not isBusy then
                        if not isKidnapped then
                            if GetVehicleDoorAngleRatio(closestVehicle, 5) > 0 then
                                ...
                                TriggerServerEvent('poly-trunk:server:setTrunkBusy', plate, true)
                                inTrunk = true
                                Wait(500)
                            ...
                            if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                                ...
                                ClearPedTasks(ped)
                                inTrunk = false
                                TriggerServerEvent('poly-smallresources:trunk:server:setTrunkBusy', plate, nil)
...
RegisterNetEvent('poly-trunk:client:GetIn', function()
    local ped = PlayerPedId()
    local closestVehicle = getNearestVeh()
    if closestVehicle ~= 0 then
        local vehClass = GetVehicleClass(closestVehicle)
                ...
                if DoesEntityExist(vehicle) then
                    ...
                    if IsControlJustPressed(0, 38) then
                        if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                            ...
                            ClearPedTasks(ped)
                            inTrunk = false
                            TriggerServerEvent('poly-trunk:server:setTrunkBusy', plate, false)
                            SetEntityCoords(ped, vehCoords.x, vehCoords.y, vehCoords.z)
                    ...
                    if GetVehicleDoorAngleRatio(vehicle, 5) > 0 then
                        ...
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('poly-radialmenu:trunk:server:Door', false, plate, 5)
                            else
                        ...
                        if IsControlJustPressed(0, 47) then
                            if not IsVehicleSeatFree(vehicle, -1) then
                                TriggerServerEvent('poly-radialmenu:trunk:server:Door', true, plate, 5)
                            else

...
QBCore.Functions.CreateCallback('poly-cityhall:server:receiveJobs', function(_, cb)
    cb(availableJobs)
end)

-- Events

RegisterNetEvent('poly-cityhall:server:requestId', function(item, hall)
    local src = source
...
RegisterNetEvent('poly-cityhall:server:sendDriverTest', function(instructors)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    for i = 1, #instructors do
        ...
        if SchoolPlayer then
            TriggerClientEvent("poly-cityhall:client:sendDriverEmail", SchoolPlayer.PlayerData.source, Player.PlayerData.charinfo)
        else
            local mailData = {
                sender = "Township",
            ...
            exports["poly-phone"]:sendNewMailToOffline(citizenid, mailData)
        end
    end
...
RegisterNetEvent('poly-cityhall:server:ApplyJob', function(job, cityhallCoords)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if QBCore.Shared.QBJobsStatus then
        exports["poly-jobs"]:submitApplication(data, "Jobs")
    else
...
RegisterNetEvent('poly-cityhall:server:getIDs', giveStarterItems)

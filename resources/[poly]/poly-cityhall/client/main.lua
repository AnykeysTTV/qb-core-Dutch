...
local function getJobs()
    QBCore.Functions.TriggerCallback('poly-cityhall:server:receiveJobs', function(result)
        SendNUIMessage({
            action = 'setJobs',
            jobs = result
        })
        ...
        if Config.UseTarget then
            ...
            if current.drivingschool then
                opts = {
                    ...
                    action = function()
                        TriggerServerEvent('poly-cityhall:server:sendDriverTest', Config.DrivingSchools[closestDrivingSchool].instructors)
                    end
                }
                    ...
                    if isLoggedIn and closestCityhall and closestDrivingSchool then
                        if inside then
                            if current.drivingschool then
                                ...
                                exports['poly-core']:DrawText('[E] Take Driving Lessons')
                            elseif current.cityhall then
                                inRangeCityhall = true
                                exports['poly-core']:DrawText('[E] Open Cityhall')
                            end
                        else
...
RegisterNetEvent('poly-cityhall:Client:AddCityJob', function()
    getJobs()
end)

RegisterNetEvent('poly-cityhall:client:getIds', function()
    TriggerServerEvent('poly-cityhall:server:getIDs')
end)

RegisterNetEvent('poly-cityhall:client:sendDriverEmail', function(charinfo)
...
RegisterNUICallback('close', function(_, cb)
    ...
    if not Config.UseTarget and inRangeCityhall then exports['poly-core']:DrawText('[E] Open Cityhall') end -- Reopen interaction when you're still inside the zone
    cb('ok')
end)
...
RegisterNUICallback('requestId', function(id, cb)
    ...
    if inRangeCityhall and license and id.cost == license.cost then
        TriggerServerEvent('poly-cityhall:server:requestId', id.type, closestCityhall)
...
RegisterNUICallback('applyJob', function(job, cb)
    if inRangeCityhall then
        TriggerServerEvent('poly-cityhall:server:ApplyJob', job, Config.Cityhalls[closestCityhall].coords)
    else
        QBCore.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
    cb('ok')

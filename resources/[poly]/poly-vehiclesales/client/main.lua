        ...
        if vehicles then
            for i = 1, #vehicles, 1 do
                ...
                if Config.UseTarget then
                    ...
                    EntityZones[i] = exports['poly-target']:AddTargetEntity(occasionVehicles[Zone][i].car, {
                        options = {
                            {
                                type = "client",
                                event = "poly-vehiclesales:client:OpenContract",
                                icon = "fas fa-car",
...
local function despawnOccasionsVehicles()
    ...
    for i = 1, #oSlot, 1 do
        ...
        if EntityZones[i] and Config.UseTarget then
            exports['poly-target']:RemoveZone(EntityZones[i])
        end
    end
    EntityZones = {}
end
...
local function SellData(data, model)
    QBCore.Functions.TriggerCallback("poly-vehiclesales:server:CheckModelName",function(DataReturning)
        local vehicleData = {}
        ...
        TriggerServerEvent('poly-occasions:server:sellVehicle', data.price, vehicleData)
        ...
        while listen do
            if IsControlJustReleased(0, 38) then -- E
                if spot then
                    ...
                    TriggerEvent('poly-vehiclesales:client:OpenContract', data)
                else
                    if IsPedInAnyVehicle(PlayerPedId(), false) then
                        listen = false
                        TriggerEvent('poly-occasions:client:MainMenu')
                        --TriggerEvent('poly-vehiclesales:client:SellVehicle')
                    else
    ...
    for k, v in pairs(Config.Zones) do
        ...
        SellSpot:onPlayerInOut(function(isPointInside)
            if isPointInside and Zone ~= k then
                Zone = k
                QBCore.Functions.TriggerCallback('poly-occasions:server:getVehicles', function(vehicles)
...
RegisterNUICallback('buyVehicle', function(_, cb)
    TriggerServerEvent('poly-occasions:server:buyVehicle', CurrentVehicle)
    cb('ok')
end)

RegisterNUICallback('takeVehicleBack', function(_, cb)
    TriggerServerEvent('poly-occasions:server:ReturnVehicle', CurrentVehicle)
    cb('ok')
end)

-- Events

RegisterNetEvent('poly-occasions:client:BuyFinished', function(vehdata)
...
RegisterNetEvent('poly-occasions:client:SellBackCar', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        QBCore.Functions.TriggerCallback('poly-garage:server:checkVehicleOwner', function(owned, balance)
            if owned then
                if balance < 1 then
                    TriggerServerEvent('poly-occasions:server:sellVehicleBack', vehicleData)
...
RegisterNetEvent('poly-occasions:client:ReturnOwnedVehicle', function(vehdata)
    local vehmods = json.decode(vehdata.mods)
    DoScreenFadeOut(250)
    Wait(500)
    QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
...
RegisterNetEvent('poly-occasion:client:refreshVehicles', function()
    if Zone then
        QBCore.Functions.TriggerCallback('poly-occasions:server:getVehicles', function(vehicles)
            despawnOccasionsVehicles()
            spawnOccasionsVehicles(vehicles)
...
RegisterNetEvent('poly-vehiclesales:client:SellVehicle', function()
...
RegisterNetEvent('poly-vehiclesales:client:OpenContract', function(data)
    CurrentVehicle = occasionVehicles[Zone][data.Contract]
    if CurrentVehicle then
        QBCore.Functions.TriggerCallback('poly-occasions:server:getSellerInformation', function(info)
            if info then
...
RegisterNetEvent('poly-occasions:client:MainMenu', function()
    local MainMenu = {
        {
            isMenuHeader = true,
        ...
        },
        {
            ...
            params = {
                event = 'poly-vehiclesales:client:SellVehicle',
            }
        },
        {
            ...
            params = {
                event = 'poly-occasions:client:SellBackCar',
            }
        }
    }

    exports['poly-menu']:openMenu(MainMenu)
end)
    ...
    for k, cars in pairs(Config.Zones) do
        ...
        SpawnZone[k]:onPlayerInOut(function(isPointInside)
            if isPointInside and IsPedInAnyVehicle(PlayerPedId(), false) then
                exports['poly-core']:DrawText(Lang:t("menu.interaction"), 'left')
                TextShown = true
                Listen4Control()
            else
                listen = false
            ...
            for k2, v in pairs(Config.Zones[k].VehicleSpots) do
                ...
                VehicleZones:onPlayerInOut(function(isPointInside)
                    if isPointInside and IsCarSpawned(k2) then
                        exports['poly-core']:DrawText(Lang:t("menu.view_contract_int"), 'left')
                        TextShown = true
                        Listen4Control(k2)
                    else
                        listen = false

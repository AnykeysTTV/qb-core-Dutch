...
QBCore.Functions.CreateCallback('poly-occasions:server:getVehicles', function(_, cb)
...
QBCore.Functions.CreateCallback("poly-occasions:server:getSellerInformation", function(_, cb, citizenid)
...
QBCore.Functions.CreateCallback("poly-vehiclesales:server:CheckModelName", function(_, cb, plate)
    if plate then
...
RegisterNetEvent('poly-occasions:server:ReturnVehicle', function(vehicleData)
    local src = source
    ...
    if result[1] then
        if result[1].seller == Player.PlayerData.citizenid then
            ...
            TriggerClientEvent("poly-occasions:client:ReturnOwnedVehicle", src, result[1])
            TriggerClientEvent('poly-occasion:client:refreshVehicles', -1)
        else
...
RegisterNetEvent('poly-occasions:server:sellVehicle', function(vehiclePrice, vehicleData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerEvent("poly-log:server:CreateLog", "vehicleshop", "Vehicle for Sale", "red","**" .. GetPlayerName(src) .. "** has a " .. vehicleData.model .. " priced at " .. vehiclePrice)
    TriggerClientEvent('poly-occasion:client:refreshVehicles', -1)
end)

RegisterNetEvent('poly-occasions:server:sellVehicleBack', function(vehData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local price = 0
...
RegisterNetEvent('poly-occasions:server:buyVehicle', function(vehicleData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local result = MySQL.query.await('SELECT * FROM occasion_vehicles WHERE plate = ? AND occasionid = ?',{vehicleData['plate'], vehicleData["oid"]})
    if result[1] and next(result[1]) then
        if Player.PlayerData.money.bank >= result[1].price then
            ...
            TriggerEvent("poly-log:server:CreateLog", "vehicleshop", "bought", "green", "**" .. GetPlayerName(src) .. "** has bought for " .. result[1].price .. " (" .. result[1].plate ..") from **" .. SellerCitizenId .. "**")
            TriggerClientEvent("poly-occasions:client:BuyFinished", src, result[1])
            TriggerClientEvent('poly-occasion:client:refreshVehicles', -1)
            MySQL.query('DELETE FROM occasion_vehicles WHERE plate = ? AND occasionid = ?',{result[1].plate, result[1].occasionid})
            exports['poly-phone']:sendNewMailToOffline(SellerCitizenId, {

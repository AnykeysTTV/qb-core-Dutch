QBCore = exports['poly-core']:GetCoreObject() -- Used Globally
inJail = false
jailTime = 0
currentJob = nil
CellsBlip = nil
...
local function ApplyClothes()
	...
	if DoesEntityExist(playerPed) then
		Citizen.CreateThread(function()
			...
			if gender == 0 then
				TriggerEvent('poly-clothing:client:loadOutfit', Config.Uniforms.male)
			else
				TriggerEvent('poly-clothing:client:loadOutfit', Config.Uniforms.female)
			end
		end)
	end
end
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	...
	exports['poly-target']:AddTargetEntity(freedom_ped, {
		options = {
			{
				type = "client",
	...
	exports['poly-target']:AddTargetEntity(canteen_ped, {
		options = {
			{
				type = "client",
...
RegisterNetEvent('prison:client:Enter', function(time)
	...
	if invokingResource and invokingResource ~= 'poly-policejob' and invokingResource ~= 'poly-ambulancejob' and invokingResource ~= GetCurrentResourceName() then
	...
	if not Config.UseTarget then
		...
		freedom:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['poly-core']:DrawText('[E] Check Time', 'left')
			else
				exports['poly-core']:HideText()
			end
		end)
		...
		canteen:onPlayerInOut(function(isPointInside)
			...
			if isPointInside then
				...
				exports['poly-core']:DrawText('[E] Open Canteen', 'left')
			else
				exports['poly-core']:HideText()
			end
		end)

...
local function enterRobberyHouse(house)
    ...
    if Config.Houses[house]["tier"] == 1 then
        data = exports['poly-interior']:CreateHouseRobbery(coords)
    end
    Wait(100)
    houseObj = data[1]
    POIOffsets = data[2]
...
local function lockpickFinish(success)
    if success then
        TriggerServerEvent('poly-houserobbery:server:enterHouse', closestHouse)
        QBCore.Functions.Notify(Lang:t("success.worked"), "success", 2500)
    else
        if usingAdvanced then
            if math.random(1, 100) < 20 then
                TriggerServerEvent("poly-houserobbery:server:removeAdvancedLockpick")
                TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["advancedlockpick"], "remove")
            end
        else
            if math.random(1, 100) < 40 then
                TriggerServerEvent("poly-houserobbery:server:removeLockpick")
...
local function searchCabin(cabin)
    ...
    LockpickDoorAnim()
    TriggerServerEvent('poly-houserobbery:server:SetBusyState', cabin, currentHouse, true)
    ...
    Skillbar.Start({
        ...
        if SucceededAttempts + 1 >= NeededAttempts then
            openingDoor = false
            ClearPedTasks(PlayerPedId())
            TriggerServerEvent('poly-houserobbery:server:searchCabin', cabin, currentHouse)
            Config.Houses[currentHouse]["furniture"][cabin]["searched"] = true
            TriggerServerEvent('poly-houserobbery:server:SetBusyState', cabin, currentHouse, false)
            SucceededAttempts = 0
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.TriggerCallback('poly-houserobbery:server:GetHouseConfig', function(HouseConfig)
        Config.Houses = HouseConfig
    end)
end)

RegisterNetEvent('poly-houserobbery:client:ResetHouseState', function(house)
...
RegisterNetEvent('poly-houserobbery:client:enterHouse', function(house)
    enterRobberyHouse(house)
end)

RegisterNetEvent('poly-houserobbery:client:setHouseState', function(house, state)
    Config.Houses[house]["opened"] = state
end)

RegisterNetEvent('poly-houserobbery:client:setCabinState', function(house, cabin, state)
    Config.Houses[house]["furniture"][cabin]["searched"] = state
end)

RegisterNetEvent('poly-houserobbery:client:SetBusyState', function(cabin, house, bool)

...
local function ResetHouseStateTimer(house)
    ...
    SetTimeout(time, function()
        ...
        TriggerClientEvent('poly-houserobbery:client:ResetHouseState', -1, house)
    end)
end

-- Callbacks

QBCore.Functions.CreateCallback('poly-houserobbery:server:GetHouseConfig', function(_, cb)
    cb(Config.Houses)
end)

-- Events

RegisterNetEvent('poly-houserobbery:server:SetBusyState', function(cabin, house, bool)
    Config.Houses[house]["furniture"][cabin]["isBusy"] = bool
    TriggerClientEvent('poly-houserobbery:client:SetBusyState', -1, cabin, house, bool)
end)

RegisterNetEvent('poly-houserobbery:server:enterHouse', function(house)
    local src = source
    if not Config.Houses[house]["opened"] then
        ResetHouseStateTimer(house)
        TriggerClientEvent('poly-houserobbery:client:setHouseState', -1, house, true)
    end
    TriggerClientEvent('poly-houserobbery:client:enterHouse', src, house)
    Config.Houses[house]["opened"] = true
end)

RegisterNetEvent('poly-houserobbery:server:searchCabin', function(cabin, house)
    local src = source
    ...
    TriggerClientEvent('poly-houserobbery:client:setCabinState', -1, house, cabin, true)
end)

RegisterNetEvent('poly-houserobbery:server:removeAdvancedLockpick', function()
...
RegisterNetEvent('poly-houserobbery:server:removeLockpick', function()
    local Player = QBCore.Functions.GetPlayer(source)

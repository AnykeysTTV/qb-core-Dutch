...
function TakeOutImpound(vehicle)
    ...
    if coords then
        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
            ...
            QBCore.Functions.TriggerCallback('poly-garage:server:GetVehicleProperties', function(properties)
...
function MenuImpound(currentSelection)
    ...
    QBCore.Functions.TriggerCallback("police:GetImpoundedVehicles", function(result)
        ...
        if shouldContinue then
            ...
            exports['poly-menu']:openMenu(impoundMenu)
        end
    end)
...
RegisterNetEvent('police:client:EvidenceStashDrawer', function(data)
    ...
    if #(pos - takeLoc) <= 1.0 then
        local drawer = exports['poly-input']:ShowInput({
...
RegisterNetEvent('poly-policejob:ToggleDuty', function()
    TriggerServerEvent("QBCore:ToggleDuty")
    TriggerServerEvent("police:server:UpdateCurrentCops")
    TriggerServerEvent("police:server:UpdateBlips")
end)

RegisterNetEvent('poly-police:client:scanFingerPrint', function()
    local player, distance = GetClosestPlayer()
...
RegisterNetEvent('poly-police:client:openArmoury', function()
...
RegisterNetEvent('poly-police:client:spawnHelicopter', function(k)
    ...
    CreateThread(function()
        while true do
            ...
            if inFingerprint and PlayerJob.name == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("poly-police:client:scanFingerPrint")
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inArmoury and PlayerJob.name == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("poly-police:client:openArmoury")
                    break
                end
            else
                break
    ...
    CreateThread(function()
        while true do
            ...
            if inHelicopter and PlayerJob.name == "police" then
                if PlayerJob.onduty then sleep = 5 end
                if IsControlJustReleased(0, 38) then
                    TriggerEvent("poly-police:client:spawnHelicopter")
                    break
                end
            else
                break
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["duty"]) do
            exports['poly-target']:AddBoxZone("PoliceDuty_"..k, vector3(v.x, v.y, v.z), 1, 1, {
                name = "PoliceDuty_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "poly-policejob:ToggleDuty",
        ...
        for k, v in pairs(Config.Locations["stash"]) do
            exports['poly-target']:AddBoxZone("PoliceStash_"..k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = "PoliceStash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "poly-police:client:openStash",
                        icon = "fas fa-dungeon",
        ...
        for k, v in pairs(Config.Locations["trash"]) do
            exports['poly-target']:AddBoxZone("PoliceTrash_"..k, vector3(v.x, v.y, v.z), 1, 1.75, {
                name = "PoliceTrash_"..k,
                heading = 11,
                debugPoly = false,
                minZ = v.z - 1,
                ...
                options = {
                    {
                        type = "client",
                        event = "poly-police:client:openTrash",
                        icon = "fas fa-trash",
                        label = "Open Trash",
                        job = "police",
                    },
        ...
        for k, v in pairs(Config.Locations["fingerprint"]) do
            exports['poly-target']:AddBoxZone("PoliceFingerprint_"..k, vector3(v.x, v.y, v.z), 2, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "poly-police:client:scanFingerPrint",
        ...
        for k, v in pairs(Config.Locations["armory"]) do
            exports['poly-target']:AddBoxZone("PoliceArmory_"..k, vector3(v.x, v.y, v.z), 5, 1, {
                ...
                options = {
                    {
                        type = "client",
                        event = "poly-police:client:openArmoury",
                        icon = "fas fa-swords",
                        label = "Open Armory",
                        job = "police",
                    },
    ...
    dutyCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if not PlayerJob.onduty then
                exports['poly-core']:DrawText(Lang:t('info.on_duty'),'left')
                dutylistener()
            else
                exports['poly-core']:DrawText(Lang:t('info.off_duty'),'left')
                dutylistener()
            end
        else
            dutylisten = false
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['poly-core']:DrawText(Lang:t('info.stash_enter'), 'left')
                stash()
            end
        else
            inStash = false
    ...
    trashCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inTrash = true
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['poly-core']:DrawText(Lang:t('info.trash_enter'),'left')
                trash()
            end
        else
            inTrash = false
    ...
    fingerprintCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inFingerprint = true
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['poly-core']:DrawText(Lang:t('info.scan_fingerprint'),'left')
                fingerprint()
            end
        else
            inFingerprint = false
    ...
    armouryCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inArmoury = true
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                exports['poly-core']:DrawText(Lang:t('info.enter_armory'),'left')
                armoury()
            end
        else
            inArmoury = false
...
CreateThread(function()
    ...
    helicopterCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['poly-core']:HideText()
                    exports['poly-core']:DrawText(Lang:t('info.store_heli'), 'left')
                    heli()
                else
                    exports['poly-core']:DrawText(Lang:t('info.take_heli'), 'left')
                    heli()
                end
            end
        else
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['poly-core']:DrawText(Lang:t('info.impound_veh'), 'left')
                    impound()
                else
    ...
    garageCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['poly-core']:DrawText(Lang:t('info.store_veh'), 'left')
		    garage()
                else
                    local currentSelection = 0

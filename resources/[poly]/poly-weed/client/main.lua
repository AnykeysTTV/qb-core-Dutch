...
RegisterNetEvent('poly-weed:client:getHousePlants', function(house)
    QBCore.Functions.TriggerCallback('poly-weed:server:getBuildingPlants', function(plants)
        currentHouse = house
                            ...
                            if plantData["plantStage"] ~= plantData["plantStats"]["highestStage"] then
                                ...
                                if IsControlJustPressed(0, 38) then
                                    QBCore.Functions.Progressbar("remove_weed_plant", Lang:t('text.harvesting_plant'), 8000, false, true, {
                                        ...
                                        TriggerServerEvent('poly-weed:server:harvestPlant', currentHouse, amount, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                                    end, function() -- Cancel
                                        ClearPedTasks(ped)
                        ...
                        elseif plantData["plantStats"]["health"] == 0 then
                            ...
                            if IsControlJustPressed(0, 38) then
                                QBCore.Functions.Progressbar("remove_weed_plant", Lang:t('text.removing_the_plant'), 8000, false, true, {
                                    ...
                                    ClearPedTasks(ped)
                                    TriggerServerEvent('poly-weed:server:removeDeathPlant', currentHouse, plantData["plantStats"]["plantId"])
                                end, function() -- Cancel
                                    ClearPedTasks(ped)
...
RegisterNetEvent('poly-weed:client:leaveHouse', function()
    despawnHousePlants()
...
RegisterNetEvent('poly-weed:client:refreshHousePlants', function(house)
...
RegisterNetEvent('poly-weed:client:refreshPlantStats', function()
    if insideHouse then
        despawnHousePlants()
        SetTimeout(100, function()
            QBCore.Functions.TriggerCallback('poly-weed:server:getBuildingPlants', function(plants)
...
RegisterNetEvent('poly-weed:client:placePlant', function(type, item)
    local ped = PlayerPedId()
	...
	LocalPlayer.state:set("inv_busy", true, true)
		...
		LocalPlayer.state:set("inv_busy", false, true)
                ...
                ClearPedTasks(ped)
                TriggerServerEvent('poly-weed:server:placePlant', json.encode(plantData["plantCoords"]), type, currentHouse)
                TriggerServerEvent('poly-weed:server:removeSeed', item.slot, type)
            end, function() -- Cancel
                ClearPedTasks(ped)
                QBCore.Functions.Notify(Lang:t('error.process_canceled'), "error")
		LocalPlayer.state:set("inv_busy", false, true)
...
RegisterNetEvent('poly-weed:client:foodPlant', function()
    if currentHouse ~= nil then
            ...
            if plyDistance < 1.0 then
                if plantData["plantStats"]["food"] == 100 then
                    ...
                    QBCore.Functions.Progressbar("plant_weed_plant", Lang:t('text.feeding_plant'), math.random(4000, 8000), false, true, {
                        ...
                        TriggerServerEvent('poly-weed:server:foodPlant', currentHouse, newFood, plantData["plantSort"]["name"], plantData["plantStats"]["plantId"])
                    end, function() -- Cancel
                        ClearPedTasks(ped)
			            LocalPlayer.state:set("inv_busy", false, true)
                        QBCore.Functions.Notify(Lang:t('error.process_canceled'), "error")

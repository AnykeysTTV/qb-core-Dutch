...
QBCore.Functions.CreateCallback('poly-weed:server:getBuildingPlants', function(_, cb, building)
...
RegisterNetEvent('poly-weed:server:placePlant', function(coords, sort, currentHouse)
    ...
    TriggerClientEvent('poly-weed:client:refreshHousePlants', -1, currentHouse)
end)

RegisterNetEvent('poly-weed:server:removeDeathPlant', function(building, plantId)
    MySQL.query('DELETE FROM house_plants WHERE plantid = ? AND building = ?', {plantId, building})
    TriggerClientEvent('poly-weed:client:refreshHousePlants', -1, building)
end)
...
CreateThread(function()
    while true do
        ...
        TriggerClientEvent('poly-weed:client:refreshPlantStats', -1)
        Wait((60 * 1000) * 19.2)
    end
end)
...
QBCore.Functions.CreateUseableItem("weed_white-widow_seed", function(source, item)
    TriggerClientEvent('poly-weed:client:placePlant', source, 'white-widow', item)
end)

QBCore.Functions.CreateUseableItem("weed_skunk_seed", function(source, item)
    TriggerClientEvent('poly-weed:client:placePlant', source, 'skunk', item)
end)

QBCore.Functions.CreateUseableItem("weed_purple-haze_seed", function(source, item)
    TriggerClientEvent('poly-weed:client:placePlant', source, 'purple-haze', item)
end)

QBCore.Functions.CreateUseableItem("weed_og-kush_seed", function(source, item)
    TriggerClientEvent('poly-weed:client:placePlant', source, 'og-kush', item)
end)

QBCore.Functions.CreateUseableItem("weed_amnesia_seed", function(source, item)
    TriggerClientEvent('poly-weed:client:placePlant', source, 'amnesia', item)
end)

QBCore.Functions.CreateUseableItem("weed_ak47_seed", function(source, item)
    TriggerClientEvent('poly-weed:client:placePlant', source, 'ak47', item)
end)

QBCore.Functions.CreateUseableItem("weed_nutrition", function(source, item)
    TriggerClientEvent('poly-weed:client:foodPlant', source, item)
end)

RegisterServerEvent('poly-weed:server:removeSeed', function(itemslot, seed)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.RemoveItem(seed, 1, itemslot)
end)

RegisterNetEvent('poly-weed:server:harvestPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
        ...
        if weedBag.amount >= sndAmount then
            if house ~= nil then
                ...
                if result[1] ~= nil then
                    ...
                    TriggerClientEvent('poly-weed:client:refreshHousePlants', -1, house)
                else
...
RegisterNetEvent('poly-weed:server:foodPlant', function(house, amount, plantName, plantId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

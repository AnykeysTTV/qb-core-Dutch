            ...
            if LocalPlayer.state.isLoggedIn then
                ...
                if entering ~= 0 and not isBlacklistedVehicle(entering) then
                    ...
                    if driver ~= 0 and not IsPedAPlayer(driver) and not HasKeys(plate) and not carIsImmune then
                        if IsEntityDead(driver) then
                            if not isTakingKeys then
                                ...
                                TriggerServerEvent('poly-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                                QBCore.Functions.Progressbar("steal_keys", Lang:t("progress.takekeys"), 2500, false, false, {
                                    ...
                                    TriggerServerEvent('poly-vehiclekeys:server:AcquireVehicleKeys', plate)
                                    isTakingKeys = false
                                end, function()
                                    isTakingKeys = false
                                end)
                        ...
                        elseif Config.LockNPCDrivingCars then
                            TriggerServerEvent('poly-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 2)
                        else
                            TriggerServerEvent('poly-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                            TriggerServerEvent('poly-vehiclekeys:server:AcquireVehicleKeys', plate)
                    ...
                    elseif driver == 0 and entering ~= lastPickedVehicle and not HasKeys(plate) and not isTakingKeys then
                        QBCore.Functions.TriggerCallback('poly-vehiclekeys:server:checkPlayerOwned', function(playerOwned)
...
RegisterNetEvent('poly-vehiclekeys:client:AddKeys', function(plate)
    KeysList[plate] = true
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        local vehicle = GetVehiclePedIsIn(ped)
...
RegisterNetEvent('poly-vehiclekeys:client:RemoveKeys', function(plate)
    KeysList[plate] = nil
end)
RegisterNetEvent('poly-vehiclekeys:client:ToggleEngine', function()
...
RegisterNetEvent('poly-vehiclekeys:client:GiveKeys', function(id)
            ...
            if id and type(id) == "number" then -- Give keys to specific ID
                ...
                if IsPedSittingInVehicle(PlayerPedId(), targetVehicle) then -- Give keys to everyone in vehicle
                    ...
                    for p=1,#otherOccupants do
                        TriggerServerEvent('poly-vehiclekeys:server:GiveVehicleKeys', GetPlayerServerId(NetworkGetPlayerIndexFromPed(otherOccupants[p])), targetPlate)
                    end
        ...
        if not isBlacklistedVehicle(veh) then
            if HasKeys(QBCore.Functions.GetPlate(veh)) or AreKeysJobShared(veh) then
                ...
                if vehLockStatus == 1 then
                    TriggerServerEvent('poly-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 2)
                    QBCore.Functions.Notify(Lang:t("notify.vlock"), "primary")
                else
                    TriggerServerEvent('poly-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 1)
...
function GiveKeys(id, plate)
    ...
    if distance < 1.5 and distance > 0.0 then
        TriggerServerEvent('poly-vehiclekeys:server:GiveVehicleKeys', id, plate)
    else
        QBCore.Functions.Notify(Lang:t("notify.nonear"),'error')
    end
end
function GetKeys()
    QBCore.Functions.TriggerCallback('poly-vehiclekeys:server:GetVehicleKeys', function(keysList)
        KeysList = keysList
    end)
end
            ...
            for _, vehicle in pairs(v.vehicles) do
                if string.upper(vehicle) == string.upper(vehName) then
                    if not HasKeys(vehPlate) then
                        TriggerServerEvent("poly-vehiclekeys:server:AcquireVehicleKeys", vehPlate)
                    end
                    return true
                end
            end
...
function LockpickFinishCallback(success)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
            TriggerServerEvent('poly-vehiclekeys:server:AcquireVehicleKeys', QBCore.Functions.GetPlate(vehicle))
        else
            QBCore.Functions.Notify(Lang:t("notify.vlockpick"), 'success')
            TriggerServerEvent('poly-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(vehicle), 1)
        end
    ...
    if usingAdvanced then
        if chance <= Config.RemoveLockpickAdvanced then
            TriggerServerEvent("poly-vehiclekeys:server:breakLockpick", "advancedlockpick")
        end
    else
        if chance <= Config.RemoveLockpickNormal then
            TriggerServerEvent("poly-vehiclekeys:server:breakLockpick", "lockpick")
        end
    end
end

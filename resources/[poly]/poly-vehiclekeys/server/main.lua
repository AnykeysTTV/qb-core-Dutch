...
RegisterNetEvent('poly-vehiclekeys:server:GiveVehicleKeys', function(receiver, plate)
    local giver = source
...
RegisterNetEvent('poly-vehiclekeys:server:AcquireVehicleKeys', function(plate)
    local src = source
    GiveKeys(src, plate)
end)

RegisterNetEvent('poly-vehiclekeys:server:breakLockpick', function(itemName)
...
RegisterNetEvent('poly-vehiclekeys:server:setVehLockState', function(vehNetId, state)
    SetVehicleDoorsLocked(NetworkGetEntityFromNetworkId(vehNetId), state)
end)

QBCore.Functions.CreateCallback('poly-vehiclekeys:server:GetVehicleKeys', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
...
QBCore.Functions.CreateCallback('poly-vehiclekeys:server:checkPlayerOwned', function(_, cb, plate)
...
function GiveKeys(id, plate)
    ...
    TriggerClientEvent('poly-vehiclekeys:client:AddKeys', id, plate)
end
...
function RemoveKeys(id, plate)
    ...
    TriggerClientEvent('poly-vehiclekeys:client:RemoveKeys', id, plate)
end
...
QBCore.Commands.Add("givekeys", Lang:t("addcom.givekeys"), {{name = Lang:t("addcom.givekeys_id"), help = Lang:t("addcom.givekeys_id_help")}}, false, function(source, args)
	local src = source
    TriggerClientEvent('poly-vehiclekeys:client:GiveKeys', src, tonumber(args[1]))
end)

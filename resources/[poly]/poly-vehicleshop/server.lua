...
RegisterNetEvent('poly-vehicleshop:server:addPlayer', function(citizenid)
    financetimer[citizenid] = os.time()
end)

-- Deduct stored game time from player on logout
RegisterNetEvent('poly-vehicleshop:server:removePlayer', function(citizenid)
...
-- Callbacks
QBCore.Functions.CreateCallback('poly-vehicleshop:server:getVehicles', function(source, cb)
    local src = source
...
RegisterNetEvent('poly-vehicleshop:server:deleteVehicle', function (netId)
    local vehicle = NetworkGetEntityFromNetworkId(netId)
    DeleteEntity(vehicle)
end)

-- Sync vehicle for other players
RegisterNetEvent('poly-vehicleshop:server:swapVehicle', function(data)
    local src = source
    TriggerClientEvent('poly-vehicleshop:client:swapVehicle', -1, data)
    Wait(1500)-- let new car spawn
    TriggerClientEvent('poly-vehicleshop:client:homeMenu', src)-- reopen main menu
end)

-- Send customer for test drive
RegisterNetEvent('poly-vehicleshop:server:customTestDrive', function(vehicle, playerid)
    local src = source
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target))) < 3 then
        TriggerClientEvent('poly-vehicleshop:client:customTestDrive', target, vehicle)
    else
...
RegisterNetEvent('poly-vehicleshop:server:financePayment', function(paymentAmount, vehData)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-vehicleshop:server:financePaymentFull', function(data)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local cash = player.PlayerData.money['cash']
    local bank = player.PlayerData.money['bank']
...
RegisterNetEvent('poly-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    local src = source
    ...
    if cash > tonumber(vehiclePrice) then
        ...
        TriggerClientEvent('poly-vehicleshop:client:buyShowroomVehicle', src, vehicle, plate)
...
RegisterNetEvent('poly-vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
    local src = source
...
RegisterNetEvent('poly-vehicleshop:server:sellShowroomVehicle', function(data, playerid)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    ...
    if #(GetEntityCoords(GetPlayerPed(src)) - GetEntityCoords(GetPlayerPed(target.PlayerData.source))) < 3 then
        ...
        if cash >= tonumber(vehiclePrice) then
            ...
            TriggerClientEvent('poly-vehicleshop:client:buyShowroomVehicle', target.PlayerData.source, vehicle, plate)
            ...
            exports['poly-management']:AddMoney(player.PlayerData.job.name, vehiclePrice)
...
RegisterNetEvent('poly-vehicleshop:server:sellfinanceVehicle', function(downPayment, paymentAmount, vehicle, playerid)
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local target = QBCore.Functions.GetPlayer(tonumber(playerid))
...
RegisterNetEvent('poly-vehicleshop:server:checkFinance', function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)

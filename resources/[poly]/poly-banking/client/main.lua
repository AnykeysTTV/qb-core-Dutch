...
local function openAccountScreen()
    QBCore.Functions.TriggerCallback('poly-banking:getBankingInformation', function(banking)
        if banking ~= nil then
...
RegisterNetEvent('poly-banking:transferError', function(msg)
    SendNUIMessage({
...
RegisterNetEvent('poly-banking:successAlert', function(msg)
    SendNUIMessage({
...
RegisterNetEvent('poly-banking:openBankScreen', function()
    openAccountScreen()
end)
    ...
    CreateThread(function()
        ...
        while BankControlPress do
            if IsControlPressed(0, 38) then
                ...
                TriggerEvent('poly-banking:openBankScreen')
            end
            Wait(0)
        end
    end)
...
CreateThread(function()
    if Config.UseTarget then
        for k, v in pairs(Config.Zones) do
            exports["poly-target"]:AddBoxZone("Bank_"..k, v.position, v.length, v.width, {
                name = "Bank_"..k,
                heading = v.heading,
                minZ = v.minZ,
                maxZ = v.maxZ
                ...
                options = {
                    {
                        type = "client",
                        event = "poly-banking:openBankScreen",
        ...
        for k, v in pairs(Config.BankLocations) do
            ...
            bankCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    exports['poly-core']:DrawText(Lang:t('info.access_bank_key'),'left')
                    BankControl()
                else
...
RegisterNetEvent('poly-banking:client:newCardSuccess', function(cardno, ctype)
    SendNUIMessage({
        status = "updateCard",
        number = cardno,
        cardtype = ctype
...
RegisterNUICallback("createSavingsAccount", function(_, cb)
    TriggerServerEvent('poly-banking:createSavingsAccount')
    cb("ok")
end)

RegisterNUICallback("doDeposit", function(data, cb)
    if tonumber(data.amount) ~= nil and tonumber(data.amount) > 0 then
        TriggerServerEvent('poly-banking:doQuickDeposit', data.amount)
        openAccountScreen()
        cb("ok")
    end
    cb(nil)
...
RegisterNUICallback("doWithdraw", function(data, cb)
    if tonumber(data.amount) ~= nil and tonumber(data.amount) > 0 then
        TriggerServerEvent('poly-banking:doQuickWithdraw', data.amount, true)
        openAccountScreen()
        cb("ok")
    end
    cb(nil)
...
RegisterNUICallback("doATMWithdraw", function(data, cb)
    if tonumber(data.amount) ~= nil and tonumber(data.amount) > 0 then
        TriggerServerEvent('poly-banking:doQuickWithdraw', data.amount, false)
        openAccountScreen()
        cb("ok")
    end
    cb(nil)
...
RegisterNUICallback("savingsDeposit", function(data, cb)
    if tonumber(data.amount) ~= nil and tonumber(data.amount) > 0 then
        TriggerServerEvent('poly-banking:savingsDeposit', data.amount)
        openAccountScreen()
        cb("ok")
    end
    cb(nil)
...
RegisterNUICallback("savingsWithdraw", function(data, cb)
    if tonumber(data.amount) ~= nil and tonumber(data.amount) > 0 then
        TriggerServerEvent('poly-banking:savingsWithdraw', data.amount)
        openAccountScreen()
        cb("ok")
    end
    cb(nil)
...
RegisterNUICallback("doTransfer", function(data, cb)
    if data ~= nil then
        TriggerServerEvent('poly-banking:initiateTransfer', data)
        cb("ok")
    end
    cb(nil)
end)
...
RegisterNUICallback("createDebitCard", function(data, cb)
    if data.pin ~= nil then
        TriggerServerEvent('poly-banking:createBankCard', data.pin)
        cb("ok")
    end
    cb(nil)
end)
...
RegisterNUICallback("lockCard", function(_, cb)
    TriggerServerEvent('poly-banking:toggleCard', true)
    cb("ok")
end)

RegisterNUICallback("unLockCard", function(_, cb)
    TriggerServerEvent('poly-banking:toggleCard', false)
    cb("ok")
end)

RegisterNUICallback("updatePin", function(data, cb)
    if data.pin and data.currentBankCard then
        TriggerServerEvent('poly-banking:updatePin', data.currentBankCard, data.pin)
        cb("ok")
    end
    cb(nil)
end)

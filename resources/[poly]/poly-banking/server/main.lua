...
--[[ -- Only used by the following "poly-banking:initiateTransfer"
...
RegisterNetEvent('poly-banking:initiateTransfer', function(_)
    --[[
    local _src = source
    ...
    if (checkAccount) then
        ...
        if receiptName ~= false or receiptName ~= nil then
            local userOnline = exports.poly-base:checkOnline(cid)

            if userOnline ~= false then
                -- User is online so we can do a straght transfer
                local _targetUser = exports.poly-base:Source(userOnline)
                if acType == "Current" then
                    ...
                    TriggerClientEvent('poly-banking:openBankScreen', _src)
                    TriggerClientEvent('poly-banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('poly-banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
QBCore.Functions.CreateCallback('poly-banking:getBankingInformation', function(source, cb)
...
RegisterNetEvent('poly-banking:createBankCard', function(pin)
    local src = source
    ...
    TriggerClientEvent('poly-banking:openBankScreen', src)
    TriggerClientEvent('poly-banking:successAlert', src, Lang:t('success.debit_card'))

    TriggerEvent('poly-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** successfully ordered a debit card")
end)

RegisterNetEvent('poly-banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('poly-banking:openBankScreen', src)
            TriggerClientEvent('poly-banking:successAlert', src, Lang:t('success.cash_deposit', {value = amount}))
            TriggerEvent('poly-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash deposit of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('poly-banking:toggleCard', function(toggle)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-banking:doQuickWithdraw', function(amount, _)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then
            TriggerClientEvent('poly-banking:openBankScreen', src)
            TriggerClientEvent('poly-banking:successAlert', src, Lang:t('success.cash_withdrawal', {value = amount}))
            TriggerEvent('poly-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash withdrawal of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('poly-banking:updatePin', function(currentBankCard, newPin)
    if newPin ~= nil then
        local src = source
        local xPlayer = QBCore.Functions.GetPlayer(src)
        while xPlayer == nil do Wait(0) end
        ...
        MySQL.update('UPDATE bank_cards SET cardPin = ? WHERE record_id = ?', {
            ...
            if result == 1 then
                TriggerClientEvent('poly-banking:openBankScreen', src)
                TriggerClientEvent('poly-banking:successAlert', src, Lang:t('success.updated_pin'))
            else
                TriggerClientEvent('QBCore:Notify', src, 'Fout bij het bijwerken van de pincode.', "error")
            end
        end)
    end
end)

RegisterNetEvent('poly-banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('poly-banking:openBankScreen', src)
        TriggerClientEvent('poly-banking:successAlert', src, Lang:t('success.savings_deposit', {value = tostring(amount)}))
        TriggerEvent('poly-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings deposit of $"..tostring(amount).." successfully..")
    end
end)

RegisterNetEvent('poly-banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentSavings then
        ...
        while savings == nil do Wait(0) end
        TriggerClientEvent('poly-banking:openBankScreen', src)
        TriggerClientEvent('poly-banking:successAlert', src, Lang:t('success.savings_withdrawal', {value = tostring(amount)}))
        TriggerEvent('poly-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings withdrawal of $"..tostring(amount).." successfully.")
    end
end)

RegisterNetEvent('poly-banking:createSavingsAccount', function()
    local src = source
    local xPlayer = QBCore.Functions.GetPlayer(src)
    local success = createSavingsAccount(xPlayer.PlayerData.citizenid)
    repeat Wait(0) until success ~= nil
    TriggerClientEvent('poly-banking:openBankScreen', src)
    TriggerClientEvent('poly-banking:successAlert', src, Lang:t('success.opened_savings'))
    TriggerEvent('poly-log:server:CreateLog', 'banking', 'Banking', "lightgreen", "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** opened a savings account")
end)

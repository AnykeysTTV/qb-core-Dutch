        ...
        if not self.cardActive then
            ...
            MySQL.update('UPDATE bank_cards SET cardnumber = ?, cardPin = ?, cardDecrypted = ?, cardActive = ?, cardLocked = ?, cardType = ? WHERE citizenid = ? AND record_id = ?', {
                ...
                if self.source ~= -1 then
                    TriggerClientEvent('poly-banking:client:newCardSuccess', self.source, cardNumber, friendlyName)
...
RegisterNetEvent('poly-banking:server:registerCurrentAccount', function(cid)
...
RegisterNetEvent('poly-banking:server:registerSavingsAccount', function(cid)

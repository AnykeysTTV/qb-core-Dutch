...
RegisterNetEvent('poly-trucker:server:DoBail', function(bool, vehInfo)
    local src = source
    ...
    if bool then
        if Player.PlayerData.money.cash >= Config.TruckerJobTruckDeposit then
            ...
            TriggerClientEvent('poly-trucker:client:SpawnVehicle', src, vehInfo)
...
RegisterNetEvent('poly-trucker:server:01101110', function(drops)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('poly-trucker:server:nano', function()

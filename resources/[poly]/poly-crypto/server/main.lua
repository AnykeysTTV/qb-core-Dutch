...
local function RefreshCrypto()
    ...
    if result ~= nil and result[1] ~= nil then
        ...
        if result[1].history ~= nil then
            ...
            TriggerClientEvent('poly-crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, json.decode(result[1].history))
        else
            TriggerClientEvent('poly-crypto:client:UpdateCryptoWorth', -1, coin, result[1].worth, nil)
        end
    end
end
    ...
    if crypto ~= nil then
        if Crypto.Worth[crypto] ~= nil then
            ...
            if NewWorth ~= nil then
                ...
                TriggerClientEvent('poly-crypto:client:UpdateCryptoWorth', -1, crypto, NewWorth)
...
RegisterServerEvent('poly-crypto:server:FetchWorth', function()
    for name,_ in pairs(Crypto.Worth) do
        ...
        if result[1] ~= nil then
            ...
            if result[1].history ~= nil then
                ...
                TriggerClientEvent('poly-crypto:client:UpdateCryptoWorth', -1, name, result[1].worth, json.decode(result[1].history))
            else
                TriggerClientEvent('poly-crypto:client:UpdateCryptoWorth', -1, name, result[1].worth, nil)
            end
        end
    end
end)

RegisterServerEvent('poly-crypto:server:ExchangeFail', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('poly-crypto:server:Rebooting', function(state, percentage)
    Crypto.Exchange.RebootInfo.state = state
    Crypto.Exchange.RebootInfo.percentage = percentage
end)

RegisterServerEvent('poly-crypto:server:GetRebootState', function()
    local src = source
    TriggerClientEvent('poly-crypto:client:GetRebootState', src, Crypto.Exchange.RebootInfo)
end)

RegisterServerEvent('poly-crypto:server:SyncReboot', function()
    TriggerClientEvent('poly-crypto:client:SyncReboot', -1)
end)

RegisterServerEvent('poly-crypto:server:ExchangeSuccess', function(LuckChance)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local ItemData = Player.Functions.GetItemByName("cryptostick")
    ...
    if ItemData ~= nil then
        ...
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items["cryptostick"], "remove")
        TriggerClientEvent('poly-phone:client:AddTransaction', src, Player, {},  Lang:t('credit.there_are_amount_credited',{amount = Amount}), "Credit")
    end
end)

-- Callbacks

QBCore.Functions.CreateCallback('poly-crypto:server:HasSticky', function(source, cb)
...
QBCore.Functions.CreateCallback('poly-crypto:server:GetCryptoData', function(source, cb, name)
    local Player = QBCore.Functions.GetPlayer(source)
    local CryptoData = {
...
QBCore.Functions.CreateCallback('poly-crypto:server:BuyCrypto', function(source, cb, data)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.money.bank >= total_price then
        ...
        TriggerClientEvent('poly-phone:client:AddTransaction', source, Player, data,  Lang:t('credit.you_have_qbit_purchased',{dataCoins = tonumber(data.Coins)}), "Credit")
...
QBCore.Functions.CreateCallback('poly-crypto:server:SellCrypto', function(source, cb, data)
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        TriggerClientEvent('poly-phone:client:AddTransaction', source, Player, data, Lang:t('depreciation.you_have_sold',{dataCoins = tonumber(data.Coins)}), "Depreciation")
...
QBCore.Functions.CreateCallback('poly-crypto:server:TransferCrypto', function(source, cb, data)
    ...
    if Player.PlayerData.money.crypto >= tonumber(data.Coins) then
        ...
        if result[1] ~= nil then
            ...
            Player.Functions.RemoveMoney('crypto', tonumber(data.Coins))
            TriggerClientEvent('poly-phone:client:AddTransaction', source, Player, data, "You have "..tonumber(data.Coins).." Qbit('s) transferred!", "Depreciation")
            ...
            if Target ~= nil then
                ...
                TriggerClientEvent('poly-phone:client:AddTransaction', Target.PlayerData.source, Player, data, "There are "..tonumber(data.Coins).." Qbit('s) credited!", "Credit")
            else

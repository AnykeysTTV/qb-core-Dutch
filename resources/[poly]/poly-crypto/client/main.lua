...
local function ExchangeSuccess()
	TriggerServerEvent('poly-crypto:server:ExchangeSuccess', math.random(1, 10))
end
...
local function ExchangeFail()
	...
	if RemoveChance == LosingNumber then
		TriggerServerEvent('poly-crypto:server:ExchangeFail')
		TriggerServerEvent('poly-crypto:server:SyncReboot')
	end
end
	...
	CreateThread(function()
		while Crypto.Exchange.RebootInfo.state do
			if (Crypto.Exchange.RebootInfo.percentage + 1) <= 100 then
				...
				TriggerServerEvent('poly-crypto:server:Rebooting', true, Crypto.Exchange.RebootInfo.percentage)
			else
				Crypto.Exchange.RebootInfo.percentage = 0
				Crypto.Exchange.RebootInfo.state = false
				TriggerServerEvent('poly-crypto:server:Rebooting', false, 0)
			end
			Wait(1200)
		end
	end)
				...
				if dist < 1.5 then
					if not Crypto.Exchange.RebootInfo.state then
						...
						if IsControlJustPressed(0, 38) then
							QBCore.Functions.TriggerCallback('poly-crypto:server:HasSticky', function(HasItem)
								if HasItem then
...
RegisterNetEvent('poly-crypto:client:SyncReboot', function()
	Crypto.Exchange.RebootInfo.state = true
	SystemCrashCooldown()
end)

RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
	TriggerServerEvent('poly-crypto:server:FetchWorth')
	TriggerServerEvent('poly-crypto:server:GetRebootState')
end)

RegisterNetEvent('poly-crypto:client:UpdateCryptoWorth', function(crypto, amount, history)
...
RegisterNetEvent('poly-crypto:client:GetRebootState', function(RebootInfo)

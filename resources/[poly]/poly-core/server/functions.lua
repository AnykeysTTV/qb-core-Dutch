            ...
            if Player then
                ...
                if Player.PlayerData.job and payment > 0 and (QBShared.Jobs[Player.PlayerData.job.name].offDutyPay or Player.PlayerData.job.onduty) then
                    if QBCore.Config.Money.PayCheckSociety then
                        local account = exports['poly-management']:GetAccount(Player.PlayerData.job.name)
                        if account ~= 0 then -- Checks if player is employed by a society
                            if account < payment then -- Checks if company has enough money to pay society
                                TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, Lang:t('error.company_too_poor'), 'error')
                            else
                                Player.Functions.AddMoney('bank', payment, 'paycheck')
                                exports['poly-management']:RemoveMoney(Player.PlayerData.job.name, payment)
...
function QBCore.Functions.UseItem(source, item)
    ...
    exports['poly-inventory']:UseItem(source, item)
end
...
function QBCore.Functions.HasItem(source, items, amount)
    if GetResourceState('poly-inventory') == 'missing' then return end
    return exports['poly-inventory']:HasItem(source, items, amount)
end
...
function QBCore.Functions.PrepForSQL(source, data, pattern)
    ...
    if not result or string.len(result) ~= string.len(data)  then
        TriggerEvent('poly-log:server:CreateLog', 'anticheat', 'SQL Exploit Attempted', 'red', string.format('%s attempted to exploit SQL!', player.PlayerData.license))
        return false
    end
    return true
end

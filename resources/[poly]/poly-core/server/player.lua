    ...
    if source and source ~= '' then
        if citizenid then
            ...
            if PlayerData and license == PlayerData.license then
                ...
                TriggerEvent('poly-log:server:CreateLog', 'anticheat', 'Anti-Cheat', 'white', GetPlayerName(source) .. ' Has Been Dropped For Character Joining Exploit', false)
            end
        else
...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.items = GetResourceState('poly-inventory') ~= 'missing' and exports['poly-inventory']:LoadInventory(PlayerData.source, PlayerData.citizenid) or {}
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    function self.Functions.AddMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            ...
            if amount > 100000 then
                TriggerEvent('poly-log:server:CreateLog', 'playermoney', 'AddMoney', 'lightgreen', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') added, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason, true)
            else
                TriggerEvent('poly-log:server:CreateLog', 'playermoney', 'AddMoney', 'lightgreen', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') added, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason)
            end
    ...
    function self.Functions.RemoveMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            self.Functions.UpdatePlayerData()
            if amount > 100000 then
                TriggerEvent('poly-log:server:CreateLog', 'playermoney', 'RemoveMoney', 'red', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') removed, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason, true)
            else
                TriggerEvent('poly-log:server:CreateLog', 'playermoney', 'RemoveMoney', 'red', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') removed, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason)
            end
            TriggerClientEvent('hud:client:OnMoneyChange', self.PlayerData.source, moneytype, amount, true)
            if moneytype == 'bank' then
                TriggerClientEvent('poly-phone:client:RemoveBankMoney', self.PlayerData.source, amount)
            end
    ...
    function self.Functions.SetMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            self.Functions.UpdatePlayerData()
            TriggerEvent('poly-log:server:CreateLog', 'playermoney', 'SetMoney', 'green', '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid .. ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype .. ') set, new ' .. moneytype .. ' balance: ' .. self.PlayerData.money[moneytype] .. ' reason: ' .. reason)
    ...
    function self.Functions.GetCardSlot(cardNumber, cardType)
        ...
        local slots = exports['poly-inventory']:GetSlotsByItem(self.PlayerData.items, item)
...
function QBCore.Player.Save(source)
    ...
    if PlayerData then
        ...
        if GetResourceState('poly-inventory') ~= 'missing' then exports['poly-inventory']:SaveInventory(source) end
...
function QBCore.Player.SaveOffline(PlayerData)
    if PlayerData then
        ...
        if GetResourceState('poly-inventory') ~= 'missing' then exports['poly-inventory']:SaveInventory(PlayerData, true) end
...
function QBCore.Player.DeleteCharacter(source, citizenid)
    ...
    if license == result then
        ...
        MySQL.transaction(queries, function(result2)
            if result2 then
                TriggerEvent('poly-log:server:CreateLog', 'joinleave', 'Character Deleted', 'red', '**' .. GetPlayerName(source) .. '** ' .. license .. ' deleted **' .. citizenid .. '**..')
            end
        end)
    else
        DropPlayer(source, Lang:t("info.exploit_dropped"))
        TriggerEvent('poly-log:server:CreateLog', 'anticheat', 'Anti-Cheat', 'white', GetPlayerName(source) .. ' Has Been Dropped For Character Deletion Exploit', true)
    end
end
...
function QBCore.Player.ForceDeleteCharacter(citizenid)
    ...
    if result then
        ...
        MySQL.transaction(queries, function(result2)
            if result2 then
                TriggerEvent('poly-log:server:CreateLog', 'joinleave', 'Character Force Deleted', 'red', 'Character **' .. citizenid .. '** got deleted')
            end
        end)
    end
end
...
function QBCore.Player.SaveInventory(source)
    if GetResourceState('poly-inventory') == 'missing' then return end
    exports['poly-inventory']:SaveInventory(source, false)
end

function QBCore.Player.SaveOfflineInventory(PlayerData)
    if GetResourceState('poly-inventory') == 'missing' then return end
    exports['poly-inventory']:SaveInventory(PlayerData, true)
end

function QBCore.Player.GetTotalWeight(items)
    if GetResourceState('poly-inventory') == 'missing' then return end
    return exports['poly-inventory']:GetTotalWeight(items)
end

function QBCore.Player.GetSlotsByItem(items, itemName)
    if GetResourceState('poly-inventory') == 'missing' then return end
    return exports['poly-inventory']:GetSlotsByItem(items, itemName)
end

function QBCore.Player.GetFirstSlotByItem(items, itemName)
    if GetResourceState('poly-inventory') == 'missing' then return end
    return exports['poly-inventory']:GetFirstSlotByItem(items, itemName)
end
...
function QBCore.Player.CreateWalletId()
    ...
    local WalletId = nil
    while not UniqueFound do
        WalletId = 'poly-' .. math.random(11111111, 99999999)

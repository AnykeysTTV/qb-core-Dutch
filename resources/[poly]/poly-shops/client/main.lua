...
local function openShop(shop, data)
    QBCore.Functions.TriggerCallback('poly-shops:server:SetShopInv', function(shopInvJson)
        ...
        while listen do
            if IsControlJustPressed(0, 38) then -- E
                ...
                if inChips then
                    ...
                    TriggerServerEvent("poly-shops:server:sellChips")
                else
                    exports["poly-core"]:KeyPressed()
...
local function createPeds()
    ...
    for k, v in pairs(Config.Locations) do
        ...
        if Config.UseTarget then
            exports['poly-target']:AddTargetEntity(ShopPed[k], {
                options = {
                    {
                        label = v["targetLabel"],
                        icon = v["targetIcon"],
    ...
    if Config.UseTarget then
        exports['poly-target']:AddTargetEntity(ShopPed["casino"], {
            options = {
                {
                    label = 'Sell Chips',
...
-- Events
RegisterNetEvent("poly-shops:client:UpdateShop", function(shop, itemData, amount)
    TriggerServerEvent("poly-shops:server:UpdateShopItems", shop, itemData, amount)
end)

RegisterNetEvent("poly-shops:client:SetShopItems", function(shop, shopProducts)
    Config.Locations[shop]["products"] = shopProducts
end)

RegisterNetEvent("poly-shops:client:RestockShopItems", function(shop, amount)
    ...
    CreateThread(function()
        ...
        combo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                exports["poly-core"]:DrawText(Lang:t("info.open_shop"))
                listenForControl()
            else
        ...
        sellChips:onPlayerInOut(function(isPointInside)
            if isPointInside then
                inChips = true
                exports["poly-core"]:DrawText(Lang:t("info.sell_chips"))
                listenForControl()
            else
                inChips = false
                exports["poly-core"]:HideText()

...
CreateThread(function()
    ...
    TriggerClientEvent("poly-garages:client:houseGarageConfig", -1, HouseGarages)
    TriggerClientEvent("poly-houses:client:setHouseConfig", -1, Config.Houses)
end)
...
QBCore.Commands.Add("decorate", Lang:t("info.decorate_interior"), {}, false, function(source)
    local src = source
    TriggerClientEvent("poly-houses:client:decorate", src)
end)
...
QBCore.Commands.Add("createhouse", Lang:t("info.create_house"), {{name = "price", help = Lang:t("info.price_of_house")}, {name = "tier", help = Lang:t("info.tier_number")}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "realestate" then
        TriggerClientEvent("poly-houses:client:createHouses", src, price, tier)
    else
...
QBCore.Commands.Add("addgarage", Lang:t('info.add_garage'), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "realestate" then
        TriggerClientEvent("poly-houses:client:addGarage", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.realestate_only"), "error")
    end
end)
...
QBCore.Commands.Add("ring", Lang:t("info.ring_doorbell"), {}, false, function(source)
    local src = source
    TriggerClientEvent('poly-houses:client:RequestRing', src)
end)
...
QBCore.Functions.CreateUseableItem("police_stormram", function(source, _)
    ...
    if (Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty) then
        TriggerClientEvent("poly-houses:client:HomeInvasion", source)
    else
...
RegisterNetEvent('poly-houses:server:setHouses', function()
    local src = source
    TriggerClientEvent("poly-houses:client:setHouseConfig", src, Config.Houses)
end)

RegisterNetEvent('poly-houses:server:createBlip', function()
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    TriggerClientEvent("poly-houses:client:createBlip", -1, coords)
end)

RegisterNetEvent('poly-houses:server:addNewHouse', function(street, coords, price, tier)
    local src = source
    street = street:gsub("%'", "")
    ...
    TriggerEvent('poly-log:server:CreateLog', 'house', Lang:t("log.house_created"), 'green', Lang:t("log.house_address", {label = label, price = price, tier = tier, agent = GetPlayerName(src)}))
end)

RegisterNetEvent('poly-houses:server:addGarage', function(house, coords)
    local src = source
    ...
    TriggerClientEvent("poly-garages:client:addHouseGarage", -1, house, garageInfo)
    TriggerClientEvent('QBCore:Notify', src, Lang:t("info.added_garage", {value = garageInfo.label}))
end)

RegisterNetEvent('poly-houses:server:viewHouse', function(house)
    local src = source
    ...
    TriggerClientEvent('poly-houses:client:viewHouse', src, houseprice, brokerfee, bankfee, taxes,
        pData.PlayerData.charinfo.firstname, pData.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('poly-houses:server:buyHouse', function(house)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    ...
    if (bankBalance >= HousePrice) then
        ...
        TriggerClientEvent('poly-houses:client:SetClosestHouse', src)
        TriggerClientEvent('poly-house:client:RefreshHouseTargets', src)
        pData.Functions.RemoveMoney('bank', HousePrice, "bought-house") -- 21% Extra house costs
        exports['poly-management']:AddMoney("realestate", (HousePrice / 100) * math.random(18, 25))
        TriggerEvent('poly-log:server:CreateLog', 'house', Lang:t("log.house_purchased"), 'green', Lang:t("log.house_purchased_by", {house = house:upper(), price = HousePrice, firstname = pData.PlayerData.charinfo.firstname, lastname = pData.PlayerData.charinfo.lastname}))
...
RegisterNetEvent('poly-houses:server:lockHouse', function(bool, house)
    TriggerClientEvent('poly-houses:client:lockHouse', -1, bool, house)
end)

RegisterNetEvent('poly-houses:server:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    TriggerClientEvent('poly-houses:server:SetRamState', -1, bool, house)
end)

RegisterNetEvent('poly-houses:server:giveKey', function(house, target)
...
RegisterNetEvent('poly-houses:server:removeHouseKey', function(house, citizenData)
    local src = source
    local newHolders = {}
    if housekeyholders[house] then
...
RegisterNetEvent('poly-houses:server:OpenDoor', function(target, house)
    local OtherPlayer = QBCore.Functions.GetPlayer(target)
    if OtherPlayer then
        TriggerClientEvent('poly-houses:client:SpawnInApartment', OtherPlayer.PlayerData.source, house)
    end
end)

RegisterNetEvent('poly-houses:server:RingDoor', function(house)
    local src = source
    TriggerClientEvent('poly-houses:client:RingDoor', -1, src, house)
end)

RegisterNetEvent('poly-houses:server:savedecorations', function(house, decorations)
    MySQL.update('UPDATE player_houses SET decorations = ? WHERE house = ?', {json.encode(decorations), house})
    TriggerClientEvent("poly-houses:server:sethousedecorations", -1, house, decorations)
end)

RegisterNetEvent('poly-houses:server:LogoutLocation', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('poly-multicharacter:client:chooseChar', src)
end)

RegisterNetEvent('poly-houses:server:giveHouseKey', function(target, house)
    local src = source
    ...
    if tPlayer then
        if housekeyholders[house] then
            ...
            MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', {json.encode(housekeyholders[house]), house})
            TriggerClientEvent('poly-houses:client:refreshHouse', tPlayer.PlayerData.source)
...
RegisterNetEvent('poly-houses:server:setLocation', function(coords, house, type)
    if type == 1 then
    ...
    TriggerClientEvent('poly-houses:client:refreshLocations', -1, house, json.encode(coords), type)
end)

RegisterNetEvent('poly-houses:server:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    TriggerClientEvent('poly-houses:client:SetHouseRammed', -1, bool, house)
end)

RegisterNetEvent('poly-houses:server:SetInsideMeta', function(insideId, bool)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('poly-houses:server:buyFurniture', function(source, cb, price)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    local bankBalance = pData.PlayerData.money["bank"]
...
QBCore.Functions.CreateCallback('poly-houses:server:ProximityKO', function(source, cb, house)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local retvalK = false
    local retvalO
...
QBCore.Functions.CreateCallback('poly-houses:server:hasKey', function(source, cb, house)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local retval = false
    if Player then
...
QBCore.Functions.CreateCallback('poly-houses:server:isOwned', function(source, cb, house)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('poly-houses:server:getHouseOwner', function(_, cb, house)
    cb(houseownercid[house])
end)

QBCore.Functions.CreateCallback('poly-houses:server:getHouseKeyHolders', function(source, cb, house)
    local retval = {}
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if housekeyholders[house] then
...
QBCore.Functions.CreateCallback('poly-phone:server:TransferCid', function(_, cb, NewCid, house)
...
QBCore.Functions.CreateCallback('poly-houses:server:getHouseDecorations', function(_, cb, house)
    local retval = nil
...
QBCore.Functions.CreateCallback('poly-houses:server:getHouseLocations', function(_, cb, house)
    local retval = nil
    local result = MySQL.query.await('SELECT * FROM player_houses WHERE house = ?', {house})
    if result[1] then
        retval = result[1]
...
QBCore.Functions.CreateCallback('poly-houses:server:getOwnedHouses', function(source, cb)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    if pData then
...
QBCore.Functions.CreateCallback('poly-houses:server:getSavedOutfits', function(source, cb)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('poly-phone:server:GetPlayerHouses', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local MyHouses = {}
...
QBCore.Functions.CreateCallback('poly-phone:server:GetHouseKeys', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local MyKeys = {}
...
QBCore.Functions.CreateCallback('poly-phone:server:MeosGetPlayerHouses', function(_, cb, input)
    if input then

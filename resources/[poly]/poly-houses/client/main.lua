QBCore = exports['poly-core']:GetCoreObject()
IsInside = false
ClosestHouse = nil
HasHouseKey = false
...
local function showEntranceHeaderMenu()
    ...
    if not isOwned then
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "poly-houses:client:ViewHouse",
                args = {}
            }
        }
    else
        if isOwned and HasHouseKey then
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "poly-houses:client:EnterHouse",
                    args = {}
                }
            }
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "poly-houses:client:giveHouseKey",
                    args = {}
                }
            }
        elseif isOwned and not HasHouseKey then
            headerMenu[#headerMenu+1] = {
                ...
                params = {
                    event = "poly-houses:client:RequestRing",
                    args = {}
                }
            }
            headerMenu[#headerMenu+1] = {
            ...
            if QBCore.Functions.GetPlayerData().job and QBCore.Functions.GetPlayerData().job.name == 'police' then
                headerMenu[#headerMenu+1] = {
                    ...
                    params = {
                        event = "poly-houses:client:ResetHouse",
                        args = {}
                    }
                }
            end
    ...
    headerMenu[#headerMenu + 1] = {
        ...
        params = {
            event = 'poly-menu:client:closeMenu'
        }
    }

    if headerMenu and next(headerMenu) then
        exports['poly-menu']:openMenu(headerMenu)
    end
end
...
local function showExitHeaderMenu()
    ...
    headerMenu[#headerMenu+1] = {
        ...
        params = {
            event = "poly-houses:client:ExitOwnedHouse",
            args = {}
        }
    }
    if isOwned then
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "poly-houses:client:FrontDoorCam",
                args = {}
            }
        }
        headerMenu[#headerMenu+1] = {
            ...
            params = {
                event = "poly-houses:client:AnswerDoorbell",
                args = {}
            }
        }
    end
...
local function RegisterStashTarget()
    ...
    stashTargetBox:onPlayerInOut(function (isPointInside)
        if isPointInside and not entering and isOwned then
            exports['poly-core']:DrawText(Lang:t("target.open_stash"), 'left')
        else
            exports['poly-core']:HideText()
        end
...
local function RegisterOutfitsTarget()
    ...
    outfitsTargetBox:onPlayerInOut(function (isPointInside)
        if isPointInside and not entering and isOwned then
            exports['poly-core']:DrawText(Lang:t("target.outfits"), 'left')
        else
            exports['poly-core']:HideText()
        end
...
local function RegisterCharactersTarget()
    ...
    charactersTargetBox:onPlayerInOut(function (isPointInside)
        if isPointInside and not entering and isOwned then
            exports['poly-core']:DrawText(Lang:t("target.change_character"), 'left')
        else
            exports['poly-core']:HideText()
        end
...
RegisterNetEvent('poly-houses:client:setHouseConfig', function(houseConfig)
...
local function FrontDoorCam(coords)
    ...
    SetCamActive(cam, true)
    RenderScriptCams(true, true, 500, true, true)
    TriggerEvent('poly-weathersync:client:EnableSync')
    FrontCam = true
...
local function SetClosestHouse()
    ...
    if not IsInside then
        ...
        if ClosestHouse ~= nil and tonumber(dist) < 30 then
            QBCore.Functions.TriggerCallback('poly-houses:server:ProximityKO', function(key, owned)
                HasHouseKey = key
                isOwned = owned
            end, ClosestHouse)
        end
    end
    TriggerEvent('poly-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
end

local function setHouseLocations()
    if ClosestHouse ~= nil then
        QBCore.Functions.TriggerCallback('poly-houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
local function LoadDecorations(house)
	if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
		QBCore.Functions.TriggerCallback('poly-houses:server:getHouseDecorations', function(result)
...
function CloseMenuFull()
    exports['poly-menu']:closeMenu()
end

local function RemoveHouseKey(citizenData)
    TriggerServerEvent('poly-houses:server:removeHouseKey', ClosestHouse, citizenData)
    CloseMenuFull()
end
...
local function getKeyHolders()
    ...
    local p = promise.new()
    QBCore.Functions.TriggerCallback('poly-houses:server:getHouseKeyHolders', function(holders)
        p:resolve(holders)
    end,ClosestHouse)
    ...
    if holders == nil or next(holders) == nil then
        ...
        for k, _ in pairs(holders) do
            keyholderMenu[#keyholderMenu+1] = {
                ...
                params = {
                    event = "poly-houses:client:OpenClientOptions",
                    args = {
        ...
        exports['poly-menu']:openMenu(keyholderMenu)
    end
    ...
    keyholderOptions = {
        {
            ...
            params = {
                event = "poly-houses:client:RevokeKey",
                args = {
        ...
        },
        {
            ...
            params = {
                event = "poly-houses:client:removeHouseKey",
                args = {}
            }
        },
    }

    exports['poly-menu']:openMenu(keyholderOptions)
end
...
local function getDataForHouseTier(house, coords)
    if Config.Houses[house].tier == 1 then
        return exports['poly-interior']:CreateApartmentShell(coords)
    elseif Config.Houses[house].tier == 2 then
        return exports['poly-interior']:CreateTier1House(coords)
    elseif Config.Houses[house].tier == 3 then
        return exports['poly-interior']:CreateTrevorsShell(coords)
    elseif Config.Houses[house].tier == 4 then
        return exports['poly-interior']:CreateCaravanShell(coords)
    elseif Config.Houses[house].tier == 5 then
        return exports['poly-interior']:CreateLesterShell(coords)
    elseif Config.Houses[house].tier == 6 then
        return exports['poly-interior']:CreateRanchShell(coords)
    elseif Config.Houses[house].tier == 7 then
        return exports['poly-interior']:CreateContainer(coords)
    elseif Config.Houses[house].tier == 8 then
        return exports['poly-interior']:CreateFurniMid(coords)
    elseif Config.Houses[house].tier == 9 then
        return exports['poly-interior']:CreateFurniMotelModern(coords)
    elseif Config.Houses[house].tier == 10 then
        return exports['poly-interior']:CreateFranklinAunt(coords)
    elseif Config.Houses[house].tier == 11 then
        return exports['poly-interior']:CreateGarageMed(coords)
    elseif Config.Houses[house].tier == 12 then
        return exports['poly-interior']:CreateMichael(coords)
    elseif Config.Houses[house].tier == 13 then
        return exports['poly-interior']:CreateOffice1(coords)
    elseif Config.Houses[house].tier == 14 then
        return exports['poly-interior']:CreateStore1(coords)
    elseif Config.Houses[house].tier == 15 then
        return exports['poly-interior']:CreateWarehouse1(coords)
    elseif Config.Houses[house].tier == 16 then
        return exports['poly-interior']:CreateFurniMotelStandard(coords) -- End of free shells
    elseif Config.Houses[house].tier == 17 then
        return exports['poly-interior']:CreateMedium2(coords)
    elseif Config.Houses[house].tier == 18 then
        return exports['poly-interior']:CreateMedium3(coords)
    elseif Config.Houses[house].tier == 19 then
        return exports['poly-interior']:CreateBanham(coords)
    elseif Config.Houses[house].tier == 20 then
        return exports['poly-interior']:CreateWestons(coords)
    elseif Config.Houses[house].tier == 21 then
        return exports['poly-interior']:CreateWestons2(coords)
    elseif Config.Houses[house].tier == 22 then
        return exports['poly-interior']:CreateClassicHouse(coords)
    elseif Config.Houses[house].tier == 23 then
        return exports['poly-interior']:CreateClassicHouse2(coords)
    elseif Config.Houses[house].tier == 24 then
        return exports['poly-interior']:CreateClassicHouse3(coords)
    elseif Config.Houses[house].tier == 25 then
        return exports['poly-interior']:CreateHighend1(coords)
    elseif Config.Houses[house].tier == 26 then
        return exports['poly-interior']:CreateHighend2(coords)
    elseif Config.Houses[house].tier == 27 then
        return exports['poly-interior']:CreateHighend3(coords)
    elseif Config.Houses[house].tier == 28 then
        return exports['poly-interior']:CreateHighend(coords)
    elseif Config.Houses[house].tier == 29 then
        return exports['poly-interior']:CreateHighendV2(coords)
    elseif Config.Houses[house].tier == 30 then
        return exports['poly-interior']:CreateStashHouse(coords)
    elseif Config.Houses[house].tier == 31 then
        return exports['poly-interior']:CreateStashHouse2(coords)
    elseif Config.Houses[house].tier == 32 then
        return exports['poly-interior']:CreateGarageLow(coords)
    elseif Config.Houses[house].tier == 33 then
        return exports['poly-interior']:CreateGarageHigh(coords)
    elseif Config.Houses[house].tier == 34 then
        return exports['poly-interior']:CreateOffice2(coords)
    elseif Config.Houses[house].tier == 35 then
        return exports['poly-interior']:CreateOfficeBig(coords)
    elseif Config.Houses[house].tier == 36 then
        return exports['poly-interior']:CreateBarber(coords)
    elseif Config.Houses[house].tier == 37 then
        return exports['poly-interior']:CreateGunstore(coords)
    elseif Config.Houses[house].tier == 38 then
        return exports['poly-interior']:CreateStore2(coords)
    elseif Config.Houses[house].tier == 39 then
        return exports['poly-interior']:CreateStore3(coords)
    elseif Config.Houses[house].tier == 40 then
        return exports['poly-interior']:CreateWarehouse2(coords)
    elseif Config.Houses[house].tier == 41 then
        return exports['poly-interior']:CreateWarehouse3(coords)
    elseif Config.Houses[house].tier == 42 then
        return exports['poly-interior']:CreateK4Coke(coords)
    elseif Config.Houses[house].tier == 43 then
        return exports['poly-interior']:CreateK4Meth(coords)
    elseif Config.Houses[house].tier == 44 then
        return exports['poly-interior']:CreateK4Weed(coords)
    elseif Config.Houses[house].tier == 45 then
        return exports['poly-interior']:CreateContainer2(coords)
    elseif Config.Houses[house].tier == 46 then
        return exports['poly-interior']:CreateFurniStash1(coords)
    elseif Config.Houses[house].tier == 47 then
        return exports['poly-interior']:CreateFurniStash3(coords)
    elseif Config.Houses[house].tier == 48 then
        return exports['poly-interior']:CreateFurniLow(coords)
    elseif Config.Houses[house].tier == 49 then
        return exports['poly-interior']:CreateFurniMotel(coords)
    elseif Config.Houses[house].tier == 50 then
        return exports['poly-interior']:CreateFurniMotelClassic(coords)
    elseif Config.Houses[house].tier == 51 then
        return exports['poly-interior']:CreateFurniMotelHigh(coords)
    elseif Config.Houses[house].tier == 52 then
        return exports['poly-interior']:CreateFurniMotelModern2(coords)
    elseif Config.Houses[house].tier == 53 then
        return exports['poly-interior']:CreateFurniMotelModern3(coords)
    elseif Config.Houses[house].tier == 54 then
        return exports['poly-interior']:CreateCoke(coords)
    elseif Config.Houses[house].tier == 55 then
        return exports['poly-interior']:CreateCoke2(coords)
    elseif Config.Houses[house].tier == 56 then
        return exports['poly-interior']:CreateMeth(coords)
    elseif Config.Houses[house].tier == 57 then
        return exports['poly-interior']:CreateWeed(coords)
    elseif Config.Houses[house].tier == 58 then
        return exports['poly-interior']:CreateWeed2(coords)
    elseif Config.Houses[house].tier == 59 then
        return exports['poly-interior']:CreateMansion(coords)
    elseif Config.Houses[house].tier == 60 then
        return exports['poly-interior']:CreateMansion2(coords)
    elseif Config.Houses[house].tier == 61 then
        return exports['poly-interior']:CreateMansion3(coords)
    elseif Config.Houses[house].tier == 62 then
        return exports['poly-interior']:CreateHotel1(coords)
    elseif Config.Houses[house].tier == 63 then
        return exports['poly-interior']:CreateHotel2(coords)
    elseif Config.Houses[house].tier == 64 then
        return exports['poly-interior']:CreateHotel3(coords)
    elseif Config.Houses[house].tier == 65 then
        return exports['poly-interior']:CreateMotel1(coords)
    elseif Config.Houses[house].tier == 66 then
        return exports['poly-interior']:CreateMotel2(coords)
    elseif Config.Houses[house].tier == 67 then
        return exports['poly-interior']:CreateMotel3(coords)
    elseif Config.Houses[house].tier == 68 then
        return exports['poly-interior']:CreateV2Default1(coords)
    elseif Config.Houses[house].tier == 69 then
        return exports['poly-interior']:CreateV2Default2(coords)
    elseif Config.Houses[house].tier == 70 then
        return exports['poly-interior']:CreateV2Default3(coords)
    elseif Config.Houses[house].tier == 71 then
        return exports['poly-interior']:CreateV2Default4(coords)
    elseif Config.Houses[house].tier == 72 then
        return exports['poly-interior']:CreateV2Default5(coords)
    elseif Config.Houses[house].tier == 73 then
        return exports['poly-interior']:CreateV2Default6(coords)
    elseif Config.Houses[house].tier == 74 then
        return exports['poly-interior']:CreateV2Deluxe1(coords)
    elseif Config.Houses[house].tier == 75 then
        return exports['poly-interior']:CreateV2Deluxe2(coords)
    elseif Config.Houses[house].tier == 76 then
        return exports['poly-interior']:CreateV2Deluxe3(coords)
    elseif Config.Houses[house].tier == 77 then
        return exports['poly-interior']:CreateV2HighEnd1(coords)
    elseif Config.Houses[house].tier == 78 then
        return exports['poly-interior']:CreateV2HighEnd2(coords)
    elseif Config.Houses[house].tier == 79 then
        return exports['poly-interior']:CreateV2HighEnd3(coords)
    elseif Config.Houses[house].tier == 80 then
        return exports['poly-interior']:CreateV2Medium1(coords)
    elseif Config.Houses[house].tier == 81 then
        return exports['poly-interior']:CreateV2Medium2(coords)
    elseif Config.Houses[house].tier == 82 then
        return exports['poly-interior']:CreateV2Medium3(coords)
    elseif Config.Houses[house].tier == 83 then
        return exports['poly-interior']:CreateV2Modern1(coords)
    elseif Config.Houses[house].tier == 84 then
        return exports['poly-interior']:CreateV2Modern2(coords)
    elseif Config.Houses[house].tier == 85 then
        return exports['poly-interior']:CreateV2Modern3(coords)
    else
...
local function enterOwnedHouse(house)
    ...
    entering = true
    Wait(500)
    TriggerServerEvent('poly-houses:server:SetInsideMeta', house, true)
    --TriggerEvent('poly-weathersync:client:DisableSync')
    TriggerEvent('poly-weathersync:client:EnableSync')
    TriggerEvent('poly-weed:client:getHousePlants', house)
    entering = false
    setHouseLocations()
    CloseMenuFull()
...
local function LeaveHouse(house)
    if not FrontCam then
        ...
        DoScreenFadeOut(250)
        Wait(500)
        exports['poly-interior']:DespawnInterior(houseObj, function()
            UnloadDecorations()
            TriggerEvent('poly-weathersync:client:EnableSync')
            Wait(250)
            DoScreenFadeIn(250)
            ...
            TriggerEvent('poly-weed:client:leaveHouse')
            TriggerServerEvent('poly-houses:server:SetInsideMeta', house, false)
            CurrentHouse = nil
...
RegisterNetEvent('poly-houses:server:sethousedecorations', function(house, decorations)
...
RegisterNetEvent('poly-houses:client:sellHouse', function()
    if ClosestHouse and HasHouseKey then
        TriggerServerEvent('poly-houses:server:viewHouse', ClosestHouse)
    end
end)

RegisterNetEvent('poly-houses:client:EnterHouse', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
...
RegisterNetEvent('poly-houses:client:RequestRing', function()
    if ClosestHouse ~= nil then
        TriggerServerEvent('poly-houses:server:RingDoor', ClosestHouse)
    end
end)

AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent('poly-houses:server:setHouses')
    SetClosestHouse()
    TriggerEvent('poly-houses:client:setupHouseBlips')
    if Config.UnownedBlips then TriggerEvent('poly-houses:client:setupHouseBlips2') end
    Wait(100)
    TriggerEvent('poly-garages:client:setHouseGarage', ClosestHouse, HasHouseKey)
    TriggerServerEvent("poly-houses:server:setHouses")
end)
...
RegisterNetEvent('poly-houses:client:lockHouse', function(bool, house)
    Config.Houses[house].locked = bool
end)

RegisterNetEvent('poly-houses:client:createHouses', function(price, tier)
    ...
    street = street:gsub("%-", " ")
    TriggerServerEvent('poly-houses:server:addNewHouse', street, coords, price, tier)
    if Config.UnownedBlips then TriggerServerEvent('poly-houses:server:createBlip') end
end)

RegisterNetEvent('poly-houses:client:addGarage', function()
    if ClosestHouse ~= nil then
        local pos = GetEntityCoords(PlayerPedId())
        local heading = GetEntityHeading(PlayerPedId())
        local coords = {
        ...
        TriggerServerEvent('poly-houses:server:addGarage', ClosestHouse, coords)
    else
        QBCore.Functions.Notify(Lang:t("error.no_house"), "error")
    end
end)

RegisterNetEvent('poly-houses:client:toggleDoorlock', function()
    local pos = GetEntityCoords(PlayerPedId())
    local dist = #(pos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
    if dist <= 1.5 then
        if HasHouseKey then
            if Config.Houses[ClosestHouse].locked then
                TriggerServerEvent('poly-houses:server:lockHouse', false, ClosestHouse)
                QBCore.Functions.Notify(Lang:t("success.unlocked"), "success", 2500)
            else
                TriggerServerEvent('poly-houses:server:lockHouse', true, ClosestHouse)
...
RegisterNetEvent('poly-houses:client:RingDoor', function(player, house)
...
RegisterNetEvent('poly-houses:client:giveHouseKey', function()
    ...
    if player ~= -1 and distance < 2.5 and ClosestHouse ~= nil then
        ...
        if housedist < 10 then
            TriggerServerEvent('poly-houses:server:giveHouseKey', playerId, ClosestHouse)
        else
...
RegisterNetEvent('poly-houses:client:removeHouseKey', function()
    if ClosestHouse ~= nil then
        local pedpos = GetEntityCoords(PlayerPedId())
        local housedist = #(pedpos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
        if housedist <= 5 then
            QBCore.Functions.TriggerCallback('poly-houses:server:getHouseOwner', function(result)
...
RegisterNetEvent('poly-houses:client:RevokeKey', function(cData)
    RemoveHouseKey(cData.citizenData)
end)

RegisterNetEvent('poly-houses:client:refreshHouse', function()
    Wait(100)
    SetClosestHouse()
end)

RegisterNetEvent('poly-houses:client:SpawnInApartment', function(house)
    local pos = GetEntityCoords(PlayerPedId())
...
RegisterNetEvent('poly-houses:client:enterOwnedHouse', function(house)
...
RegisterNetEvent('poly-houses:client:LastLocationHouse', function(houseId)
    QBCore.Functions.GetPlayerData(function(PlayerData)
		if PlayerData.metadata["injail"] == 0 then
...
RegisterNetEvent('poly-houses:client:setupHouseBlips', function() -- Setup owned on load
    CreateThread(function()
        Wait(2000)
        if LocalPlayer.state['isLoggedIn'] then
            QBCore.Functions.TriggerCallback('poly-houses:server:getOwnedHouses', function(ownedHouses)
                if ownedHouses then
...
RegisterNetEvent('poly-houses:client:setupHouseBlips2', function() -- Setup unowned on load
...
RegisterNetEvent('poly-houses:client:createBlip', function(coords) -- Create unowned on command
...
RegisterNetEvent('poly-houses:client:refreshBlips', function() -- Refresh unowned on buy
    for _, v in pairs(UnownedHouseBlips) do RemoveBlip(v) end
    Wait(250)
    TriggerEvent('poly-houses:client:setupHouseBlips2')
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('poly-houses:client:SetClosestHouse', function()
    SetClosestHouse()
end)

RegisterNetEvent('poly-houses:client:viewHouse', function(houseprice, brokerfee, bankfee, taxes, firstname, lastname)
...
RegisterNetEvent('poly-houses:client:setLocation', function(cData)
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    ...
    if IsInside then
        if HasHouseKey then
            if cData.id == 'setstash' then
                TriggerServerEvent('poly-houses:server:setLocation', coords, ClosestHouse, 1)
            elseif cData.id == 'setoutift' then
                TriggerServerEvent('poly-houses:server:setLocation', coords, ClosestHouse, 2)
            elseif cData.id == 'setlogout' then
                TriggerServerEvent('poly-houses:server:setLocation', coords, ClosestHouse, 3)
            end
        else
            QBCore.Functions.Notify(Lang:t("error.not_owner"), "error")
        end
...
RegisterNetEvent('poly-houses:client:refreshLocations', function(house, location, type)
...
RegisterNetEvent('poly-houses:client:HomeInvasion', function()
    local ped = PlayerPedId()
    local pos = GetEntityCoords(ped)
    local Skillbar = exports['poly-skillbar']:GetSkillbarObject()
    if ClosestHouse ~= nil then
                        ...
                        if not Config.Houses[ClosestHouse].IsRaming then
                            ...
                            Skillbar.Start({
                                ...
                                if RamsDone + 1 >= Config.RamsNeeded then
                                    ...
                                    TriggerServerEvent('poly-houses:server:SetHouseRammed', true, ClosestHouse)
                                    TriggerServerEvent('poly-houses:server:SetRamState', false, ClosestHouse)
                            ...
                            TriggerServerEvent('poly-houses:server:SetRamState', true, ClosestHouse)
                        else
...
RegisterNetEvent('poly-houses:client:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('poly-houses:client:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)

RegisterNetEvent('poly-houses:client:ResetHouse', function()
    if ClosestHouse ~= nil then
        if Config.Houses[ClosestHouse].IsRammed == nil then
            Config.Houses[ClosestHouse].IsRammed = false
            TriggerServerEvent('poly-houses:server:SetHouseRammed', false, ClosestHouse)
            TriggerServerEvent('poly-houses:server:SetRamState', false, ClosestHouse)
        end
...
RegisterNetEvent('poly-houses:client:ExitOwnedHouse', function()
...
RegisterNetEvent('poly-houses:client:FrontDoorCam', function()
    local door = vector3(Config.Houses[CurrentHouse].coords.enter.x + POIOffsets.exit.x, Config.Houses[CurrentHouse].coords.enter.y + POIOffsets.exit.y, Config.Houses[CurrentHouse].coords.enter.z - Config.MinZOffset + POIOffsets.exit.z)
    if CheckDistance(door, 1.5) then
...
RegisterNetEvent('poly-houses:client:AnswerDoorbell', function()
    ...
    if CheckDistance(door, 1.5) and CurrentDoorBell ~= 0 then
        TriggerServerEvent("poly-houses:server:OpenDoor", CurrentDoorBell, ClosestHouse)
        CurrentDoorBell = 0
    end
end)

RegisterNetEvent('poly-houses:client:OpenStash', function()
...
RegisterNetEvent('poly-houses:client:ChangeCharacter', function()
    ...
    if CheckDistance(stashLoc, 1.5) then
        ...
        exports['poly-interior']:DespawnInterior(houseObj, function()
            ...
            InOwnedHouse = false
            IsInside = false
            TriggerServerEvent('poly-houses:server:LogoutLocation')
        end)
    end
end)

RegisterNetEvent('poly-houses:client:ChangeOutfit', function()
    ...
    if CheckDistance(outfitLoc, 1.5) then
        ...
        TriggerEvent('poly-clothing:client:openOutfitMenu')
    end
end)

RegisterNetEvent('poly-houses:client:ViewHouse', function()
...
RegisterNetEvent('poly-houses:client:KeyholderOptions', function(cData)
    optionMenu(cData.citizenData)
end)

RegisterNetEvent('poly-house:client:RefreshHouseTargets', function ()
    DeleteHousesTargets()
    SetHousesEntranceTargets()
end)
...
RegisterNUICallback('HasEnoughMoney', function(cData, cb)
    QBCore.Functions.TriggerCallback('poly-houses:server:HasEnoughMoney', function(_)
        cb('ok')
...
RegisterNUICallback('buy', function(_, cb)
    ...
    if Config.UnownedBlips then TriggerEvent('poly-houses:client:refreshBlips') end
    TriggerServerEvent('poly-houses:server:buyHouse', ClosestHouse)
    cb("ok")
end)
        ...
        if IsInside then
            ...
            if isInsideStashTarget then
                ...
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('poly-houses:client:OpenStash')
                    exports['poly-core']:HideText()
                end
            end
            ...
            if isInsideOutfitsTarget then
                wait = 0
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('poly-houses:client:ChangeOutfit')
                    exports['poly-core']:HideText()
                end
            end
            ...
            if isInsiteCharactersTarget then
                wait = 0
                if IsControlJustPressed(0, 38) then
                    TriggerEvent('poly-houses:client:ChangeCharacter')
                    exports['poly-core']:HideText()
                end
            end
        end

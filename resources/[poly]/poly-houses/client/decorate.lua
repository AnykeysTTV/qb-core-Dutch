...
local function EnableEditMode()
	...
	CreateEditCamera()
	DecoMode = true
	TriggerEvent('poly-anticheat:client:ToggleDecorate', true)
end
...
local function SaveDecorations()
	if ClosestHouse then
		...
		TriggerServerEvent("poly-houses:server:savedecorations", ClosestHouse, ObjectList)
	end
end
...
local function DisableEditMode()
	...
	peanut = false
	DecoMode = false
	TriggerEvent('poly-anticheat:client:ToggleDecorate', false)
end
...
RegisterNetEvent('poly-houses:client:decorate', function()
	Wait(500)
	if IsInside then
		if HasHouseKey then
...
RegisterNUICallback("buySelectedObject", function(data, cb)
    QBCore.Functions.TriggerCallback('poly-houses:server:buyFurniture', function(isSuccess)
        if isSuccess then
            SetNuiFocus(false, false)
            cursorEnabled = false
            SaveDecorations()

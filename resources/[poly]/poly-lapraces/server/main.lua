...
RegisterNetEvent('poly-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    local src = source
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('poly-phone:client:RaceNotify', src, 'You have won the WR from ' .. RaceData.RaceName ..
    ...
    TriggerClientEvent('poly-lapraces:client:PlayerFinishs', -1, RaceData.RaceId, PlayersFinished, Player)
    ...
    TriggerClientEvent('poly-phone:client:UpdateLapraces', -1)
end)

RegisterNetEvent('poly-lapraces:server:CreateLapRace', function(RaceName)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            TriggerClientEvent('poly-lapraces:client:StartRaceEditor', source, RaceName)
        else
...
RegisterNetEvent('poly-lapraces:server:JoinRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('poly-lapraces:client:LeaveRace', src, Races[CurrentRace])
        else
    ...
    TriggerClientEvent('poly-lapraces:client:JoinRace', src, Races[RaceId], AvailableRaces[AvailableKey].Laps)
    TriggerClientEvent('poly-phone:client:UpdateLapraces', -1)
    ...
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('poly-phone:client:RaceNotify', creatorsource,
            string.sub(Player.PlayerData.charinfo.firstname, 1, 1) .. '. ' .. Player.PlayerData.charinfo.lastname ..
                ' the race has been joined!')
    end
end)

RegisterNetEvent('poly-lapraces:server:LeaveRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local RaceName
    ...
    if (AmountOfRacers - 1) == 0 then
        ...
        table.remove(AvailableRaces, AvailableKey)
        TriggerClientEvent('QBCore:Notify', src, 'You were the only one in the race.The race had ended.', 'error')
        TriggerClientEvent('poly-lapraces:client:LeaveRace', src, Races[RaceId])
...
RegisterNetEvent('poly-lapraces:server:SetupRace', function(RaceId, Laps)
                    ...
                    if Races[RaceId].Waiting then
                        ...
                        for cid, _ in pairs(Races[RaceId].Racers) do
                            ...
                            if RacerData ~= nil then
                                TriggerClientEvent('poly-lapraces:client:LeaveRace', RacerData.PlayerData.source,
                                    Races[RaceId])
                            end
                        end
                        table.remove(AvailableRaces, AvailableKey)
...
RegisterNetEvent('poly-lapraces:server:CancelRace', function(raceId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(source)
    local AvailableKey = GetOpenedRaceKey(raceId)
    ...
    if AvailableKey ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == Player.PlayerData.citizenid then
            for cid, _ in pairs(Races[raceId].Racers) do
                local RacerData = QBCore.Functions.GetPlayerByCitizenId(cid)
                if RacerData ~= nil then
                    TriggerClientEvent('poly-lapraces:client:LeaveRace', RacerData.PlayerData.source, Races[raceId])
                end
            end
...
RegisterNetEvent('poly-lapraces:server:UpdateRaceState', function(RaceId, Started, Waiting)
    Races[RaceId].Waiting = Waiting
    Races[RaceId].Started = Started
end)

RegisterNetEvent('poly-lapraces:server:UpdateRacerData', function(RaceId, Checkpoint, Lap, Finished)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('poly-lapraces:client:UpdateRaceRacerData', -1, RaceId, Races[RaceId])
end)

RegisterNetEvent('poly-lapraces:server:StartRace', function(RaceId)
    local src = source
    ...
    if RaceId ~= nil then
        if AvailableRaces[AvailableKey].SetupCitizenId == MyPlayer.PlayerData.citizenid then
            ...
            for CitizenId, _ in pairs(Races[RaceId].Racers) do
                local Player = QBCore.Functions.GetPlayerByCitizenId(CitizenId)
                if Player ~= nil then
                    TriggerClientEvent('poly-lapraces:client:RaceCountdown', Player.PlayerData.source)
                end
            end
            TriggerClientEvent('poly-phone:client:UpdateLapraces', -1)
        else
...
RegisterNetEvent('poly-lapraces:server:SaveRace', function(RaceData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('poly-lapraces:server:GetRacingLeaderboards', function(_, cb)
    cb(Races)
end)

QBCore.Functions.CreateCallback('poly-lapraces:server:GetRaces', function(_, cb)
    cb(AvailableRaces)
end)

QBCore.Functions.CreateCallback('poly-lapraces:server:GetListedRaces', function(_, cb)
    cb(Races)
end)

QBCore.Functions.CreateCallback('poly-lapraces:server:GetRacingData', function(_, cb, RaceId)
    cb(Races[RaceId])
end)

QBCore.Functions.CreateCallback('poly-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(QBCore.Functions.GetPlayer(source).PlayerData.citizenid))
end)

QBCore.Functions.CreateCallback('poly-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
    cb(IsWhitelisted(QBCore.Functions.GetPlayer(source).PlayerData.citizenid), IsNameAvailable(TrackName))
end)

QBCore.Functions.CreateCallback('poly-lapraces:server:CanRaceSetup', function(_, cb)
    cb(Config.RaceSetupAllowed)
end)

QBCore.Functions.CreateCallback('poly-lapraces:server:GetTrackData', function(_, cb, RaceId)

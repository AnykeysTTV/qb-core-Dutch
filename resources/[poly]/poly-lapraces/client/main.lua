...
local function SaveRace()
    ...
    TriggerServerEvent('poly-lapraces:server:SaveRace', CreatorData)
...
local function FinishRace()
    TriggerServerEvent('poly-lapraces:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime, CurrentRaceData.TotalLaps, CurrentRaceData.BestLap)
...
RegisterNetEvent('poly-lapraces:client:StartRaceEditor', function(RaceName)
...
RegisterNetEvent('poly-lapraces:client:UpdateRaceRacerData', function(RaceId, aRaceData)
    if (CurrentRaceData.RaceId ~= nil) and CurrentRaceData.RaceId == RaceId then
        CurrentRaceData.Racers = aRaceData.Racers
    end
end)

RegisterNetEvent('poly-lapraces:client:JoinRace', function(Data, Laps)
    if not RaceData.InRace then
        RaceData.InRace = true
        SetupRace(Data, Laps)
        TriggerServerEvent('poly-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, false, true)
    else
        QBCore.Functions.Notify('Youre already in a race..', 'error')
    end
end)

RegisterNetEvent('poly-lapraces:client:LeaveRace', function(_)
...
RegisterNetEvent('poly-lapraces:client:RaceCountdown', function()
    TriggerServerEvent('poly-lapraces:server:UpdateRaceState', CurrentRaceData.RaceId, true, false)
...
RegisterNetEvent('poly-lapraces:client:PlayerFinishs', function(RaceId, Place, FinisherData)
    if CurrentRaceData.RaceId ~= nil then
...
RegisterNetEvent('poly-lapraces:client:WaitingDistanceCheck', function()
    Wait(1000)
    CreateThread(function()
        while true do
                ...
                if CurrentRaceData.Checkpoints[1] ~= nil then
                    ...
                    if dist > 115.0 then
                        if ToFarCountdown ~= 0 then
                            ...
                            TriggerServerEvent('poly-lapraces:server:LeaveRace', CurrentRaceData)
                            ToFarCountdown = 10
                            break
                        end
                        Wait(1000)
                ...
                if CheckpointDistance < MaxDistance then
                    if CurrentRaceData.TotalLaps == 0 then
                        if CurrentRaceData.CurrentCheckpoint + 1 < #CurrentRaceData.Checkpoints then
                            ...
                            SetNewWaypoint(CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.x, CurrentRaceData.Checkpoints[CurrentRaceData.CurrentCheckpoint + 1].coords.y)
                            TriggerServerEvent('poly-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, false)
                            DoPilePfx()
                            ...
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            CurrentRaceData.CurrentCheckpoint = CurrentRaceData.CurrentCheckpoint + 1
                            TriggerServerEvent('poly-lapraces:server:UpdateRacerData', CurrentRaceData.RaceId, CurrentRaceData.CurrentCheckpoint, CurrentRaceData.Lap, true)
                            FinishRace()
                        end
                    else
                        if CurrentRaceData.CurrentCheckpoint + 1 > #CurrentRaceData.Checkpoints then

...
local function DestroyPickupTarget()
  ...
  if Config.UseTarget then
    exports['poly-target']:RemoveZone(pickupTargetID)
    pickupZone = nil
  else
    pickupZone:destroy()
    pickupZone = nil
...
local function RegisterEntranceTarget()
  ...
  if Config.UseTarget then
    entranceZone = exports['poly-target']:AddBoxZone(entranceTargetID, coords, 1, 4, {
      ...
      options = {
        {
          type = 'client',
          event = 'poly-recyclejob:client:target:enterLocation',
    ...
    entranceZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['poly-core']:DrawText(Lang:t("text.point_enter_warehouse"), 'left')
      else
...
local function RegisterExitTarget()
  ...
  if Config.UseTarget then
    exitZone = exports['poly-target']:AddBoxZone(exitTargetID, coords, 1, 4, {
      name = exitTargetID,
      heading = 270,
      ...
      options = {
        {
          type = 'client',
          event = 'poly-recyclejob:client:target:exitLocation',
    ...
    exitZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['poly-core']:DrawText(Lang:t("text.point_exit_warehouse"), 'left')
      else
        exports['poly-core']:HideText()
      end
...
local function DestroyExitTarget()
  ...
  if Config.UseTarget then
    exports['poly-target']:RemoveZone(exitTargetID)
    exitZone = nil
  else
    exitZone:destroy()
    exitZone = nil
...
local function RegisterDutyTarget()
  ...
  if Config.UseTarget then
    dutyZone = exports['poly-target']:AddBoxZone(dutyTargetID, coords, 1, 1, {
      name = dutyTargetID,
      heading = 270,
      ...
      options = {
        {
          type = 'client',
          event = 'poly-recyclejob:client:target:toggleDuty',
    ...
    dutyZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['poly-core']:DrawText(GetDutyTargetText(), 'left')
      else
        exports['poly-core']:HideText()
      end
...
local function DestroyDutyTarget()
  ...
  if Config.UseTarget then
    exports['poly-target']:RemoveZone(dutyTargetID)
    dutyZone = nil
  else
    dutyZone:destroy()
    dutyZone = nil
...
local function RegisterDeliveyTarget()
  ...
  if Config.UseTarget then
    deliveryZone = exports['poly-target']:AddBoxZone(deliveryTargetID, coords, 1, 1, {
      ...
      options = {
        {
          type = 'client',
          event = 'poly-recyclejob:client:target:dropPackage',
    ...
    deliveryZone:onPlayerInOut(function(isPointInside)
      if isPointInside and carryPackage then
        exports['poly-core']:DrawText(Lang:t("text.point_hand_in_package"), 'left')
      else
        exports['poly-core']:HideText()
      end
...
local function DestroyDeliveryTarget()
  ...
  if Config.UseTarget then
    exports['poly-target']:RemoveZone(deliveryTargetID)
    deliveryZone = nil
  else
...
function RegisterPickupTarget(coords)
  ...
  if Config.UseTarget then
    pickupZone = exports['poly-target']:AddBoxZone(pickupTargetID, targetCoords, 4, 1.5, {
      name = pickupTargetID,
      heading = coords.h,
      minZ = coords.z - 1.0,
      maxZ = coords.z + 2.0,
      ...
      options = {
        {
          type = 'client',
          event = 'poly-recyclejob:client:target:pickupPackage',
    ...
    pickupZone:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['poly-core']:DrawText( Lang:t("text.point_get_package"), 'left')
      else
        exports['poly-core']:HideText()
      end
...
RegisterNetEvent('poly-recyclejob:client:target:enterLocation', function()
  EnterLocation()
end)

RegisterNetEvent('poly-recyclejob:client:target:exitLocation', function()
  ExitLocation()
end)

RegisterNetEvent('poly-recyclejob:client:target:toggleDuty', function()
  onDuty = not onDuty
  if onDuty then
...
RegisterNetEvent('poly-recyclejob:client:target:pickupPackage', function()
...
RegisterNetEvent('poly-recyclejob:client:target:dropPackage', function()
  ...
  QBCore.Functions.Progressbar('deliver_reycle_package',  Lang:t("text.unpacking_the_package"), Config.DeliveryActionDuration, false, true, {
      ...
      -- Done
      StopAnimTask(PlayerPedId(), 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
      TriggerServerEvent('poly-recycle:server:getItem')
      GetRandomPackage()
  end)
end)
    ...
    while true do
      ...
      if isInsideEntranceZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['poly-core']:KeyPressed()
          Wait(500)
          TriggerEvent('poly-recyclejob:client:target:enterLocation')
          exports['poly-core']:HideText()
        end
      end
      ...
      if isInsideExitZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['poly-core']:KeyPressed()
          Wait(500)
          TriggerEvent('poly-recyclejob:client:target:exitLocation')
          exports['poly-core']:HideText()
        end
      end
      ...
      if isInsideDutyZone then
        ...
        if IsControlJustReleased(0, 38) then
          exports['poly-core']:KeyPressed()
          Wait(500)
          TriggerEvent('poly-recyclejob:client:target:toggleDuty')
          exports['poly-core']:HideText()
        end
      end
      ...
      if onDuty then
        if isInsidePickupZone and not carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['poly-core']:KeyPressed()
            Wait(500)
            TriggerEvent('poly-recyclejob:client:target:pickupPackage')
            exports['poly-core']:HideText()
          end
        ...
        if isInsideDeliveryZone and carryPackage then
          ...
          if IsControlJustReleased(0, 38) then
            exports['poly-core']:KeyPressed()
            Wait(500)
            TriggerEvent('poly-recyclejob:client:target:dropPackage')
            exports['poly-core']:HideText()
          end
        end
      end

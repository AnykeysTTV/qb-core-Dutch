...
QBCore.Functions.CreateCallback("poly-garage:server:GetGarageVehicles", function(source, cb, garage, type, category)
    local src = source
...
QBCore.Functions.CreateCallback("poly-garage:server:validateGarageVehicle", function(source, cb, garage, type, plate)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    if type == "public" then        --Public garages give player cars in the garage only
...
QBCore.Functions.CreateCallback("poly-garage:server:checkOwnership", function(source, cb, plate, type, house, gang)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    ...
    elseif type == "house" then     --House garages only for player cars that have keys of the house
        MySQL.query('SELECT * FROM player_vehicles WHERE plate = ?', {plate}, function(result)
            if result[1] then
                local hasHouseKey = exports['poly-houses']:hasKey(result[1].license, result[1].citizenid, house)
                if hasHouseKey then
                    cb(true)
                else
                    cb(false)
...
QBCore.Functions.CreateCallback('poly-garage:server:spawnvehicle', function (source, cb, vehInfo, coords, warp)
...
QBCore.Functions.CreateCallback("poly-garage:server:GetVehicleProperties", function(_, cb, plate)
    local properties = {}
    local result = MySQL.query.await('SELECT mods FROM player_vehicles WHERE plate = ?', {plate})
    if result[1] then
...
QBCore.Functions.CreateCallback("poly-garage:server:IsSpawnOk", function(_, cb, plate, type)
...
RegisterNetEvent('poly-garage:server:updateVehicle', function(state, fuel, engine, body, plate, garage, type, gang)
    QBCore.Functions.TriggerCallback('poly-garage:server:checkOwnership', source, function(owned)     --Check ownership
        if owned then
...
RegisterNetEvent('poly-garage:server:updateVehicleState', function(state, plate, garage)
    local type
    ...
    QBCore.Functions.TriggerCallback('poly-garage:server:validateGarageVehicle', source, function(owned)     --Check ownership
        if owned then
...
RegisterNetEvent('poly-garages:server:UpdateOutsideVehicle', function(plate, vehicle)
...
RegisterNetEvent('poly-garage:server:PayDepotPrice', function(data)
    local src = source
    ...
    MySQL.query('SELECT * FROM player_vehicles WHERE plate = ?', {vehicle.plate}, function(result)
        if result[1] then
            if cashBalance >= result[1].depotprice then
                ...
                TriggerClientEvent("poly-garages:client:takeOutGarage", src, data)
...
--External Calls
--Call from poly-vehiclesales
QBCore.Functions.CreateCallback("poly-garage:server:checkVehicleOwner", function(source, cb, plate)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
    MySQL.query('SELECT * FROM player_vehicles WHERE plate = ? AND citizenid = ?',{plate, pData.PlayerData.citizenid}, function(result)
        if result[1] then
...
--Call from poly-phone
QBCore.Functions.CreateCallback('poly-garage:server:GetPlayerVehicles', function(source, cb)

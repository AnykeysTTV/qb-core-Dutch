...
local function MenuGarage(type, garage, indexgarage)
    ...
    exports['poly-menu']:openMenu({
        ...
        },
        {
            ...
            params = {
                event = "poly-garages:client:VehicleList",
                args = {
                    type = type,
                    garage = garage,
                    index = indexgarage,
...
local function ClearMenu()
    TriggerEvent("poly-menu:closeMenu")
end
    ...
    garageZones[type .. "_" .. index].zonecombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            if type == "in" then
                ...
                exports['poly-core']:DrawText(text, 'left')
                InputIn = true
            elseif type == "out" then
                if garage.type == "house" then
            ...
            elseif type == "house" then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    exports['poly-core']:DrawText(Lang:t("info.park_e"), 'left')
                    InputIn = true
                else
                    exports['poly-core']:DrawText(Lang:t("info.car_e"), 'left')
                    InputOut = true
                end
            end
        else
...
RegisterNetEvent("poly-garages:client:VehicleList", function(data)
    ...
    QBCore.Functions.TriggerCallback("poly-garage:server:GetGarageVehicles", function(result)
        if result == nil then
            ...
            for _, v in pairs(result) do
                ...
                if type == "depot" then
                    MenuGarageOptions[#MenuGarageOptions + 1] = {
                        ...
                        params = {
                            event = "poly-garages:client:TakeOutDepot",
                            args = {
                                vehicle = v,
                                type = type,
                                garage = garage,
                    ...
                    MenuGarageOptions[#MenuGarageOptions + 1] = {
                        ...
                        params = {
                            event = "poly-garages:client:takeOutGarage",
                            args = {
                                vehicle = v,
                                type = type,
                                garage = garage,
            ...
            exports['poly-menu']:openMenu(MenuGarageOptions)
        end
    end, indexgarage, type, garage.vehicle)
end)

RegisterNetEvent('poly-garages:client:takeOutGarage', function(data)
    local type = data.type
    local vehicle = data.vehicle
    local garage = data.garage
    local index = data.index
    QBCore.Functions.TriggerCallback('poly-garage:server:IsSpawnOk', function(spawn)
        if spawn then
            local location
            if type == "house" then
            ...
            QBCore.Functions.TriggerCallback('poly-garage:server:spawnvehicle', function(netId, properties)
                local veh = NetToVeh(netId)
                QBCore.Functions.SetVehicleProperties(veh, properties)
                exports['LegacyFuel']:SetFuel(veh, vehicle.fuel)
                doCarDamage(veh, vehicle)
                TriggerServerEvent('poly-garage:server:updateVehicleState', 0, vehicle.plate, index)
                closeMenuFull()
...
local function enterVehicle(veh, indexgarage, type, garage)
    ...
    if GetVehicleNumberOfPassengers(veh) == 0 then
        QBCore.Functions.TriggerCallback('poly-garage:server:checkOwnership', function(owned)
            if owned then
                ...
                TriggerServerEvent('poly-garage:server:updateVehicle', 1, totalFuel, engineDamage, bodyDamage, plate, indexgarage, type, PlayerGang.name)
                ...
                if plate then
                    TriggerServerEvent('poly-garages:server:UpdateOutsideVehicle', plate, nil)
                end
...
RegisterNetEvent('poly-garages:client:setHouseGarage', function(house, hasKey)
...
RegisterNetEvent('poly-garages:client:houseGarageConfig', function(garageConfig)
    Config.HouseGarages = garageConfig
end)

RegisterNetEvent('poly-garages:client:addHouseGarage', function(house, garageInfo)
...
RegisterNetEvent('poly-garages:client:TakeOutDepot', function(data)
    local vehicle = data.vehicle

    if vehicle.depotprice ~= 0 then
        TriggerServerEvent("poly-garage:server:PayDepotPrice", data)
    else
        TriggerEvent("poly-garages:client:takeOutGarage", data)
    end
end)
